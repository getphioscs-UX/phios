...
$zip = Get-ChildItem `
  ...
  -Filter "PHIOS-M4A-W1-Professional-Service-Definition-*.zip" `
  -File `
  -Recurse `
...
if (-not $zip) {
  throw "M4A-W1 ZIP not found. Download it first, then run this command again."
}

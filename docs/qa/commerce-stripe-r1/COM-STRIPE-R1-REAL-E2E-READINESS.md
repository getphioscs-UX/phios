...
Current Preview readback: STRIPE_SECRET_KEY is configured as an encrypted secret; STRIPE_ENVIRONMENT=QA is configured. STRIPE_WEBHOOK_SECRET and approved general Commerce login are still absent. No parallel Auth/Customer runtime or application test account has been created. Synthetic identities exist only inside isolated automated test fixtures.
...
- New books reuse BOOKS, BOOK_ACCESS_TOKEN_SECRET, BOOK_WATERMARK_SERVICE_URL and BOOK_WATERMARK_SERVICE_TOKEN. Non-secret `COMMERCE_BOOK_SOURCE_KEYS_JSON` maps the immutable product IDs COM-BOOK-02–07 and COM-BOOK-CONFIGURATION to owner-approved private keys; COM-BOOK-06 still means Observation (now VII), COM-BOOK-07 still means Navigation (now VIII). No key/path is guessed. Book I reuses BOOK_ONE_SOURCE_KEY.
...
3. Confirm Preview deployment SHA and non-production D1/R2 bindings. Migrations 0001–0006 have been applied and read back on QA only; see eight-volume-qa-d1.json. Migration 0006 rebuilds the existing Commerce tables in one transaction and copies all legacy columns, including download and watermark records; make a backup and verify foreign keys and row counts first.
...
Use the exact Preview URL and deployment recorded in eight-volume-remote-qa.json after successful remote health verification. The base pages.dev site is not a QA payment destination. https://getphios.com is explicitly excluded from this round's QA checkout.


## COM-8V-STRIPE-R1A dependencies

Book VI price is registered and verified at MYR109. Its Part ownership, manuscript and approved visual sources still require owner input. No private source key was inferred. Test the distinct Configuration, Observation and Navigation entitlements separately; old purchases must retain their original work. Human acceptance remains PENDING in UNIFIED-HUMAN-REVIEW.html (239 live interfaces/cases, no embedded snapshots).

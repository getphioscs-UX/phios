...
function renderElements(native){return `<section id="bazi-section-elements" class="cx-bazi-w12-section" tabindex="-1"><header class="cx-bazi-w12-section-head"><p class="cx-eyebrow">${esc(tr('FIVE ELEMENTS & TEN GODS','五行与十神'))}</p><h2>${esc(tr('Turn the chart into a visual composition you can actually read.','把命盘转成一眼能读懂的五行与十神结构。'))}</h2><p>${esc(tr('First read the Five Elements and season; then read all ten Ten-God roles, their source locations and how they connect to the current pattern layer.','先看五行与季节，再看十个十神角色、它们来自哪些柱位，以及怎样与当前格局层连接。'))}</p></header>${renderBaziFiveElementSurface(native,{embedded:true})}${renderBaziTenGodSurface(native,{embedded:true})}</section>`;}

...
import {customerLayerLabel,customerLayerExplanation,selectedCatalog} from './customer-language.js';
...
const pct=value=>{const x=n(value);return x===null?'—':`${(x*100).toFixed(1)}%`;};
...
function nodeAttrs({layer,id,title,copy,meta='',selected=false,relation=''}){
  return `tabindex="0" role="button" aria-pressed="false"${selected?' aria-current="true"':''} data-ecr-mandala-node="true" data-layer="${data(layer)}" data-node-id="${data(id)}" data-detail-title="${data(title)}" data-detail-copy="${data(copy)}" data-detail-meta="${data(meta)}" data-selected="${selected?'true':'false'}"${relation?` data-relation="${data(relation)}"`:''} aria-label="${data(`${title}. ${copy}${meta?` ${meta}`:''}`)}"`;
}
...
function ring(projection,{catalog,idKey,layerId,selectedId,labelFor,detailFor,metaFor,glyphFor=null}){
  const xs=arr(catalog),layer=PHI_MANDALA_LAYER_GEOMETRY[layerId];
  return xs.map(item=>{const id=item[idKey],selected=id===selectedId,g=ringSegmentGeometry(item.ordinal,xs.length,layer),title=labelFor(item),copy=detailFor(item),meta=metaFor?.(item)||'',baseGlyph=glyphFor?.(item)||id,glyph=selected?`● ${baseGlyph}`:baseGlyph,titleText=`${selected?`${local(projection,'Selected','当前')} · `:''}${id} · ${title}`;
    return `<g class="cx-ecr-mandala__node cx-ecr-mandala__node--${layerId.toLowerCase()}${selected?' is-selected':''}" ${nodeAttrs({layer:layerId,id,title,copy,meta,selected})}><path d="${g.path}"/><title>${esc(titleText)}</title>${textAt(g.label,glyph,{rotate:textRotation(g.mid),className:'cx-ecr-mandala__code'})}</g>`;}).join('');
}

function capabilityNetwork(projection){
  ...
  const nodes=xs.map(item=>{const primary=item.capabilityId===selected.primaryCapabilityId,supporting=arr(selected.supportingCapabilityIds).includes(item.capabilityId),g=circularNodeGeometry(item.ordinal,xs.length),title=isZh(projection)?item.labelZhHans:item.label,copy=primary?local(projection,'Primary response capability for the selected question.','当前基础问题的主要回应能力。'):supporting?local(projection,'Supporting response capability for the selected question.','当前基础问题的辅助回应能力。'):local(projection,'One of the nine governed response capabilities.','九项受治理回应能力之一。'),meta=primary?local(projection,'Primary','主要'):supporting?local(projection,'Supporting','辅助'):item.capabilityId,marker=primary?'★':supporting?'＋':'';
    return {item,g,primary,supporting,html:`<g class="cx-ecr-mandala__capability${primary?' is-primary':''}${supporting?' is-supporting':''}" ${nodeAttrs({layer:'R9',id:item.capabilityId,title,copy,meta,selected:primary||supporting,relation:primary?'PRIMARY':supporting?'SUPPORTING':''})}><circle cx="${g.x.toFixed(2)}" cy="${g.y.toFixed(2)}" r="${PHI_MANDALA_LAYER_GEOMETRY.R9.nodeSize}"/><text x="${g.x.toFixed(2)}" y="${g.y.toFixed(2)}" text-anchor="middle" dominant-baseline="middle">${esc(`${marker}${item.capabilityId}`)}</text></g>`};});
  const connectors=qPoint?nodes.filter(x=>x.primary||x.supporting).map(x=>{const c=connectorGeometry(qPoint,x.g);return `<line class="cx-ecr-mandala__relation${x.primary?' is-primary':' is-supporting'}" x1="${c.x1.toFixed(2)}" y1="${c.y1.toFixed(2)}" x2="${c.x2.toFixed(2)}" y2="${c.y2.toFixed(2)}" aria-hidden="true"/>`;}).join(''):'';
  return connectors+nodes.map(x=>x.html).join('');
}

function driverProfile(projection){
  ...
  return catalog.map(item=>{const d=index.get(item.driverId)||{},g=radialBarGeometry(item.ordinal,catalog.length,d.baselineAffinity,max),selected=d.rank===1,title=isZh(projection)?item.labelZhHans:item.label,copy=local(projection,'Birth-baseline affinity. This is not a claim about current Reality priority.','出生基线亲和度；这不是对当前现实优先级的判断。'),meta=local(projection,`Rank ${d.rank??'—'} · ${pct(d.baselineAffinity)} · distance ${deg(d.angularDistanceDegrees)}`,`第 ${d.rank??'—'} 位 · ${pct(d.baselineAffinity)} · 距离 ${deg(d.angularDistanceDegrees)}`);
    return `<g class="cx-ecr-mandala__driver${selected?' is-selected':''}" ${nodeAttrs({layer:'D12',id:item.driverId,title,copy,meta,selected})}><line x1="${g.start.x.toFixed(2)}" y1="${g.start.y.toFixed(2)}" x2="${g.end.x.toFixed(2)}" y2="${g.end.y.toFixed(2)}"/><circle cx="${g.end.x.toFixed(2)}" cy="${g.end.y.toFixed(2)}" r="${selected?4:2.5}"/><title>${esc(`${selected?local(projection,'Top baseline driver','首位基线驱动'):'Driver'} · ${item.driverId} · ${title} · ${meta}`)}</title></g>`;}).join('');
}

function configurationRing(projection){
  ...
  return xs.map(item=>{const selected=item.configurationId===selectedId,g=ringSegmentGeometry(item.ordinal,xs.length,layer),title=selected?(isZh(projection)?`${item.chineseNameZhHans} · ${item.configurationId}`:`${item.canonicalName} · ${item.configurationId}`):item.configurationId,copy=selected?local(projection,`Environment priority ${item.environmentPriorityMotionId}; embodied response ${item.embodiedResponseMotionId}.`,`环境优先运动 ${item.environmentPriorityMotionId}；载体回应运动 ${item.embodiedResponseMotionId}。`):local(projection,'One governed environment-response configuration position.','一个受治理的环境—回应构型位置。'),meta=selected?local(projection,`Upper ${item.upperTrigramRef} · Lower ${item.lowerTrigramRef}`,`上卦 ${item.upperTrigramRef} · 下卦 ${item.lowerTrigramRef}`):'';
    return `<g class="cx-ecr-mandala__node cx-ecr-mandala__node--h64${selected?' is-selected':''}" ${nodeAttrs({layer:'H64',id:item.configurationId,title,copy,meta,selected})}><path d="${g.path}"/><title>${esc(`${selected?'● ':''}${title}`)}</title></g>`;}).join('');
}

function selectedSummary(projection){
  ...
  const row=(title,x,explanation)=>`<div><dt>${esc(title)}</dt><dd><strong>${esc(x.primary)}</strong><p>${esc(explanation)}</p><small>${esc(x.secondary)}</small></dd></div>`;
  return `<div class="cx-ecr-mandala__summary" data-ecr-mandala-summary><p class="cx-eyebrow">${esc(local(projection,'YOUR COORDINATE AT A GLANCE','先看懂你的构型坐标'))}</p><p class="cx-ecr-mandala__summary-lead">${esc(local(projection,'Human-readable meaning comes first. Technical codes remain secondary so you can trace every highlighted Mandala position without having to learn the internal notation first.','先用自然语言理解每一层，再把技术编号作为追溯索引。你不需要先学会内部代码，才看得懂自己的 PHI 构型。'))}</p><dl>${row(local(projection,'Context','背景'),c,customerLayerExplanation(projection,'CC12',selected.context))}${row(local(projection,'Grammar','语法'),g,customerLayerExplanation(projection,'G16',selected.grammar))}${row(local(projection,'Baseline question','基础问题'),q,customerLayerExplanation(projection,'Q16',selected.question))}${row(local(projection,'Primary capability','主要能力'),r,customerLayerExplanation(projection,'R9',selected.primaryCapability,{role:'PRIMARY'}))}${row(local(projection,'Top baseline driver','首位基线驱动'),driver,customerLayerExplanation(projection,'D12',d))}${row(local(projection,'Change motion','变化运动'),m,customerLayerExplanation(projection,'M8',selected.motion))}${row(local(projection,'Configuration','环境—回应构型'),h,customerLayerExplanation(projection,'H64',selected.configuration))}${row(local(projection,'Activation stage','激活阶段'),a,customerLayerExplanation(projection,'A8',selected.activation))}</dl></div>`;
}

export function renderPhiMandalaVisual(visual){
  const projection=visual?.payload;if(projection?.schemaVersion!==SCHEMA)return '';
  const selected=projection.selected||{},contexts=arr(projection.catalogs?.contexts),grammars=arr(projection.catalogs?.grammars),questions=arr(projection.catalogs?.questions),motions=arr(projection.catalogs?.motions),activations=arr(projection.catalogs?.activations),context=by(contexts,'contextId',selected.contextId),question=by(questions,'questionId',selected.questionId),title=visual?.title||local(projection,'Your PHI Configuration','你的 PHI 构型'),defaultDetailTitle=context?(isZh(projection)?`${context.labelZhHans} · ${context.contextId}`:`${context.label} · ${context.contextId}`):title,defaultDetailCopy=question?(isZh(projection)?question.questionZhHans:question.question):local(projection,'Explore a layer to see how the selected coordinate is organized.','聚焦任一层，查看这次构型如何被组织。');
  const contextRing=ring(projection,{catalog:contexts,idKey:'contextId',layerId:'CC12',selectedId:selected.contextId,labelFor:item=>isZh(projection)?item.labelZhHans:item.label,detailFor:item=>local(projection,`Longitude interval ${item.startLongitudeInclusive}° to <${item.endLongitudeExclusive}°. Position label only; no zodiac personality meaning is imported.`,`黄经区间 ${item.startLongitudeInclusive}° 至 <${item.endLongitudeExclusive}°。这里只是位置名称，不导入星座人格意义。`),metaFor:item=>item.contextId,glyphFor:item=>`${zodiacGlyph[item.zodiacCode]||''} ${item.contextId}`.trim()});
  const grammarRing=ring(projection,{catalog:grammars,idKey:'grammarId',layerId:'G16',selectedId:selected.grammarId,labelFor:item=>isZh(projection)?item.labelZhHans:item.label,detailFor:item=>local(projection,'One of sixteen governed Reality Grammar positions.','十六个受治理现实语法位置之一。'),metaFor:item=>item.grammarId,glyphFor:item=>item.grammarId});
  const questionRing=ring(projection,{catalog:questions,idKey:'questionId',layerId:'Q16',selectedId:selected.questionId,labelFor:item=>isZh(projection)?item.questionZhHans:item.question,detailFor:item=>local(projection,'The baseline question paired to the same Grammar ordinal.','与同序号现实语法配对的基础问题。'),metaFor:item=>item.questionId,glyphFor:item=>item.questionId});
  ...
  const svg=`<svg class="cx-ecr-mandala__svg" data-ecr-phi-mandala="true" viewBox="0 0 ${PHI_MANDALA_VIEWBOX_SIZE} ${PHI_MANDALA_VIEWBOX_SIZE}" role="img" aria-labelledby="${esc(titleId)} ${esc(descId)}" preserveAspectRatio="xMidYMid meet"><title id="${esc(titleId)}">${esc(title)}</title><desc id="${esc(descId)}">${esc(local(projection,'Interactive PHI Configuration map. Keyboard users can focus each governed segment; selected segments are also marked with symbols, not color alone.','互动 PHI 构型图。键盘用户可以逐一聚焦受治理分区；当前选中位置除了颜色，也使用符号标记。'))}</desc><circle class="cx-ecr-mandala__bg" cx="${center.x}" cy="${center.y}" r="382"/>${guides}${contextRing}${grammarRing}${questionRing}${capabilityNetwork(projection)}${driverProfile(projection)}${motionRing}${configurationRing(projection)}${activationRing}<circle class="cx-ecr-mandala__core" cx="${center.x}" cy="${center.y}" r="${PHI_MANDALA_LAYER_GEOMETRY.CORE.radius}"/><text class="cx-ecr-mandala__core-title" x="${center.x}" y="${center.y-12}" text-anchor="middle">${esc(local(projection,'PHI CONFIGURATION','PHI 构型'))}</text><text class="cx-ecr-mandala__core-meta" x="${center.x}" y="${center.y+9}" text-anchor="middle">${esc(`${local(projection,'Solar anchor','太阳锚点')} ${deg(projection.anchor?.longitude)}`)}</text><text class="cx-ecr-mandala__core-meta" x="${center.x}" y="${center.y+26}" text-anchor="middle">${esc(local(projection,'Human labels first','先读自然语言'))}</text><text class="cx-ecr-mandala__core-meta" x="${center.x}" y="${center.y+42}" text-anchor="middle">${esc(local(projection,'Codes remain traceable','编号保留追溯'))}</text></svg>`;
  return `<section id="ecr-section-01" class="cx-ecr-mandala" data-ecr-mandala-root="true" tabindex="-1"><header><p class="cx-eyebrow">${esc(local(projection,'PHI OS ORIGINAL · INTERACTIVE CONFIGURATION MAP','PHI OS 原生体系 · 互动构型图'))}</p><h3>${esc(title)}</h3><p>${esc(local(projection,'All highlighted positions come from the governed ECR projection. The diagram calculates layout only; it does not recalculate your ECR result.','所有高亮位置都来自受治理的 ECR 投影。本图只计算视觉几何，不会重新计算你的 ECR 结果。'))}</p></header>${selectedSummary(projection)}<details class="cx-ecr-mandala__explore" data-ecr-mandala-explore open><summary>${esc(local(projection,'Explore the complete PHI Mandala','查看完整 PHI Mandala'))}</summary><div class="cx-ecr-mandala__viewport" data-ecr-mandala-scroll-region tabindex="0" aria-label="${esc(local(projection,'Scrollable full PHI Mandala','可滚动的完整 PHI Mandala'))}">${svg}</div></details><aside id="${esc(detailId)}" class="cx-ecr-mandala__detail" data-ecr-mandala-detail aria-live="polite" aria-atomic="true"><strong data-ecr-mandala-detail-title>${esc(defaultDetailTitle)}</strong><p data-ecr-mandala-detail-copy>${esc(defaultDetailCopy)}</p><small data-ecr-mandala-detail-meta>${esc(`${local(projection,'Solar anchor','太阳锚点')} ${deg(projection.anchor?.longitude)} · ${local(projection,'Technical indices remain visible for traceability.','保留技术编号以便追溯。')}`)}</small></aside><p class="cx-meta">${esc(local(projection,'Calculated astronomical position; PHI OS first-party interpretive layer mappings. Not a scientific personality fact or guaranteed outcome.','天文位置属于计算结果；各层映射属于 PHI OS 原生解释约定，不是科学人格事实或必然结果。'))}</p></section>`;
}
...
export function installPhiMandalaInteractions(scope){
  ...
  for(const root of roots){
    ...
    root.dataset.ecrMandalaInteractionInstalled='true';configureResponsiveExplore(root);
    const initial=root.querySelector('[data-ecr-mandala-node][data-selected="true"]');if(initial)activate(root,initial);
  }
  return roots.length;
}

export default Object.freeze({renderPhiMandalaVisual,installPhiMandalaInteractions});

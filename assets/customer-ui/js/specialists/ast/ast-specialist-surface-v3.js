...
export function buildTimingActivationHtml(p){
  ...
  if(!available){
    return `<section class="ast-cx-r3-panel ast-cx-r3-timing" data-astcx-section="timing-activation" tabindex="-1"><header class="ast-cx-r3-section-head"><p class="ast-cx-r3-kicker">${esc(tr('TIMING & ACTIVATION','时间与激活'))}</p><h2>${esc(tr('Natal reading is ready; current activation is not attached','本命读取已经准备好；当前激活尚未附加'))}</h2><p>${esc(tr('No target moment was included in this request, and the page does not infer one from the browser or server clock without your action. Return to the timing input, tick “Use today and now” or enter a date and time, then confirm the target place. The canonical route will call the existing transit calculation and this specialist surface will present the calculated result.','这次请求没有附带目标时刻；在你没有操作时，页面不会从浏览器或服务器时钟自行推断。请回到时间输入，勾选「使用今天和现在」或填写日期与时间，再确认目标地点。canonical route 会调用既有行运计算，随后由专业界面呈现已经算出的结果。'))}</p></header><div class="ast-cx-r3-timing-lane" data-state="unavailable"><article><small>${esc(tr('Natal','本命'))}</small><strong>${esc(tr('Available','可读取'))}</strong><span>${esc(tr('Your birth structure remains complete on its own.','你的出生结构本身仍可完整阅读。'))}</span></article><i aria-hidden="true">→</i><article><small>${esc(tr('Target moment','目标时刻'))}</small><strong>${esc(tr('Choose above','请在上方选择'))}</strong><span>${esc(tr('Date · time · confirmed place','日期 · 时间 · 已确认地点'))}</span></article><i aria-hidden="true">→</i><article><small>${esc(tr('Current activation','当前激活'))}</small><strong>${esc(tr('Calculated after submission','提交后计算'))}</strong><span>${esc(tr('No fallback prediction is generated.','不会生成替代性的预测文字。'))}</span></article></div></section>`;
  }

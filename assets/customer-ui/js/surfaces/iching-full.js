...
q('[data-iching-save]')?.addEventListener('click',async()=>{if(!currentView)return;const consent=globalThis.confirm(localeText('Save this reading to a signed guest session for the stated retention period? Nothing is saved unless you choose OK.','是否把这次阅读保存到签名访客会话，并按页面说明的保留期限保存？只有你选择“确定”后才会保存。'));if(!consent){q('[data-save-status]').textContent=localeText('Not saved.','未保存。');return;}const body={question:q('[data-iching-question]').value,methodEvidence:currentView.hierarchy?.[1]?.data,projection:currentView.hierarchy?.[2]?.data,reading:currentView,userNotes:'',retentionConsent:{accepted:true,scope:'SYMBOLIC_READING',policyVersion:'ICHING-GUEST-RETENTION-v1'}};const response=await fetch('/api/iching-full-save',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});const payload=await response.json().catch(()=>null);q('[data-save-status]').textContent=payload?.ok?localeText('Saved to this guest session.','已保存到当前访客会话。'):payload?.error?.code||localeText('Save unavailable.','暂时无法保存。');});
window.addEventListener('phios:localechange',()=>{loadContext();if(currentView)renderIChingView(currentView);});
loadContext();

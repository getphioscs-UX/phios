...
function coinPanelMarkup(){
  return `<div class="cx-cast-coin-grid">
    ${Array.from({length:6},(_,line)=>`<fieldset>
      ...
      <div>
        ${Array.from({length:3},(_,coin)=>`<label><span>${t('Coin','钱币')} ${coin+1}</span><select data-iching-coin-line="${line+1}" data-iching-coin="${coin+1}"><option value="2">2</option><option value="3">3</option></select></label>`).join('')}
      </div>
    </fieldset>`).join('')}
  </div>`;
}

...
export function installSecureDrafts({host,kind,capture,restore}){
 ...
 function render(){
  ...
  ${!saved&&drafts.length?`<ul>${drafts.map(d=>`<li><a href="${esc(draftUrl(d))}">${esc(tr('Continue saved draft','继续已保存草稿'))} · ${esc(draftState(d))}</a></li>`).join('')}</ul>`:''}`:`<a class="cx-button" href="/account/">${esc(tr('Sign in to save or continue','登录以保存或继续'))}</a>`}<p role="status" aria-live="polite" data-draft-message>${esc(typeof message==='function'?message():message)}</p>`;

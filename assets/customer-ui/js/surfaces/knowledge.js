...
function bookCard(book,catalogRecord,commerceProduct){
 ...
 const commerceReady=published&&commerceProduct?.active&&Number(book.volume)<=6;
 ...
 return `<article class="cx-knowledge-book-card"><figure class="cx-knowledge-book-card__cover cx-visual"><img data-cx-seven-volume-asset="${esc(BOOK_ASSET[code])}" alt="${esc(title)}"><span class="cx-knowledge__asset-fallback cx-meta" data-cx-asset-fallback hidden>${esc(tr('Cover temporarily unavailable','封面暂时无法显示'))}</span></figure><div class="cx-knowledge-book-card__body"><div class="cx-knowledge-meta-row"><span>${esc(tr(`Volume ${book.volume}`,`第 ${book.volume} 册`))}</span><span>${esc(published?tr('Published · complete','已出版 · 完整版'):clean(book.content_status).replaceAll('-',' '))}</span></div><h2 class="cx-heading-2">${esc(title)}</h2><p class="cx-knowledge-book-card__role">${esc(role)}</p>${subtitle&&subtitle!==role?`<p class="cx-meta">${esc(subtitle)}</p>`:''}${catalogRecord?.hasPublishedKnowledge?`<p class="cx-meta">${esc(tr(`${catalogRecord.publishedArticleCount} published articles`,`${catalogRecord.publishedArticleCount} 篇已发布文章`))}</p>`:''}<div class="cx-cluster">${purchase}<a class="cx-button cx-button--secondary" href="${esc(href)}">${esc(tr('Open volume','打开本册'))}</a><a class="cx-button cx-button--quiet" href="${esc(askHref('BOOK',code,title,href,code))}">${esc(tr('Ask about this','针对这册提问'))}</a></div></div></article>`;
}

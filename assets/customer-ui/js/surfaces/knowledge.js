...
function bookCard(book,catalogRecord){
 ...
 return `<article class="cx-knowledge-book-card"><figure class="cx-knowledge-book-card__cover cx-visual"><img data-cx-seven-volume-asset="${esc(BOOK_ASSET[code])}" alt="${esc(title)}"><span class="cx-knowledge__asset-fallback cx-meta" data-cx-asset-fallback hidden>${esc(tr('Cover temporarily unavailable','封面暂时无法显示'))}</span></figure><div class="cx-knowledge-book-card__body"><div class="cx-knowledge-meta-row"><span>${esc(tr(`Volume ${book.volume}`,`第 ${book.volume} 册`))}</span><span>${esc(clean(book.content_status).replaceAll('-',' '))}</span></div><h2 class="cx-heading-2">${esc(title)}</h2><p class="cx-knowledge-book-card__role">${esc(role)}</p>${subtitle&&subtitle!==role?`<p class="cx-meta">${esc(subtitle)}</p>`:''}${catalogRecord?.hasPublishedKnowledge?`<p class="cx-meta">${esc(tr(`${catalogRecord.publishedArticleCount} published articles`,`${catalogRecord.publishedArticleCount} 篇已发布文章`))}</p>`:''}<div class="cx-cluster"><a class="cx-button cx-button--secondary" href="${esc(href)}">${esc(tr('Open volume','打开本册'))}</a><a class="cx-button cx-button--quiet" href="${esc(askHref('BOOK',code,title,href))}">${esc(tr('Ask about this','针对这册提问'))}</a></div></div></article>`;
}
async function renderBooks(){const node=$('[data-cx-book-grid]');if(!node)return;loading(node);try{const [registry,catalog]=await Promise.all([books(),discoveryCatalog().catch(()=>({books:[]}))]);const byCode=new Map((catalog.books||[]).map(record=>[record.bookCode,record]));node.innerHTML=(registry.books||[]).sort((a,b)=>Number(a.volume)-Number(b.volume)).map(book=>bookCard(book,byCode.get(book.bookCode))).join('');await hydrateSevenVolumeAssets(document)}catch{unavailable(node)}}
...
async function searchCorpus(){
 ...
 const book6Index=await fetchJson('/content/civilization-atlas/reconfiguration/search/book-vi-reconfiguration-search-index-v1.json').catch(()=>({rows:[]}));
 const book6Layer={CASE:'reconfiguration-cases',WINDOW:'reconfiguration-windows',SNAPSHOT:'reconfiguration-snapshots',DOSSIER:'dossiers',BOOK_SECTION:'chapters',LIVED_REALITY:'lived-reality'};
 const book6Rows=(book6Index.rows||[]).map(row=>{const title=locale()==='zh-Hans'?row.titleZh:row.titleEn;const layer=book6Layer[row.type]||'overview';const route='/books/reality-configuration/#reconfiguration-atlas';const params=new URLSearchParams({contextType:'KNOWLEDGE',contextRef:'BOOK:BOOK-6',contextLabel:title,contextRoute:route,contextSummary:`Layer=${layer} · Entity=${row.id}`});return {type:row.type,ids:[row.id],title,summary:row.search||tr('Book VI reconfiguration atlas entity','第六册文明重组图谱实体'),source:tr('Book VI · Civilization Reconfiguration Atlas','第六册 · 文明重组图谱'),related:row.id,href:route,ask:'/knowledge/ask/?'+params.toString(),terms:[row.id,row.titleEn,row.titleZh,row.search,row.type].join(' ')}}); 
 return [...articleRows,...bookRows,...figureRows,...conceptRows,...structuredDiscoveryRows(structured,index.objects||index,bookRegistry,locale()),...book6Rows];
}
function resultType(type){return ({ARTICLE:tr('Article','文章'),BOOK:tr('Book','书籍'),FIGURE:tr('Figure','图示'),CONCEPT:tr('Concept','概念'),STRUCTURED_OBJECT:tr('Structured object','结构化对象'),ATLAS:tr('Atlas','图谱'),CASE:tr('Reconfiguration case','重组案例'),WINDOW:tr('Reconfiguration window','重组窗口'),SNAPSHOT:tr('World snapshot','世界横切面'),DOSSIER:tr('Runtime dossier','Runtime Dossier'),BOOK_SECTION:tr('Book VI section','第六册正文'),LIVED_REALITY:tr('Lived Reality','日常现实')})[type]||type}

...
export function renderAcceptedInsight(item){return `<article class="cx-method-insight"><p class="cx-eyebrow">${esc(tr('Reviewed insight','经过审核的重点'))}</p><h3>${esc(item.title)}</h3><p>${esc(item.body||item.plainLanguageExplanation)}</p><div class="cx-method-evidence"><strong>${esc(tr('Why this is here','为什么出现这项内容'))}</strong><p>${esc(item.summary)}</p></div><div class="cx-method-alternative"><strong>${esc(tr('Another possible expression','另一种可能表现'))}</strong><p>${esc(arr(item.alternativeInterpretations)[0]||'')}</p></div><div class="cx-method-observation"><strong>${esc(tr('Reality-comparison question','现实对照问题'))}</strong><p>${esc(arr(item.openQuestions)[0]||'')}</p></div></article>`}

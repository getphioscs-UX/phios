...
function installOne(scope){
  ...
  useNow?.addEventListener('change',()=>{if(!useNow.checked)return;if(scope.dataset.cxTargetDateOnly==='true'){state.applyingNow=true;const current=currentCivilMoment();date.value=current.date;time.value='';state.applyingNow=false;dispatchChanged(scope);setStatus(scope,tr('Today is filled as the optional Numerology target date.','已填入今天作为可选的数字学目标日期。'),'success');return}fillNow(timezone.value||null);if(placeRef.value)void resolve();else setStatus(scope,tr('Today and now are filled. Choose the target place to confirm its local timezone.','已填入今天和现在；请选择目标地点以确认当地时区。'))});

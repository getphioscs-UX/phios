var O=e=>String(e??"").replace(/[&<>"']/g,n=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[n]);function H(e,{t:n,text:a,panel:d}){let s=e.visual,t=E=>`<div data-part="metrics">${E.map(o=>`<span data-source-ref="${a(o.sourceRefs[0])}">${a(o.label)}<b>${a(o.value)}</b></span>`).join("")}</div>`,r=(E,o)=>{let l=E.map((m,T)=>({x:220+145*Math.cos(-Math.PI/2+T*2*Math.PI/E.length),y:210+145*Math.sin(-Math.PI/2+T*2*Math.PI/E.length)}));return`<svg data-part="balance-orbit" viewBox="0 0 440 420" role="img" aria-label="${a(E.map(m=>`${m.label}: ${m.value}`).join("; "))}"><circle class="vrpt-orbit" cx="220" cy="210" r="145"/>${E.map((m,T)=>`<path class="vrpt-link" d="M220 210L${l[T].x} ${l[T].y}"/>`).join("")}<g class="vrpt-node"><circle cx="220" cy="210" r="57"/><text ${o.length<3?"data-total":""} x="220" y="205" text-anchor="middle">${a(o)}</text><text data-caption x="220" y="226" text-anchor="middle">${a(n("CORE","\u6838\u5FC3"))}</text></g>${E.map((m,T)=>`<g class="vrpt-node"><circle cx="${l[T].x}" cy="${l[T].y}" r="54"/><text data-total x="${l[T].x}" y="${l[T].y+10}" text-anchor="middle">${a(m.value)}</text><text data-caption x="${l[T].x}" y="${l[T].y+36}" text-anchor="middle">${T+1}</text></g>`).join("")}</svg>`},N=E=>`<div data-part="balance-key">${E.map((o,l)=>`<div class="vrpt-tile"><h4>${l+1} \xB7 ${a(o.label)}</h4><b>${a(o.value)}</b></div>`).join("")}</div>`,u=E=>{let o=Math.max(1,...E.map(l=>l.value));return`<div data-part="balance-bars">${E.map(l=>`<div><span>${a(l.label)}</span><svg viewBox="0 0 200 14" role="img" aria-label="${a(`${l.label}: ${l.value}`)}"><rect data-track width="200" height="14" rx="7"/><rect class="vrpt-node" width="${200*l.value/o}" height="14" rx="7" fill="currentColor"/></svg><b>${l.value}</b></div>`).join("")}<small>${a(n("Raw visible counts \xB7 common scale","\u53EF\u89C1\u539F\u59CB\u8BA1\u6570 \xB7 \u7EDF\u4E00\u523B\u5EA6"))}: 0\u2013${o}</small></div>`};if(e.pageNumber===11)return d(n("Visible signal balance","\u53EF\u89C1\u4FE1\u53F7\u5206\u5E03"),u(s.signalNodes))+d(n("Separate context","\u72EC\u7ACB\u80CC\u666F"),t([{label:n("Root records","\u6839\u6C14\u8BB0\u5F55"),value:s.context.roots.value,sourceRefs:s.context.roots.sourceRefs},{label:n("Day Master","\u65E5\u4E3B"),value:s.context.dayMaster.glyph,sourceRefs:s.context.dayMaster.sourceRefs},{label:n("Month command","\u6708\u4EE4"),value:s.context.season.glyph,sourceRefs:s.context.season.sourceRefs}]));if(e.pageNumber===12)return d(n("Primary verdict remains open","\u4E3B\u683C\u5C40\u5224\u65AD\u4FDD\u6301\u5F00\u653E"),`<div data-part="season-structure">${r(s.candidates,n("Open","\u672A\u5B9A"))}${N(s.candidates)}</div>`)+d(n("Candidate evidence","\u5019\u9009\u8BC1\u636E"),`<div data-part="balance-candidates">${s.candidates.map(E=>`<section class="vrpt-tile"><h4>${a(E.label)}</h4><p>${a(E.visibleStemMatch?n("Visible stem match","\u6709\u900F\u5E72\u5BF9\u5E94"):n("No visible stem match","\u672A\u89C1\u900F\u5E72\u5BF9\u5E94"))}</p><p>${a(n("Visible paths / recorded paths","\u53EF\u89C1\u8DEF\u5F84\uFF0F\u5DF2\u8BB0\u5F55\u8DEF\u5F84"))}: ${E.visiblePaths} / ${E.pathCount}</p></section>`).join("")||`<p>${a(n("No candidates recorded.","\u65E0\u5DF2\u8BB0\u5F55\u5019\u9009\u3002"))}</p>`}</div>`);if(e.pageNumber===13)return d(n("Candidate condition map","\u5019\u9009\u6761\u4EF6\u56FE"),`<div data-part="balance-conditions">${s.candidates.map(E=>`<section class="vrpt-tile"><h4>${a(E.label)} \xB7 ${a(E.value)}</h4>${s.paths.filter(o=>o.candidateId===E.id).map((o,l)=>`<div data-part="condition-row" data-state="${o.state}"><small>${l+1} \xB7 ${a(o.pathLabel||o.pathCode.replaceAll("_"," "))}</small><strong>${a(o.value)}</strong></div>`).join("")}</section>`).join("")||`<p>${a(n("No recorded candidate conditions.","\u65E0\u5DF2\u8BB0\u5F55\u5019\u9009\u6761\u4EF6\u3002"))}</p>`}</div>`)+d(n("Verdict boundaries","\u5224\u65AD\u8FB9\u754C"),t([{label:n("Primary","\u4E3B\u683C\u5C40"),value:n("Open","\u672A\u5B9A"),sourceRefs:e.evidenceRefs},{label:n("Secondary","\u6B21\u8981\u683C\u5C40"),value:n("Not set","\u672A\u6307\u5B9A"),sourceRefs:e.evidenceRefs},{label:n("Candidates","\u5019\u9009"),value:s.candidates.length,sourceRefs:e.evidenceRefs}]));if(e.pageNumber===14)return d(n("Support records around the Day Master","\u65E5\u4E3B\u5468\u56F4\u7684\u652F\u6301\u8BB0\u5F55"),`<div data-part="season-structure">${r(s.supportNodes,s.context.dayMaster.glyph)}${N(s.supportNodes)}</div>`)+d(n("Recorded root positions","\u5DF2\u8BB0\u5F55\u6839\u6C14\u4F4D\u7F6E"),s.rootRecords.length?t(s.rootRecords):`<p>${a(n("No root records are available.","\u6CA1\u6709\u53EF\u7528\u6839\u6C14\u8BB0\u5F55\u3002"))}</p>`);if(e.pageNumber===15){let E=[{x:85,y:90},{x:355,y:90},{x:85,y:310},{x:355,y:310}];return d(n("Pillar tension map","\u67F1\u4F4D\u5F20\u529B\u56FE"),`<div data-part="season-structure"><svg data-part="balance-orbit" viewBox="0 0 440 410" role="img" aria-label="${a(s.edges.map(o=>`${o.from} \u2013 ${o.to}: ${o.label}`).join("; ")||n("No tension relationships recorded","\u65E0\u5DF2\u8BB0\u5F55\u5F20\u529B\u5173\u7CFB"))}">${s.edges.map(o=>{let l=E[s.nodes.findIndex(T=>T.id===o.from)],m=E[s.nodes.findIndex(T=>T.id===o.to)];return`<path class="vrpt-link" stroke-dasharray="5 5" d="M${l.x} ${l.y}L${m.x} ${m.y}"/>`}).join("")}${s.nodes.map((o,l)=>`<g class="vrpt-node"><circle cx="${E[l].x}" cy="${E[l].y}" r="62"/><text data-total x="${E[l].x}" y="${E[l].y-10}" text-anchor="middle">${a(o.value)}</text><text data-caption x="${E[l].x}" y="${E[l].y+19}" text-anchor="middle">${a(o.label)}</text></g>`).join("")}</svg><div data-part="balance-key">${s.edges.map(o=>`<section class="vrpt-tile"><h4>${a(o.label)}</h4><p>${a(s.nodes.find(l=>l.id===o.from).label)} \u2194 ${a(s.nodes.find(l=>l.id===o.to).label)}</p></section>`).join("")||`<p>${a(n("No recorded tension relationships.","\u65E0\u5DF2\u8BB0\u5F55\u5F20\u529B\u5173\u7CFB\u3002"))}</p>`}</div></div>`)+d(n("Distinct signal counts","\u4E0D\u540C\u53E3\u5F84\u7684\u4FE1\u53F7\u8BA1\u6570"),t([s.signalNodes.find(o=>o.id==="PRESSURE"),{label:n("Tension relationships","\u5F20\u529B\u5173\u7CFB"),value:s.frictionCount,sourceRefs:e.evidenceRefs}]))}throw Error("BAZI_BATCH_2_PAGE_OUT_OF_SCOPE")}function b(e,{t:n,text:a,panel:d}){let s=e.visual,t=n("Day Master","\u65E5\u4E3B").split(" / "),r=s.nodes.map((E,o)=>({x:220+142*Math.cos(-Math.PI/2+o*2*Math.PI/s.nodes.length),y:215+142*Math.sin(-Math.PI/2+o*2*Math.PI/s.nodes.length)})),N=`<svg data-part="domain-orbit" viewBox="0 0 440 430" role="img" aria-label="${a(s.nodes.map(E=>`${E.label}: ${E.value}${E.lead?" \xB7 "+n("Lead","\u91CD\u70B9"):""}`).join("; "))}"><title>${a(s.domain)}</title><circle class="vrpt-orbit" cx="220" cy="215" r="142"/>${r.map(E=>`<path class="vrpt-link" d="M220 215L${E.x} ${E.y}"/>`).join("")}${s.nodes.map((E,o)=>`<g class="vrpt-node" data-selected="${E.lead}"><circle cx="${r[o].x}" cy="${r[o].y}" r="57"/><text x="${r[o].x}" y="${r[o].y-22}" text-anchor="middle">${a(E.label.split(" / ")[0])}</text>${E.label.includes(" / ")?`<text x="${r[o].x}" y="${r[o].y+8}" text-anchor="middle">${a(E.label.split(" / ")[1])}</text>`:""}<text data-total x="${r[o].x}" y="${r[o].y+38}" text-anchor="middle">${E.value}</text></g>`).join("")}<g class="vrpt-node" data-center><circle cx="220" cy="215" r="63"/><text data-total x="220" y="${t.length===2?201:214}" text-anchor="middle">${a(s.context.dayMaster.glyph)}</text>${t.map((E,o)=>`<text data-caption x="220" y="${t.length===2?231+o*24:240}" text-anchor="middle">${a(E)}</text>`).join("")}</g></svg>`,u=E=>`<span data-source-ref="${a(E.sourceRefs[0])}">${a(E.label)}<b>${E.value}</b></span>`;return`<div data-part="domain-main">${d(n("Domain structure","\u4E3B\u9898\u7ED3\u6784"),N,"domain-structure")}${d(n("Key themes","\u5173\u952E\u4E3B\u9898"),`<div data-part="domain-lead"><small>${a(n("Source-selected lead","\u6765\u6E90\u6307\u5B9A\u91CD\u70B9"))}</small><strong>${a(s.leadLabel)}</strong></div><div data-part="domain-themes">${s.themes.map(u).join("")}</div>`,"domain-themes-panel")}</div><div data-part="domain-conditions">${d(n("Support context","\u652F\u6301\u6761\u4EF6"),`<div data-part="metrics">${s.facts.slice(0,2).map(u).join("")}</div>`,"helps")}${d(n("Demand context","\u9700\u6C42\u6761\u4EF6"),`<div data-part="metrics">${s.facts.slice(2).map(u).join("")}</div>`,"costs")}</div>${d(n("Observe in context","\u60C5\u5883\u4E2D\u7684\u89C2\u5BDF"),`<div data-part="metrics"><span>${a(n("Day Master","\u65E5\u4E3B"))}<b>${a(s.context.dayMaster.glyph)}</b></span><span>${a(n("Month branch","\u6708\u652F"))}<b>${a(s.context.season.glyph)}</b></span>${u(s.themes[2])}</div>`,"observe")}`}function M(e,{t:n,text:a,panel:d}){let s=e.visual,t=s.nodes,r=(o,l)=>`<span>${a(o)}<b>${a(l)}</b></span>`,N=(o,l="guided-flow")=>`<div data-part="${l}">${o.map((m,T)=>`<section class="vrpt-tile" data-step="${T+1}" data-source-ref="${a(m.sourceRefs[0])}"><small>${String(T+1).padStart(2,"0")}</small><h3>${a(m.label)}</h3><p>${a(m.value)}</p></section>`).join("")}</div>`,u=()=>d(n("Natal baseline","\u672C\u547D\u57FA\u7EBF"),`<div data-part="metrics">${r(n("Day Master","\u65E5\u4E3B"),s.context.dayMaster.glyph)}${r(n("Month branch","\u6708\u652F"),s.context.season.glyph)}${r(n("Root records","\u6839\u6C14\u8BB0\u5F55"),s.context.roots.value)}</div>`),E=(o,l="route-map")=>`<svg data-part="${l}" viewBox="0 0 640 165" role="img" aria-label="${a(o.join(" \u2192 "))}"><path class="vrpt-link" d="M65 80H575"/>${o.map((m,T)=>{let p=65+T*510/(o.length-1);return`<g class="vrpt-node"><circle cx="${p}" cy="80" r="32"/><text x="${p}" y="86" text-anchor="middle">${T+1}</text></g>`}).join("")}</svg>`;return e.pageNumber===20?d(n("Time layers","\u65F6\u95F4\u5C42\u7EA7"),N([{label:n("Natal","\u672C\u547D"),value:n("Baseline","\u57FA\u7EBF"),sourceRefs:t[0].sourceRefs},{label:n("Luck pillars","\u5927\u8FD0"),value:n("Age bands","\u5E74\u9F84\u9636\u6BB5"),sourceRefs:t[0].sourceRefs},{label:n("Annual pillar","\u6D41\u5E74"),value:s.annualAvailable?n("Available","\u53EF\u7528"):n("Not supplied","\u672A\u63D0\u4F9B"),sourceRefs:s.sourceRefs}],"time-layers"))+d(n("Luck pillar sequence \xB7 ages","\u5927\u8FD0\u5E8F\u5217 \xB7 \u5E74\u9F84"),`<div data-part="period-bands">${s.bands.map(o=>`<section class="vrpt-tile" data-selected="${o.selected}"><small>${a(n("Cycle","\u9636\u6BB5"))} ${o.cycleNumber}</small><b data-glyph>${a(o.value)}</b><strong>${a(o.label)}</strong><small>${a(o.selected?n("Selected","\u5DF2\u9009\u62E9"):n("Age band","\u5E74\u9F84\u6BB5"))}</small></section>`).join("")}</div>`)+d(n("Target context","\u76EE\u6807\u60C5\u5883"),`<p>${a(s.selection)}</p>`):e.pageNumber===21?d(n("Explicit selection","\u660E\u786E\u9009\u62E9"),`<p>${a(s.selection)}</p><strong data-part="state-label">${a(s.completeness==="FULL"?n("Complete window","\u5B8C\u6574\u7A97\u53E3"):s.completeness==="NO_TARGET"?n("Target required","\u9700\u8981\u76EE\u6807\u65F6\u95F4"):n("Partial window","\u90E8\u5206\u7A97\u53E3"))}</strong>`)+d(n("Baseline \u2192 selected layers","\u57FA\u7EBF \u2192 \u6240\u9009\u5C42"),N(t,"selected-layers"))+u()+d(n("Layer boundary","\u65F6\u95F4\u5C42\u8FB9\u754C"),`<div data-part="metrics">${r(n("Natal","\u672C\u547D"),n("Retained","\u4FDD\u7559"))}${r(n("Timing","\u65F6\u95F4"),s.target?n("Explicit","\u660E\u786E\u6307\u5B9A"):n("Not selected","\u672A\u9009\u62E9"))}${r(n("Reality evidence","\u73B0\u5B9E\u8BC1\u636E"),n("Separate","\u72EC\u7ACB"))}</div>`):e.pageNumber===22?d(n("Comparison boundary","\u5BF9\u7167\u8FB9\u754C"),`<div data-part="reality-split">${N(t.slice(0,1),"method-side")}<div data-part="reality-inputs">${N(t.slice(1,3),"evidence-side")}</div></div>`)+d(n("Comparison state","\u5BF9\u7167\u72B6\u6001"),N(t.slice(3),"comparison-state"))+d(n("Independent records are needed","\u9700\u8981\u72EC\u7ACB\u8BB0\u5F55"),E([t[0].label,t[1].label,t[3].label])+`<div data-part="metrics">${r(n("Method","\u65B9\u6CD5"),n("Available","\u53EF\u7528"))}${r(n("Evidence","\u8BC1\u636E"),n("Required","\u5F85\u63D0\u4F9B"))}${r(n("Comparison","\u6BD4\u8F83"),n("Open","\u5F00\u653E"))}</div>`):e.pageNumber===23||e.pageNumber===24?d(e.pageNumber===23?n("Observe through contrasting situations","\u5728\u5BF9\u7167\u60C5\u5883\u4E2D\u89C2\u5BDF"):n("Explore conditions before conclusions","\u5148\u63A2\u7D22\u6761\u4EF6\uFF0C\u518D\u5F62\u6210\u7ED3\u8BBA"),E(t.map(o=>o.label))+N(t))+u()+d(n("Observation boundary","\u89C2\u5BDF\u8FB9\u754C"),`<div data-part="metrics">${r(n("Prompt","\u63D0\u793A"),n("Question","\u95EE\u9898"))}${r(n("Record","\u8BB0\u5F55"),n("Not supplied","\u672A\u63D0\u4F9B"))}${r(n("Conclusion","\u7ED3\u8BBA"),n("Open","\u5F00\u653E"))}</div>`):e.pageNumber===25?d(n("A path from noticing to review","\u4ECE\u89C9\u5BDF\u8D70\u5411\u56DE\u987E"),E(t.map(o=>o.label))+N(t,"navigation-route"))+d(n("Keep the comparison open","\u4FDD\u6301\u5BF9\u7167\u5F00\u653E"),`<div data-part="metrics">${r(n("Notice","\u89C9\u5BDF"),n("Context","\u60C5\u5883"))}${r(n("Compare","\u6BD4\u8F83"),n("Contrast","\u53CD\u4F8B"))}${r(n("Review","\u56DE\u987E"),n("Changes","\u53D8\u5316"))}</div>`)+u():d(n("Evidence & lineage","\u8BC1\u636E\u4E0E\u6765\u6E90\u8FFD\u6EAF"),`<div data-part="lineage-flow">${s.lineage.map((o,l)=>`<section class="vrpt-tile" data-step="${l+1}"><small>${String(l+1).padStart(2,"0")}</small><h3>${a(o.label)}</h3><code title="${a(o.value)}">${a(o.value.length>30?o.value.slice(0,22)+"\u2026":o.value)}</code></section>`).join("")}</div>`)+d(n("Method-side source records","\u65B9\u6CD5\u4FA7\u6765\u6E90\u8BB0\u5F55"),`<div data-part="trace-metrics">${s.trace.map(o=>r(o.label,o.value)).join("")}</div>`)+d(n("Current Reality stays independent","\u5F53\u524D\u73B0\u5B9E\u4FDD\u6301\u72EC\u7ACB"),`<div data-part="metrics">${r(n("Method","\u65B9\u6CD5"),n("Traceable","\u53EF\u8FFD\u6EAF"))}${r(n("Reality","\u73B0\u5B9E"),n("Not supplied","\u672A\u63D0\u4F9B"))}${r(n("Verdict","\u5224\u65AD"),n("Not promoted","\u4E0D\u63D0\u5347"))}</div>`)}function P(e){let n={"BAZI-DYNAMIC-R1-BATCH-01":[6,5],"BAZI-DYNAMIC-R1-BATCH-02":[11,5],"BAZI-DYNAMIC-R1-BATCH-03":[16,4],"BAZI-DYNAMIC-R1-BATCH-04":[20,3],"BAZI-DYNAMIC-R1-BATCH-05":[23,4]}[e.visualBatch];if(!n)throw Error("BAZI_BATCH_REVIEW_SCOPE_REQUIRED");let[a,d]=n;if(e.reviewMode!==!0||e.customerPublishable!==!1||e.depth!=="PAID"||e.pages.length!==d||e.pages.some((c,i)=>c.pageNumber!==i+a))throw Error("BAZI_BATCH_REVIEW_SCOPE_REQUIRED");let s=e.locale,t=(c,i)=>s==="bilingual"?`${i} / ${c}`:s==="zh-Hans"?i:c,r=c=>O(c),N=(c,i,I="")=>`<section class="vrpt-tile" ${I?`data-part="${I}"`:""}><h3>${r(c)}</h3>${i}</section>`,u=c=>`<div data-part="context">${N(t("Day Master","\u65E5\u4E3B"),`<b data-glyph data-element="${c.context.dayMaster.element}">${r(c.context.dayMaster.glyph)}</b><p>${r(c.context.dayMaster.code)} \xB7 ${r(c.context.dayMaster.polarity)} ${r(c.context.dayMaster.label)}</p>`)}${N(t("Season","\u5B63\u8282"),`<b data-glyph data-element="${c.context.season.element}">${r(c.context.season.glyph)}</b><p>${r(c.context.season.name)} \xB7 ${r(c.context.season.label)}</p>`)}</div>`,E=(c,i=!1)=>`<div data-part="pillars">${c.pillars.map(I=>`<article class="vrpt-tile" data-selected="${I.id==="DAY"}"><h4>${r(I.label)}</h4><small>${r(t("Heavenly stem","\u5929\u5E72"))}</small><b data-glyph data-element="${I.stemElement}">${r(I.stem)}</b><span>${r(I.stemCode)} \xB7 ${r(I.polarity)}</span><strong data-element="${I.stemElement}">${r(I.stemLabel)}</strong><hr><small>${r(t("Earthly branch","\u5730\u652F"))}</small><b data-glyph data-element="${I.branchElement}">${r(I.branch)}</b><span>${r(I.branchCode)}</span><strong data-element="${I.branchElement}">${r(I.branchLabel)}</strong>${i&&I.hiddenStems?`<hr><small>${r(t("Hidden stems","\u85CF\u5E72"))}</small><p>${r(I.hiddenStems)}</p>`:""}</article>`).join("")}</div>`,o=c=>{let i=c.elements.reduce((S,A)=>S+A.value,0),I=2*Math.PI*88,R=0;return`<svg data-part="donut" viewBox="0 0 260 260" role="img" aria-label="${r(c.elements.map(S=>`${S.label}: ${S.value}`).join("; "))}"><title>${r(c.unit)}</title>${c.elements.map(S=>{let A=R;return R+=S.value/i*I,`<circle data-element="${S.id}" cx="130" cy="130" r="88" fill="none" stroke="currentColor" stroke-width="32" stroke-dasharray="${S.value/i*I} ${I}" stroke-dashoffset="${-A}" transform="rotate(-90 130 130)"/>`}).join("")}<text x="130" y="125" text-anchor="middle" data-total>${i}</text><text x="130" y="149" text-anchor="middle">${r(t("COUNT","\u8BA1\u6570"))}</text></svg>`},l=c=>`<div data-part="bars">${[...c.elements].sort((i,I)=>I.value-i.value).map(i=>`<div data-element="${i.id}"><span>${r(i.label)}</span><svg viewBox="0 0 160 12" role="img" aria-label="${r(`${i.label}: ${i.value} / ${c.context.total}`)}"><rect data-track width="160" height="12" rx="6"/><rect width="${160*i.value/c.context.total}" height="12" rx="6" fill="currentColor"/></svg><strong>${i.value}</strong></div>`).join("")}<small>${r(t("Count / total inventory","\u8BA1\u6570\uFF0F\u6E05\u5355\u603B\u6570"))}: ${c.context.total}</small></div>`,m=c=>`<div data-part="distribution">${o(c)}${l(c)}</div>`,T=c=>{let i=c.context;return`<div data-part="season-structure"><svg viewBox="0 0 440 400" role="img" aria-label="${r(`${t("Day Master","\u65E5\u4E3B")}: ${i.dayMaster.code}; ${t("Season","\u5B63\u8282")}: ${i.season.name}; ${t("Root records","\u6839\u6C14\u8BB0\u5F55")}: ${i.roots.value}`)}"><circle class="vrpt-orbit" cx="220" cy="200" r="155"/><path class="vrpt-link" d="M220 85V315M75 200H365"/>${[[220,58,i.season.glyph,i.season.name],[75,200,String(i.roots.value),t("Roots","\u6839\u6C14")],[365,200,"\u2014",t("Open","\u5F00\u653E")],[220,342,i.season.label,t("Month element","\u6708\u4EE4\u4E94\u884C")]].map(([I,R,S,A])=>`<g class="vrpt-node"><circle cx="${I}" cy="${R}" r="55"/><text x="${I}" y="${R-4}" text-anchor="middle">${r(S)}</text><text data-caption x="${I}" y="${R+20}" text-anchor="middle">${r(A)}</text></g>`).join("")}<g class="vrpt-node" data-center><circle cx="220" cy="200" r="73"/><text data-total x="220" y="196" text-anchor="middle">${r(i.dayMaster.glyph)}</text><text x="220" y="226" text-anchor="middle">${r(i.dayMaster.label)}</text></g></svg>${u(c)}</div>`},p=c=>{let i=c.nodes.map((I,R)=>({id:I.id,x:220+145*Math.cos(-Math.PI/2+R*2*Math.PI/5),y:205+145*Math.sin(-Math.PI/2+R*2*Math.PI/5)}));return`<svg data-part="relationships" viewBox="0 0 440 410" role="img" aria-label="${r(t("Solid arrows generate; dashed arrows control. Counts label the inventory only.","\u5B9E\u7EBF\u7BAD\u5934\u76F8\u751F\uFF0C\u865A\u7EBF\u7BAD\u5934\u76F8\u514B\u3002\u6570\u5B57\u4EC5\u8868\u793A\u6E05\u5355\u8BA1\u6570\u3002"))}"><defs><marker id="arrow-${s}" markerWidth="7" markerHeight="7" refX="6" refY="3" orient="auto"><path d="M0 0L6 3L0 6" fill="currentColor"/></marker></defs><circle class="vrpt-orbit" cx="220" cy="205" r="145"/>${c.edges.map(I=>{let R=i.find(g=>g.id===I.from),S=i.find(g=>g.id===I.to),A=S.x-R.x,C=S.y-R.y,h=Math.hypot(A,C),Y=R.x+A/h*43,$=R.y+C/h*43,G=S.x-A/h*48,B=S.y-C/h*48;return`<path class="vrpt-link" ${I.relation==="SUBJECT_CONTROLS_TARGET"?'stroke-dasharray="5 5"':""} marker-end="url(#arrow-${s})" d="M${Y} ${$}L${G} ${B}"/>`}).join("")}${c.nodes.map((I,R)=>`<g class="vrpt-node" data-element="${I.id}"><circle cx="${i[R].x}" cy="${i[R].y}" r="43"/><text x="${i[R].x}" y="${i[R].y-2}" text-anchor="middle">${r(I.label)}</text><text x="${i[R].x}" y="${i[R].y+22}" text-anchor="middle">${I.value}</text></g>`).join("")}</svg>`},v=c=>{let i=c.visual;return c.pageNumber>=20?M(c,{t,text:r,panel:N}):c.pageNumber>=16?b(c,{t,text:r,panel:N}):c.pageNumber>=11?H(c,{t,text:r,panel:N}):c.pageNumber===6?N(t("Four pillars","\u56DB\u67F1\u547D\u76D8"),`<div data-part="snapshot">${E(i)}${u(i)}</div>`)+N(t("Five elements \xB7 unweighted inventory","\u4E94\u884C \xB7 \u672A\u52A0\u6743\u6E05\u5355"),m(i)):c.pageNumber===7?N(t("Day Master in context","\u65E5\u4E3B\u7684\u7ED3\u6784\u4F4D\u7F6E"),T(i))+N(t("Seasonal reference","\u5B63\u8282\u5B9A\u4F4D"),`<div data-part="season-line">${[["SPRING","Spring","\u6625"],["SUMMER","Summer","\u590F"],["AUTUMN","Autumn","\u79CB"],["WINTER","Winter","\u51AC"]].map(([I,R,S])=>`<span data-selected="${I===i.context.season.name.toUpperCase()||i.context.season.name.includes(S)}">${r(t(R,S))}</span>`).join("")}</div><div data-part="metrics"><span>${r(t("Month branch","\u6708\u652F"))}<b>${r(i.context.season.glyph)}</b></span><span>${r(t("Element","\u4E94\u884C"))}<b>${r(i.context.season.label)}</b></span><span>${r(t("Root records","\u6839\u6C14\u8BB0\u5F55"))}<b>${i.context.roots.value}</b></span></div>`):c.pageNumber===8?N(t("Stems \xB7 branches \xB7 positions","\u5929\u5E72 \xB7 \u5730\u652F \xB7 \u67F1\u4F4D"),E(i,!0))+N(t("The day stem is the reference point","\u4EE5\u65E5\u5E72\u4E3A\u53C2\u7167"),`<div data-part="metrics"><span>${r(t("Day Master","\u65E5\u4E3B"))}<b>${r(i.context.dayMaster.glyph)} ${r(i.context.dayMaster.label)}</b></span><span>${r(t("Month command","\u6708\u4EE4"))}<b>${r(i.context.season.glyph)} ${r(i.context.season.label)}</b></span><span>${r(t("Pillars","\u56DB\u67F1"))}<b>${i.pillars.length}</b></span></div>`):c.pageNumber===9?N(t("Element inventory","\u4E94\u884C\u6E05\u5355"),m(i))+N(t("Count composition","\u8BA1\u6570\u6784\u6210"),`<table data-part="inventory"><thead><tr><th>${r(t("Element","\u4E94\u884C"))}</th><th>${r(t("Stems","\u5929\u5E72"))}</th><th>${r(t("Branches","\u5730\u652F"))}</th><th>${r(t("Hidden","\u85CF\u5E72"))}</th><th>${r(t("Total","\u5408\u8BA1"))}</th></tr></thead><tbody>${i.elements.map(I=>`<tr><th data-element="${I.id}">${r(I.label)}</th><td>${I.breakdown.visibleStems}</td><td>${I.breakdown.visibleBranches}</td><td>${I.breakdown.hiddenStemsUnweighted}</td><td>${I.value}</td></tr>`).join("")}</tbody></table>`):N(t("Generating & controlling cycles","\u76F8\u751F\u4E0E\u76F8\u514B\u5FAA\u73AF"),`<div data-part="relationship-layout">${p(i)}<div data-part="relation-keys">${["SUBJECT_GENERATES_TARGET","SUBJECT_CONTROLS_TARGET"].map((I,R)=>`<section><h4>${r(R?t("Dashed \u2192 controls","\u865A\u7EBF \u2192 \u76F8\u514B"):t("Solid \u2192 generates","\u5B9E\u7EBF \u2192 \u76F8\u751F"))}</h4>${i.edges.filter(S=>S.relation===I).map(S=>`<p>${r(i.nodes.find(A=>A.id===S.from).label)} \u2192 ${r(i.nodes.find(A=>A.id===S.to).label)}</p>`).join("")}</section>`).join("")}</div></div>`)+N(t("Inventory stays distinct from relationship intensity","\u6E05\u5355\u8BA1\u6570\u4E0D\u7B49\u4E8E\u5173\u7CFB\u5F3A\u5EA6"),l(i))};return`<article class="vrpt-report" data-visual-batch="${e.visualBatch}" data-report-locale="${s}" lang="${s==="bilingual"?"zh-Hans":s}">${e.pages.map(c=>`<section class="vrpt-page" id="${c.pageId}" data-page-number="${c.pageNumber}" data-master="${c.visualTemplateId}"><header data-part="chrome"><div><strong>P H I O S</strong><small>HUMAN POTENTIAL INTELLIGENCE</small></div><span>${r(t("BAZI \xB7 FULL REPORT","\u516B\u5B57 \xB7 \u5B8C\u6574\u62A5\u544A"))}<br>${String(c.pageNumber).padStart(2,"0")}</span></header><div class="vrpt-heading"><span data-part="page-index">${String(c.pageNumber).padStart(2,"0")}<small>/ 26</small></span><div><small>${r(t("STRUCTURAL","\u7ED3\u6784\u5C42"))}</small><h2>${c.title.split(" / ").map(i=>`<span>${r(i)}</span>`).join("")}</h2><p>${r(c.question)}</p></div></div><div data-part="page-body"><figure class="vrpt-primary">${v(c)}<figcaption>${r(c.visual.unit)}</figcaption></figure><div class="vrpt-insights">${c.insights.map((i,I)=>`<article class="vrpt-tile" data-card-type="${["PRIMARY_INSIGHT","CONDITION","OPEN_QUESTION"][I]}" data-source-ref="${r(i.sourceRef)}"><small>${r([t("Structure","\u7ED3\u6784"),t("Condition","\u6761\u4EF6"),t("Observe","\u89C2\u5BDF")][I])}</small><p>${r(i.text)}</p></article>`).join("")}</div></div><aside class="vrpt-boundary">${r(c.boundaryText)}</aside><details class="vrpt-evidence"><summary>${r(t("Data & source lineage","\u6570\u636E\u4E0E\u6765\u6E90\u8FFD\u6EAF"))}</summary><ul>${c.evidenceRefs.map(i=>`<li>${r(i)}</li>`).join("")}</ul></details><footer><span>${r(t("Your life, in context.","\u5728\u60C5\u5883\u4E2D\u7406\u89E3\u4F60\u7684\u4EBA\u751F\u3002"))}</span><span>${String(c.pageNumber).padStart(2,"0")} / 26</span></footer></section>`).join("")}</article>`}var f=[{methodId:"BZR",sourceConversation:"6aaa576d-03bc-83ec-a176-5f8885313c6f",pages:[{pageNumber:1,title:{"zh-Hans":"\u516B\u5B57\u5B8C\u6574\u62A5\u544A",en:"BAZI FULL REPORT"},sourceText:`## 01\uFF5C\u516B\u5B57\u5B8C\u6574\u62A5\u544A
### BAZI FULL REPORT
...
---`,sourcePath:"docs/guided-report-successor-r1/reference-cross.md",role:"PARTICIPATION_BOUNDARY_CLOSING",master:"M08"}],humanReview:"PENDING",sourceRole:"USER_DESIGN_REFERENCE_NOT_NEW_METHOD_MEANING"}];var U="GUIDED_REPORT_SUCCESSOR_R2",rn=Object.freeze({reportSchemaVersion:U,visualSystemVersion:"2.0.0",composerPolicyVersion:"2.0.0",temporalPolicyVersion:"2.0.0",pageRegistryVersion:"2.0.0"}),V=e=>e.pageNumber<=6?"INTRO_EXISTING":e.master==="M07"?"TIMING":/CLOS|BOUNDARY/.test(e.role)?"CLOSURE":/NAVIGATION/.test(e.role)?"NAVIGATION":e.master==="M06"||e.master==="M08"?"DOMAIN_READING":"STRUCTURAL",tn=Object.freeze(f.map(e=>Object.freeze({method:e.methodId,totalPages:e.pages.length,pages:e.pages.map(n=>Object.freeze({method:e.methodId,pageKey:n.role,sequence:n.pageNumber,section:n.master,contentType:V(n),executionClass:n.pageNumber<=6?"T1_CANONICAL_ASSEMBLY":n.pageNumber<16?"T2_LIGHT_COMPOSITION":"T3_DEEP_COMPOSITION",requiredEvidence:["ADMITTED_METHOD_READING","SOURCE_BOUND_PAGE"],visualVariant:n.pageNumber===7?"P07_ENTRY_VARIANT":[16,20,25].includes(n.pageNumber)?"SECTION":"BODY",localeSupport:["zh-Hans","en"],pagination:n.pageNumber===1?"NONE":"GLOBAL",customerVisible:!0,fallbackPolicy:"CANONICAL_HUMANIZATION",title:n.title}))}))),L=Object.freeze(Object.fromEntries([["BZR","BaZi","\u516B\u5B57","LANDSCAPE_RINGS"],["ZWR","Zi Wei","\u7D2B\u5FAE\u6597\u6570","PALACE_GRID"],["AST","Astrology","\u5360\u661F","ORBITS"],["PROFILE","Profile","\u81EA\u6211\u753B\u50CF","LAYERS"],["NUM","Numerology","\u6570\u5B57\u5B66","SEQUENCE"],["ECR","ECR","\u5177\u8EAB\u73B0\u5B9E","FIELDS"],["HD","Human Design","\u4EBA\u7C7B\u56FE","CHANNELS"],["CROSS","Cross","\u7EFC\u5408\u8BFB\u53D6","INTERSECTIONS"]].map(([e,n,a,d])=>[e,Object.freeze({method:e,label:{en:n,"zh-Hans":a},bodyBackground:null,sectionBackground:null,motif:d,localeIndependent:!0,backgroundSafeArea:"CENTRAL_80_PERCENT",status:"VISUAL_ASSET_ENHANCEMENT_PENDING",fallback:"CSS_CANONICAL_PREMIUM",version:"2.0.0",accentTokens:["--phi-report-gold","--phi-report-navy"],diagramTokens:["--phi-report-gold-line"]})])));function x(e,n){if(!Number.isInteger(e)||!Number.isInteger(n)||e<1||e>n)throw Error("PUBLICATION_PAGINATION_INVALID");return e===1?"":`${String(e).padStart(2,"0")} / ${n}`}function y(e){if(e?.schemaVersion!==U||e.customerPublishable!==!1||e.successorBaselineActivated!==!1)throw Error("PUBLICATION_CANDIDATE_GATE_REQUIRED");if(!L[e.methodId]||!["zh-Hans","en"].includes(e.locale))throw Error("PUBLICATION_METHOD_OR_LOCALE_INVALID");return e}function D(e,n){let a=x(e,n);return a?`<span data-pagination-owner="GlobalReportPagination" aria-label="${O(a)}">${a}</span>`:""}function En(e){return[...e.querySelectorAll(".pub-page")].map(n=>{let a=!1;for(let d of["STANDARD","COMPACT","REFLOW"]){n.dataset.textFit=d;let s=n.getBoundingClientRect().bottom,t=n.querySelector("footer").getBoundingClientRect();if(a=n.scrollHeight<=n.clientHeight+2&&t.bottom<=s,a)break}return{pageNumber:Number(n.dataset.pageNumber),variant:n.dataset.textFit,fits:a}})}function dn(e,n=26){let a=P(e),d=a.indexOf('<section class="vrpt-page"'),s=a.indexOf('<section class="vrpt-page"',d+1),t=a.slice(d,s);if(!t||!t.includes('data-page-number="6"')||!t.includes("</footer>"))throw Error("FROZEN_BAZI_INTRO_SHAPE_CHANGED");let r=t.replace(/<details class="vrpt-evidence">[\s\S]*?<\/details>/g,"").replace(/ data-source-ref="[^"]*"/g,"").replace(/id="[^"]*"/g,"").replace(/<span data-part="page-index">[\s\S]*?<\/span>/,"").replace(/<br>06<\/span>/,"</span>").replace(/<span>06 \/ 26<\/span>/,D(6,n));return`<article class="vrpt-report" data-visual-batch="${e.visualBatch}" data-frozen-intro="P06" lang="${e.locale}">${r}</article>`}function ln(e,n){let a=P(e),d=a.indexOf(`data-page-number="${n}"`),s=a.slice(d),t=s.match(/<figure class="vrpt-primary">([\s\S]*?)<\/figure>/)?.[1];if(d<0||!t)throw Error("PUBLICATION_SOURCE_DIAGRAM_MISSING");return t.replace(/ data-source-ref="[^"]*"/g,"").replace(/<figcaption>[\s\S]*?<\/figcaption>/g,"")}function _(e){let n={LANDSCAPE_RINGS:'<path d="M0 270L70 190 130 240 210 120 300 245 390 155 480 270 600 180 720 280V340H0Z"/><path d="M0 300L130 210 230 280 350 195 460 270 580 220 720 310V340H0Z"/>',PALACE_GRID:'<path d="M80 60H640V300H80ZM220 60V300M360 60V300M500 60V300M80 140H640M80 220H640"/>',ORBITS:'<ellipse cx="360" cy="170" rx="260" ry="90"/><ellipse cx="360" cy="170" rx="180" ry="130"/>',CHANNELS:'<path d="M200 80L520 80 360 280ZM200 80L360 170 520 80M360 170V280"/>',INTERSECTIONS:'<circle cx="280" cy="170" r="120"/><circle cx="440" cy="170" r="120"/>',LAYERS:'<path d="M90 250Q360 20 630 250M90 210Q360 -20 630 210M90 290Q360 60 630 290"/>',SEQUENCE:'<path d="M100 250L220 90 360 250 500 90 620 250"/>',FIELDS:'<ellipse cx="360" cy="170" rx="240" ry="130"/><ellipse cx="360" cy="170" rx="180" ry="95"/><ellipse cx="360" cy="170" rx="100" ry="50"/>'};return`<svg class="pub-motif" viewBox="0 0 720 340" aria-hidden="true" focusable="false">${n[e]||n.LAYERS}</svg>`}function un(e){y(e);let{locale:n,totalPages:a,methodId:d}=e,s=L[d],t=(u,E)=>n==="en"?u:E,r=e.intro.map(u=>u.kind==="STATIC"?`<section class="pub-static" data-page-number="${u.pageNumber}" data-pagination-exception="APPROVED_BAKED_ASSET"><img src="${O(u.src)}" alt="${O(u.alt)}"></section>`:u.html).join(""),N=e.pages.map(u=>{if(u.pageFamily)return F(u,{locale:n,totalPages:a,skin:s,t});let E=u.facts.filter(l=>l.value!==void 0),o=u.primaryVisualHtml||(u.temporal?`<div class="pub-time"><span>${t("Observation date","\u89C2\u5BDF\u65E5\u671F")}<b>${O(u.temporal.date)}</b></span>${u.temporal.selectedLuck?`<span>${t("Luck cycle","\u5927\u8FD0")}<b>${O(u.temporal.selectedLuck)}</b></span>`:""}${u.temporal.annual?`<span>${t("Year pillar","\u6D41\u5E74")}<b>${O(u.temporal.annual)}</b></span>`:""}</div>`:E.length?`<div class="pub-facts" role="list">${E.slice(0,5).map(l=>`<div role="listitem"><b>${O(l.value)}</b><span>${O(l.label)}</span></div>`).join("")}</div>`:u.pageNumber===26?'<div class="pub-closing-mark" aria-hidden="true">\u25C7</div>':"");return`<section class="pub-page" data-page-number="${u.pageNumber}" data-page-key="${O(u.pageKey)}" data-variant="${u.visualVariant}" data-method-skin="${d}" data-text-fit="STANDARD">
  ${_(s.motif)}<header class="pub-header"><span>PHI OS<small>${t("SEE DEEPER \xB7 LIVE CLEARER","\u770B\u89C1\u66F4\u6DF1 \xB7 \u6D3B\u51FA\u66F4\u6E05\u6670")}</small></span><span>${O(s.label[n])}<small>${t("PERSONAL READING","\u4E2A\u4EBA\u8BFB\u53D6")}</small></span></header>
  <div class="pub-heading"><p class="pub-section">${O(t({M04:"Structure & context",M05:"Balance & relationships",M06:"Your life in context",M07:"Time & experience",M08:"Observation & direction"}[u.section]||"Personal reading",{M04:"\u7ED3\u6784\u4E0E\u60C5\u5883",M05:"\u5E73\u8861\u4E0E\u8054\u7CFB",M06:"\u4EBA\u751F\u60C5\u5883",M07:"\u65F6\u95F4\u4E0E\u7ECF\u9A8C",M08:"\u89C2\u5BDF\u4E0E\u65B9\u5411"}[u.section]||"\u4E2A\u4EBA\u8BFB\u53D6"))}</p><h2>${O(u.title)}</h2>${u.lead?`<p class="pub-lead">${O(u.lead)}</p>`:""}</div>
  ${o?`<figure class="pub-visual" aria-label="${O(u.title)}">${o}</figure>`:""}
  <div class="pub-narrative">${u.paragraphs.map(l=>`<p>${O(l)}</p>`).join("")}</div>
  ${u.provenanceHighlights?.length?`<ul class="pub-provenance">${u.provenanceHighlights.map(l=>`<li>${O(l)}</li>`).join("")}</ul>`:""}
  ${u.observation||u.counterSignal?`<aside class="pub-reflection"><h3>${t("Bring it into experience","\u628A\u8BFB\u53D6\u5E26\u56DE\u7ECF\u9A8C")}</h3>${u.observation?`<p>${O(u.observation)}</p>`:""}${u.counterSignal?`<p>${O(u.counterSignal)}</p>`:""}</aside>`:""}
  <p class="pub-boundary">${O(u.boundary)}</p><footer class="pub-footer"><span>${t("Your life, in context.","\u5728\u60C5\u5883\u4E2D\u7406\u89E3\u4F60\u7684\u4EBA\u751F\u3002")}</span>${D(u.pageNumber,a)}</footer></section>`}).join("");return`<article class="pub-report" lang="${n}" data-publication-version="2.0.0" data-method="${d}">${r}${N}</article>`}function z(e){let n=e.visualBinding||{},a=[[0,n.bodyUrl],[1,n.motifUrl],[2,e.pageFamily==="SECTION_OPENER_PAGE"?n.url:null]].filter(([,s])=>s),d=n.intensityByFamily?.[e.pageFamily]??.17;return`<div class="pub-decoration" aria-hidden="true">${a.map(([s,t])=>`<img data-decorative-layer="${s}" style="opacity:${s===1?.16:s===0&&e.pageFamily==="SECTION_OPENER_PAGE"?.22:d}" ${s===2?`data-fallback-sources="${O(JSON.stringify(n.candidates||[]))}"`:""} src="${O(t)}" alt="">`).join("")}</div>`}function W(e){return`<svg class="pub-section-landscape" viewBox="0 0 800 600" aria-hidden="true"><circle cx="${600-Number(e)%3*45}" cy="240" r="46" fill="#cfb77e" opacity=".25"/><path d="M0 480 Q100 470 175 360 L260 425 350 310 445 420 530 355 655 450 800 330V600H0Z" fill="#bcc5c0" opacity=".48"/><path d="M0 555L110 445 200 485 330 390 430 505 550 460 640 410 800 520V600H0Z" fill="#8e9b93" opacity=".35"/><path d="M0 600Q120 500 260 555T550 530T800 575" fill="none" stroke="#b7975f" stroke-width="2"/><path d="M100 540Q280 505 410 548T760 555" fill="none" stroke="#b7975f" opacity=".65"/></svg>`}function F(e,{locale:n,totalPages:a,skin:d,t:s}){let t=e.pageFamily==="SECTION_OPENER_PAGE",r=e.heroPlacement||["hero-bottom","hero-right","hero-left","hero-full-fade"][(Number(e.sectionNumber)-1)%4],N=`<header class="pub-header"><span>PHI OS<small>${s("SEE DEEPER \xB7 LIVE CLEARER","\u770B\u89C1\u66F4\u6DF1 \xB7 \u6D3B\u51FA\u66F4\u6E05\u6670")}</small></span><span>${O(d.label[n])}<small>${O(e.sectionTitle[n])}</small></span></header>`,u=t?`<div class="pub-opener-heading"><span class="pub-section-number">${O(e.sectionNumber)}</span><h2 lang="zh-Hans">${O(e.sectionTitle["zh-Hans"])}</h2><p lang="en">${O(e.sectionTitle.en)}</p></div>`:`<div class="pub-heading"><p class="pub-section">${O(e.sectionNumber)} \xB7 ${O(e.sectionTitle[n])}</p><h2>${O(e.title)}</h2></div>`,E=e.temporal?`<dl class="pub-time"><div><dt>${s("Observation time","\u89C2\u5BDF\u65F6\u95F4")}</dt><dd>${O(e.temporal.date)}<small>${O(e.temporal.localTime||"")} \xB7 ${O(e.temporal.timezone)}</small></dd></div><div><dt>${s("Luck cycle","\u5927\u8FD0")}</dt><dd>${O(e.temporal.selectedLuck||s("Outside resolved range","\u8D85\u51FA\u5DF2\u89E3\u6790\u8303\u56F4"))}</dd></div><div><dt>${s("Year pillar","\u6D41\u5E74")}</dt><dd>${O(e.temporal.annual||s("Not admitted","\u672A\u83B7\u51C6"))}</dd></div></dl>`:"",o=e.items?.length?`<ol class="pub-insights">${e.items.map((T,p)=>`<li><span aria-hidden="true">${String(p+1).padStart(2,"0")}</span><p>${O(T)}</p></li>`).join("")}</ol>`:"",l=e.facts?.length?`<div class="pub-facts">${e.facts.map(T=>`<div><b>${O(T.value)}</b><span>${O(T.label)}</span></div>`).join("")}</div>`:"",m=e.observations?.length?`<aside class="pub-reflection"><h3>${e.sectionKey==="S08_TIMING"?s("Questions for this period","\u8FD9\u4E00\u9636\u6BB5\u7684\u89C2\u5BDF\u95EE\u9898"):s("Questions to consider","\u89C2\u5BDF\u95EE\u9898")}</h3>${e.observations.map(T=>`<p>${O(T)}</p>`).join("")}</aside>`:"";return`<section class="pub-page pub-family" data-page-number="${e.pageNumber}" data-page-key="${O(e.pageKey)}" data-page-family="${O(e.pageFamily)}" data-section="${O(e.sectionKey)}" data-hero-placement="${r}" data-body-variant="BODY_${["A","B","C"][(e.pageNumber-7)%3]}" data-text-fit="STANDARD">${z(e)}${t?W(e.sectionNumber):_(d.motif)}${N}${u}${e.primaryVisualHtml?`<figure class="pub-visual">${e.primaryVisualHtml}</figure>`:""}${E}${l}<div class="pub-narrative">${e.paragraphs.map(T=>`<p>${O(T)}</p>`).join("")}</div>${o}${m}${e.boundary?`<p class="pub-boundary">${O(e.boundary)}</p>`:""}<footer class="pub-footer"><span>${s("Your life, in context.","\u5728\u60C5\u5883\u4E2D\u7406\u89E3\u4F60\u7684\u4EBA\u751F\u3002")}</span>${D(e.pageNumber,a)}</footer></section>`}async function In(e){await Promise.all([...e.querySelectorAll(".pub-decoration img")].map(async n=>{let a=n.dataset.fallbackSources?JSON.parse(n.dataset.fallbackSources):[n.getAttribute("src")];for(let d of a)try{n.src=d,await n.decode();return}catch{}n.remove()}))}export{ln as extractPublicationDiagram,En as fitPublicationForPrint,dn as renderFrozenBaziIntro,D as renderGlobalReportPagination,un as renderPublicationReport,In as settlePublicationAssets};

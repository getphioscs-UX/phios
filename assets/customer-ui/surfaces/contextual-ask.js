...
const ASK_STATES=new Set(['IDLE','COMPOSING','RETRIEVING','SUFFICIENT','PARTIAL','INSUFFICIENT','ERROR','OFFLINE']);
function setAskState(state){
  const next=ASK_STATES.has(state)?state:'IDLE';
  document.body.dataset.cxAskState=next;
  const form=document.querySelector('[data-cx-contextual-ask-form]');
  if(form)form.setAttribute('aria-busy',next==='RETRIEVING'?'true':'false');
  return next;
}
function stateFromView(result){
  const quality=String(result?.qualityOutcome||'').toUpperCase();
  if(quality==='SUFFICIENT')return 'SUFFICIENT';
  if(quality==='PARTIAL')return 'PARTIAL';
  if(['AMBIGUOUS','CONTRADICTORY','INSUFFICIENT'].includes(quality))return 'INSUFFICIENT';
  const answerState=String(result?.state||'').toUpperCase();
  if(answerState==='ANSWERED')return 'SUFFICIENT';
  if(answerState==='PARTIALLY_ANSWERED')return 'PARTIAL';
  return 'INSUFFICIENT';
}
...
function boot(){
  ...
  const syncTypingState=()=>{ if(!navigator.onLine)setAskState('OFFLINE'); else setAskState(String(questionInput?.value||'').trim()?'COMPOSING':'IDLE'); };
  questionInput?.addEventListener('input',syncTypingState);
  window.addEventListener('offline',()=>setAskState('OFFLINE'));
  window.addEventListener('online',syncTypingState);
  setAskState(navigator.onLine?'IDLE':'OFFLINE');
  ...
  form.addEventListener('submit',async event=>{
    ...
    if(!navigator.onLine){setAskState('OFFLINE');setStatus(status,tr('You are offline. Reconnect before sending this question.','目前处于离线状态。重新连接后再发送这个问题。'),'error');return;}
    setAskState('RETRIEVING');

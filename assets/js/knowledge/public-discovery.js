...
async function loadBook6AtlasDiscovery(locale='en'){
 ...
 const records=[];
 for(const x of data.cases?.cases||[])records.push({locale:l,type:'CASE',slug:x.id,title:title(x.titleZh,x.titleEn),summary:[x.timeLabel,(x.regions||[]).join(' · '),(x.caseTypes||[]).join(' · ')].filter(Boolean).join(' · '),href:route('cases','case',x.id),bookTitle,partTitle,themeCode:'CIVILIZATION_RECONFIGURATION',tags:[...(x.caseTypes||[]),...(x.regions||[])],searchText:[x.id,x.titleZh,x.titleEn,x.timeLabel,x.priorRuntime,x.trigger,x.successorRuntime,...(x.regions||[]),...(x.caseTypes||[]),...(x.triggers||[]),...(x.pressureField||[]),...(x.relatedBookSections||[])].join(' '),atlasScope:scope('cases',x.id)});
 for(const x of data.windows?.windows||[])records.push({locale:l,type:'WINDOW',slug:x.id,title:title(x.titleZh,x.titleEn),summary:`${x.startYear}–${x.endYear} · ${x.majorCases?.length||0} linked cases`,href:route('windows','window',x.id),bookTitle,partTitle,themeCode:'CIVILIZATION_RECONFIGURATION',tags:[...(x.regions||[]),...(x.systemLayers||[])],searchText:[x.id,x.titleZh,x.titleEn,x.startYear,x.endYear,...(x.majorCases||[]),...(x.regions||[]),...(x.systemLayers||[])].join(' '),atlasScope:scope('windows',x.id,{windowId:x.id})});
 for(const x of data.snapshots?.snapshots||[])records.push({locale:l,type:'SNAPSHOT',slug:x.id,title:title(x.metadata?.titleZh,x.metadata?.titleEn),summary:`${x.year} · ${x.dataClass||x.knowledgeState||'UNKNOWN'} · ${x.asset?.resolutionStatus||'UNVERIFIED'}`,href:route('snapshots','snapshot',x.id),bookTitle,partTitle,themeCode:'CIVILIZATION_RECONFIGURATION',tags:x.structuredLayers||[],searchText:[x.id,x.year,x.metadata?.titleZh,x.metadata?.titleEn,x.dataClass,x.asset?.resolutionStatus,...(x.majorReconfigurationCases||[]),...(x.structuredLayers||[])].join(' '),atlasScope:scope('snapshots',x.id,{snapshotId:x.id})});

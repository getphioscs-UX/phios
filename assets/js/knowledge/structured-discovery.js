...
export function structuredDiscoveryRows(registry,index,books,locale='en') {
 ...
 const rows=registry.objects.filter(x=>['IN_REVIEW','ACTIVE','ACCEPTED'].includes(x.status)).map(x=>{
  ...
  return {type:'STRUCTURED_OBJECT',ids:[x.objectId],aliases:item?.aliases||[],title,summary:zh?'打开主题，查看可用解释与来源。':'Open this topic for available explanations and sources.',source:x.bookCode,related:x.partCode,href:x.explorerHref,ask:`/knowledge/ask/?${params}`,terms:[x.objectId,x.title,...(item?.aliases||[]),...(item?.keywords||[]),'structured object 结构化对象'].join(' ')};
 });

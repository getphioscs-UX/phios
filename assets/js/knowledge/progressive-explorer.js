...
export async function mountProgressiveExplorer(host,bookCode,locale,openFullReadingView){
 ...
 search.parentNode.firstChild.textContent=tr('Find a topic in this book ','在本书查找主题 ');
 ...
 async function show(id,push=false){
  searchRevision++;clearTimeout(searchTimer);
  ...
  try{
   ...
   if(r.reviewPreview){
    ...
    fields.innerHTML=r.reviewPreview.properties.map(f=>'<details><summary>'+esc(locale==='zh-Hans'?(names[f.field]||f.field):f.field)+'</summary><p>'+esc(f.reason||tr('Source passage selected for review','已选取待审原文'))+'</p>'+f.evidence.map(e=>'<blockquote>'+esc(e.quote)+'<small> · PDF '+esc(e.pages.join('–'))+'</small></blockquote>').join('')+'</details>').join('');detail.append(fields);
   }
 ...
 const filter=()=>{
  const ticket=++searchRevision;clearTimeout(searchTimer);
  if(!search.value.trim()){list(selectedId);return;}
  searchTimer=setTimeout(async()=>{try{
   const matches=await searchStructuredKnowledge(search.value,{bookCode});if(disposed||ticket!==searchRevision)return;
   nav.innerHTML=matches.filter(o=>book.objects[o.objectId]).map(o=>`<button type="button" data-id="${esc(o.objectId)}" aria-pressed="${o.objectId===selectedId}">${esc(title(book.objects[o.objectId]))}</button>`).join('')||`<p role="status">${tr('No matching topics','没有符合的主题')}</p>`;
  }catch{if(!disposed&&ticket===searchRevision)nav.innerHTML=`<p role="status">${tr('Search unavailable. Clear the search to browse topics.','搜索暂不可用，清空搜索即可浏览主题。')}</p>`;}},180);
 };
 ...
 const cleanup=()=>{disposed=true;revision++;searchRevision++;clearTimeout(searchTimer);nav.removeEventListener('keydown',keyboard);host.removeEventListener('click',click);select.removeEventListener('change',change);search.removeEventListener('input',filter);window.removeEventListener('popstate',sync);};

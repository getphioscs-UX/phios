export function renderBookPublicSamples(host,book,locale='en'){
 ...
 for(const f of book.figures){const details=doc.createElement('details'),summary=doc.createElement('summary');summary.textContent=`${zh?'部分':'Part'} ${f.part} · FIG ${f.number}${f.title?.[locale]?' · '+f.title[locale]:''}`;details.append(summary);if(f.deliveryState==='MISSING_BUCKET_OBJECT'){const unavailable=doc.createElement('p');unavailable.textContent=zh?'此总结图暂时无法显示，请参阅本册正文。':'This summary figure is temporarily unavailable. Please consult the book text.';details.append(unavailable);host.append(details);continue;}details.addEventListener('toggle',()=>{if(!details.open||details.querySelector('img'))return;const img=doc.createElement('img');img.src=f.url;img.alt=summary.textContent;img.loading='lazy';img.style.cssText='width:100%;height:auto';const a=doc.createElement('a');a.href=f.url;a.target='_blank';a.rel='noopener';a.textContent=zh?'查看原图':'Open full image';img.onerror=()=>{img.hidden=true};details.append(img,a);});host.append(details);}
 }
}

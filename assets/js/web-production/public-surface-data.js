...
export async function loadCanonicalBooks() {
  ...
  if (!Array.isArray(registry.books) || registry.architecture !== 'five-volume-15-part') {
    throw new Error('WPR_BOOK_REGISTRY_INVALID');
  }
  if (registry.books.length !== 5 || registry.books.some((book, index) => Number(book.volume) !== index + 1)) {
...
export async function loadCanonicalParts() {
  ...
  if (!Array.isArray(registry.parts) || registry.architecture !== 'five-volume-15-part') {
...
export async function loadFiveVolumePublicationContextRegistry() {
  ...
  if (registry?.architecture !== 'five-volume-15-part' || !Array.isArray(registry.partOwnership)) {
...
export function resolvePublicationContextForNode(node, booksRegistry, partsRegistry, contextRegistry) {
  if (!node || !booksRegistry || !partsRegistry) return null;
  ...
  if (!part) return null;
  const book = (booksRegistry.books || []).find(candidate => candidate.book_id === part.book);
  if (!book) return null;
  ...
  return Object.freeze({
    ...
    resolutionAuthority: override ? override.authority : 'CANONICAL_PART_OWNERSHIP_PLUS_CURRENT_BOOK_REGISTRY',
...
export function resolveFigurePublicationContext(figure, booksRegistry, partsRegistry, contextRegistry) {
  if (!figure || !booksRegistry || !partsRegistry) return null;
  ...
  const part = partsRegistry.parts.find(candidate => Number(candidate.number) === number);
  const book = part && booksRegistry.books.find(candidate => candidate.book_id === part.book);
  return book ? Object.freeze({
...
export function figureHasCanonicalBookOwnership(figure, partsRegistry) {
  ...
  if (Number(figure.part) === 0) return Number(figure.book) === 1;

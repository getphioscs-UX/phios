...
export async function loadEightVolumeAssets(){const r=await json(ASSETS);if(!Array.isArray(r.assets)||r.assets.length<15)throw new Error('EIGHT_VOLUME_ASSET_REGISTRY_INVALID');return r}
export const loadSevenVolumeBooks=loadEightVolumeBooks;
export const loadSevenVolumeParts=loadEightVolumeParts;
export const loadSevenVolumeAssets=loadEightVolumeAssets;
...
export async function resolveSevenVolumeAsset(assetId){const r=await loadEightVolumeAssets();const a=sevenVolumeAssetRecord(r,assetId);if(!a?.available||!a?.publicUrl) return null;return {assetCode:a.assetId,src:a.publicUrl,width:a.width??null,height:a.height??null,renderable:true,verification:a.verificationState}}

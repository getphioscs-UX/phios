...
export function renderAtlasShell(root,state,{locale='en',data={},onLayerChange=()=>{},onStateChange=()=>{}}={}){
  if(!root) return; const lang=locale==='zh-Hans'?'zh-Hans':'en'; const c=COPY[lang]; const activeEntry=customerEntryForLayer(state.activeLayer); const presentationMode=data.templateSlots?.layers?.[state.activeLayer]?.presentationMode||null; const systemComposed=presentationMode==='SYSTEM_COMPOSED_FROM_ASSETS';

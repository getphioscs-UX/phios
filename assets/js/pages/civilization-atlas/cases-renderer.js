...
export function renderCases(container,{registry,state,locale='en',onCaseSelect=()=>{},onCompareToggle=()=>{},onSearchChange=()=>{}}={}){
  ...
  container.innerHTML=`<div class="civ-cases" data-atlas-cases>
    ...
    <div class="civ-case-grid">${cases.map((c,index)=>`<article class="civ-case-card${active?.caseId===c.caseId?' is-active':''}" data-case-card data-case-index="${index}"${index>=PAGE?' hidden':''}><div><p class="knowledge-eyebrow">${esc(loc(c.region?.label,lang)||formatRange(c.timeWindow,lang))}</p><h4>${esc(loc(c.title,lang))}</h4><p>${esc(formatRange(c.timeWindow,lang))}</p></div><dl><div><dt>${lang==='zh-Hans'?'地区':'Region'}</dt><dd>${esc(loc(c.region?.label,lang))}</dd></div><div><dt>${lang==='zh-Hans'?'运行特征':'Pattern'}</dt><dd>${esc(label(c.politicalArchitecture,lang)||label(c.economicRuntime,lang))}</dd></div></dl><div class="civ-case-card__actions"><button type="button" class="knowledge-action" data-open-case="${esc(c.caseId)}">${lang==='zh-Hans'?'查看档案':'View dossier'}</button><button type="button" class="knowledge-action knowledge-action--quiet" data-compare-case="${esc(c.caseId)}" aria-pressed="${(state.compareBasket||[]).includes(c.caseId)?'true':'false'}">${(state.compareBasket||[]).includes(c.caseId)?(lang==='zh-Hans'?'移出比较':'Remove compare'):(lang==='zh-Hans'?'＋加入比较':'+ Compare')}</button></div></article>`).join('')}</div>
...
export function renderCaseInspector(container,{caseRecord,state,locale='en'}={}){
  ...
  const rows=[[lang==='zh-Hans'?'时期':'Period',formatRange(caseRecord.timeWindow,lang)],[lang==='zh-Hans'?'地区':'Region',loc(caseRecord.region?.label,lang)],[lang==='zh-Hans'?'空间范围':'Reach',label(caseRecord.geographicReach,lang)],[lang==='zh-Hans'?'政治结构':'Political architecture',label(caseRecord.politicalArchitecture,lang)],[lang==='zh-Hans'?'经济运行':'Economic organization',label(caseRecord.economicRuntime,lang)],[lang==='zh-Hans'?'知识系统':'Knowledge system',label(caseRecord.knowledgeSystem,lang)],[lang==='zh-Hans'?'外部网络':'External network',label(caseRecord.externalNetwork,lang)],[lang==='zh-Hans'?'扩展模式':'Expansion',label(caseRecord.expansionPattern,lang)],[lang==='zh-Hans'?'继任结构':'Successor',label(caseRecord.successorStructure,lang)],[lang==='zh-Hans'?'长期遗产':'Legacy',label(caseRecord.legacy,lang)]];
  container.innerHTML=`<p class="knowledge-eyebrow">${esc(formatRange(caseRecord.timeWindow,lang))}</p><h3>${esc(loc(caseRecord.title,lang))}</h3><dl>${rows.map(([k,v])=>`<div><dt>${esc(k)}</dt><dd>${esc(v)}</dd></div>`).join('')}</dl>${caseRecord.unknown?.note?`<details class="civ-atlas-evidence-note"><summary>${lang==='zh-Hans'?'资料边界':'Evidence note'}</summary><p>${esc(loc(caseRecord.unknown.note,lang))}</p></details>`:''}${(state.compareBasket||[]).length?`<p class="civ-atlas-note">${lang==='zh-Hans'?'已加入比较':'Selected for comparison'}: ${state.compareBasket.length}</p>`:''}`;
  return true;
}

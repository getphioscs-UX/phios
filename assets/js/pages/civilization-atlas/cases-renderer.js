...
export function renderCases(container,{registry,state,locale='en',onCaseSelect=()=>{},onCompareToggle=()=>{},onSearchChange=()=>{}}={}){
  ...
  container.innerHTML=`<div class="civ-cases" data-atlas-cases>
    <div class="civ-cases__toolbar"><label><span>${lang==='zh-Hans'?'搜索文明案例':'Search cases'}</span><input type="search" data-case-search aria-describedby="civ-case-count"  value="${esc(state.caseSearch||'')}" placeholder="${esc(lang==='zh-Hans'?'名称、别名、ID 或 Runtime family':'Name, alias, ID, or runtime family')}"></label><p id="civ-case-count" aria-live="polite">${lang==='zh-Hans'?`显示 ${cases.length} / ${all.length} 个案例`:`Showing ${cases.length} of ${all.length} cases`}</p></div>

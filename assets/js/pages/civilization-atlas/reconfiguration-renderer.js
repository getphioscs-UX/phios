...
const resultTypeLabel=v=>tr('resultType.'+String(v||''),humanize(v));
...
function renderCases(host,data,l,state,store){
 ...
 host.innerHTML=`${caseDetail(selected,l,data,store)}<section class="civ-reconfig-filters" aria-label="${esc(c.filters)}"><h3>${esc(c.filters)}</h3><div class="civ-reconfig-filter-grid"><label>${esc(c.search)}<input data-case-q value="${esc(state.caseSearch)}"></label><label>${esc(c.timeWindow)}<select data-window-filter>${options(data.windows?.windows||[],x=>x.id,(x)=>`${x.startYear}–${x.endYear}`,c.allTime,l)}</select></label><label>${esc(c.region)}<select data-region>${options(regions,x=>x,x=>regionLabel(x),c.allRegions,l)}</select></label><label>${esc(c.caseType)}<select data-type>${options(types,x=>x,(x,loc)=>typeLabel(x,loc),c.allTypes,l)}</select></label><label>${esc(c.trigger)}<input data-trigger value="${esc(state.triggerQuery)}"></label><label>${esc(c.pressureField)}<input data-pressure value="${esc(state.pressureQuery)}"></label><label>${esc(c.structuralChange)}<select data-change><option value="">${esc(c.all)}</option>${[['carrier','Carrier Change'],['boundary','Boundary Change'],['institutional','Institutional Change'],['economic','Economic Change'],['WAR','War'],['REVOLUTION','Revolution'],['REFORM','Reform'],['COLLAPSE','Collapse'],['SUCCESSION','Succession'],['DECOLONIZATION','Decolonization'],['TECHNOLOGICAL_RECONFIGURATION','Technology'],['NETWORK_RECONFIGURATION','Network']].map(([v,n])=>`<option value="${v}">${esc(l==='zh-Hans'?changeLabel(v):n)}</option>`).join('')}</select></label></div><button type="button" data-clear>${esc(c.clear)}</button></section><p class="cx-meta">${all.length} ${esc(c.results)}</p><div class="civ-reconfig-grid">${rows.map(x=>`<article class="civ-reconfig-card"><small>${esc(x.id)} · ${badge(x.knowledgeState||x.dataClass,l)}</small><h4>${esc(l==='zh-Hans'?x.titleZh:x.titleEn)}</h4><p>${esc(x.timeLabel)} · ${esc((x.caseTypes||[]).map(t=>typeLabel(t,l)).join(' · '))}</p><label><input type="checkbox" data-compare="${esc(x.id)}" ${state.compareCaseIds.includes(x.id)?'checked':''}> ${esc(c.compare)}</label><button type="button" data-case="${esc(x.id)}">${esc(c.open)}</button><a href="${esc(askHref(l,'cases',{entityId:x.id}))}">${esc(c.ask)}</a></article>`).join('')}</div>`;
...
function renderSnapshots(host,data,l,state,store){
 ...
 const declaredStatus=visualRecord?.status||'UNVERIFIED';
 const resolved=declaredStatus==='PRESENT'?resolveAtlasVisualById(data.visualBindings,s.visualAssetId):null;
 const visualState=declaredStatus==='PRESENT'&&resolved?'PRESENT':declaredStatus==='MISSING'?'MISSING':'UNVERIFIED';
 const visualTitle=l==='zh-Hans'?s.metadata?.titleZh:s.metadata?.titleEn;
 const visualMarkup=visualState==='PRESENT'
  ?`<figure data-snapshot-visual><img data-snapshot-image loading="lazy" decoding="async" src="${esc(resolved.publicUrl)}" alt="${esc(visualTitle)}"><figcaption>${esc(c.imageBaseNote)}</figcaption></figure>`
  :`<div role="status" class="civ-reconfig-missing" data-resolver-state="${esc(visualState)}"><strong>${esc(visualState==='MISSING'?c.missing:c.unverified)}</strong><br><span>${esc(c.resolverMissing)}</span><br><code>${esc(s.asset?.r2Path||visualRecord?.expectedR2Path||'')}</code></div>`;
 host.innerHTML=`<div class="civ-reconfig-snapshot-controls"><label>${esc(c.selectSnapshot)}<select data-snapshot-select>${rows.map(x=>`<option value="${esc(x.id)}">${esc(String(x.year))}</option>`).join('')}</select></label><label>${esc(c.layer)}<select data-layer>${Object.keys(SNAPSHOT_FIELDS).map(k=>`<option value="${k}">${esc(layerLabel(k))}</option>`).join('')}</select></label></div><article class="civ-reconfig-snapshot"><div class="civ-reconfig-detail-head"><div><small>${badge(s.knowledgeState||s.dataClass,l)}</small><h3>${esc(visualTitle)}</h3></div><a href="${esc(askHref(l,'snapshots',{snapshotId:s.id,entityId:s.id}))}">${esc(c.ask)}</a></div>${visualMarkup}<section class="civ-reconfig-layer-panel"><h4>${esc(layerLabel(state.snapshotLayer))}</h4><p>${values.length?esc(values.map(v=>typeof v==='object'?loc(v,l)||Object.values(v).map(valueLabel).join(' · '):valueLabel(v)).join(' · ')):esc(c.unknown)}</p></section><details open><summary>${esc(c.textAlternative)}</summary><p><strong>${esc(c.dataState)}:</strong> ${esc(stateLabel(s.dataClass||s.knowledgeState))}</p><p><strong>${esc(c.relatedCases)}:</strong> ${esc(cases.map(id=>l==='zh-Hans'?caseMap.get(id)?.titleZh:caseMap.get(id)?.titleEn).filter(Boolean).join(' · ')||c.unknown)}</p><p><strong>${esc(c.unknown)}:</strong> ${esc(listText(s.unknown,l))}</p></details></article>`;
 const img=host.querySelector('[data-snapshot-image]');
 img?.addEventListener('error',()=>{const figure=img.closest('[data-snapshot-visual]');if(figure)figure.outerHTML=`<div role="status" class="civ-reconfig-missing" data-resolver-state="RUNTIME_LOAD_FAILED"><strong>${esc(c.missing)}</strong><br><span>${esc(c.resolverMissing)}</span><br><code>${esc(s.asset?.r2Path||'')}</code></div>`;},{once:true});
...
function renderDossiers(host,data,l,state,store){
 ...
 const body=admitted
  ?runtimeDl(d,l)
  :`<section class="civ-reconfig-current-boundary"><h4>${esc(c.currentDataNotAdmitted)}</h4><p>${esc(c.currentDataBoundary)}</p>${linkedCases.length?`<p><strong>${esc(c.historicalEvidence)}:</strong> ${esc(linkedCases.slice(0,8).map(x=>l==='zh-Hans'?x.titleZh:x.titleEn).join(' · '))}</p>`:''}</section>`;
...
function renderLived(host,data,l,state,store){
 ...
 if(sel){sel.value=dossier?.id||'';sel.onchange=e=>set(store,{dossierId:e.target.value},'lived-dossier');}
...
function renderDossierCompare(host,data,l,state){
 ...
 const listRows=[[fieldLabel('externalDependency'),d=>listText(d.externalDependency,l)],[fieldLabel('pressure'),d=>listText(d.pressureFields,l)],[fieldLabel('direction'),d=>listText(d.direction,l)],[fieldLabel('transitionSignals'),d=>listText(d.transitionSignals,l)]];

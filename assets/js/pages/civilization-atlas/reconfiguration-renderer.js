...
function renderCases(host,data,l,state,store){
 ...
 const wf=host.querySelector('[data-window-filter]');selectValue(wf,state.filterWindowId||'');const rg=host.querySelector('[data-region]');selectValue(rg,state.regionId||'');const ty=host.querySelector('[data-type]');selectValue(ty,state.caseType||'');const ch=host.querySelector('[data-change]');selectValue(ch,state.changeFilter||'');
...
function renderSnapshots(host,data,l,state,store){
 ...
 const ss=host.querySelector('[data-snapshot-select]');selectValue(ss,s.id);ss.onchange=e=>set(store,{snapshotId:e.target.value},'snapshot-select');const ly=host.querySelector('[data-layer]');selectValue(ly,state.snapshotLayer);ly.onchange=e=>set(store,{snapshotLayer:e.target.value},'snapshot-layer');
}

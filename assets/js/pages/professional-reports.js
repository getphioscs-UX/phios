...
function render() {
  ...
  document.querySelector('#reportContent').innerHTML = list(payload.sections).filter(section => section.client_visible !== false).map(section => `<section class="report-section" data-source-type="${esc(section.source_type)}"><h3>${esc(localized(section.title))}</h3><p>${esc(localized(section.content))}</p><small>${esc(t('professionalReports.source'))}: ${esc(section.source_type)} · ${esc(t('professionalReports.confidence'))}: ${esc(section.confidence)}</small></section>`).join('');
  document.querySelector('#reportSources').innerHTML = `<div class="report-source-list">${list(payload.sections).map(section => `<article><strong>${esc(localized(section.title))}</strong><dl><div><dt>${esc(t('professionalReports.source'))}</dt><dd>${esc(section.source_type)}</dd></div><div><dt>${esc(t('professionalReports.sourceReference'))}</dt><dd>${esc(section.source_reference)}</dd></div><div><dt>${esc(t('professionalReports.correspondence'))}</dt><dd>${esc(section.correspondence_status)}</dd></div><div><dt>${esc(t('professionalReports.registryVersion'))}</dt><dd>${esc(section.registry_version || '—')}</dd></div></dl></article>`).join('')}</div>`;

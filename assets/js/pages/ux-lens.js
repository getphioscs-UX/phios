...
function renderNext(state,payload){const node=result.querySelector('[data-uxl-next]');const actions=[];if(state==='NEEDS_CONTEXT'||payload?.ask2?.plan?.orchestrationState==='CURRENT_CONTEXT_REQUIRED')actions.push(`<button class="uxl-primary" type="button" data-next-context>${esc(T.addContext())}</button>`);if(state==='ASK2_INPUT_REQUIRED')actions.push(`<span class="uxl-state">${esc(T.addInput())}</span>`);if(state==='NEEDS_CURRENT_AUTHORITY')actions.push(`<span class="uxl-state">${esc(zh()?'这需要最新受治理来源':'Up-to-date governed sources are required')}</span>`);actions.push(`<button class="uxl-secondary" type="button" data-stage15-reality>${esc(zh()?'带回 Reality 继续':'Continue in Reality')}</button>`);actions.push(`<a class="uxl-secondary" href="/library?query=${encodeURIComponent(lastQuestion)}">${esc(T.searchKnowledge())}</a>`);node.innerHTML=actions.join('');node.querySelector('[data-next-context]')?.addEventListener('click',()=>{contextPanel.hidden=false;contextPanel.scrollIntoView({behavior:'smooth',block:'center'})});node.querySelector('[data-stage15-reality]')?.addEventListener('click',async()=>{const answer=payload?.cka?.clientAnswer||{};const external=(payload?.ask2?.plan?.currentReality?.external||[]).map(x=>({sourceId:x.sourceId,statement:x.claimText||x.title,sourceUrl:x.sourceUrl,authorityClass:x.authorityClass}));try{await openRealityEntryWithBundle({sourceType:'ASK',locale:getLocale(),source:{question:lastQuestion,reportedContext:[],externalEvidence:external,unknown:answer.unknown?.details||[]}})}catch(e){setStatus(`${T.failed()} (${e.message})`)}})}
...
onLocaleChange(()=>{if(lastPayload)render(lastPayload)});const params=new URLSearchParams(location.search);const initialIntent=String(params.get('intent')||'').toUpperCase();if(['CURRENT','DECISION','RELATIONSHIP','TIME','DOMAIN','STRUCTURE'].includes(initialIntent)){taxonomyHint=initialIntent;root.querySelectorAll('[data-taxonomy-hint]').forEach(x=>x.setAttribute('aria-pressed',String(x.dataset.taxonomyHint===taxonomyHint)))}const initial=params.get('q');if(initial){question.value=initial.slice(0,500);queueMicrotask(()=>run())}
}

...
function renderEligibility(input,entries){ const target=document.querySelector('[data-mcd7-eligibility]'); if(!target)return []; const targetDate=value('targetDate')||null; const checks=entries.map(entry=>({entry,evaluation:evaluateSurfaceEligibility(entry,input,{targetDate})})); target.innerHTML=checks.length?checks.map(({entry,evaluation})=>`<article class="pr-eligibility-card" data-state="${escapeHTML(evaluation.state)}"><strong>${escapeHTML(labelFor(entry))}</strong><span>${escapeHTML(evaluation.state)}</span>${evaluation.missingFields.length?`<small>${escapeHTML(evaluation.missingFields.join(' · '))}</small>`:''}<em>${escapeHTML(locale()==='zh-Hans'?'真正的处理资格与 dispatch 会在 Processing 阶段由治理规则决定':'Actual processing eligibility and dispatch are decided by governed rules during Processing')}</em></article>`).join(''):`<p>${escapeHTML(t('personalRuntime.noMethodsSelected'))}</p>`; return checks; }

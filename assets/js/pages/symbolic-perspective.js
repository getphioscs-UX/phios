...
let method=new URLSearchParams(location.search).get('method')||'I_CHING';
method=method==='TAROT'?'TAROT':'I_CHING';
...
function setMethod(next){
  method=next;
  qa('[data-method]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.method===method)));
  loadContext();
}
...
async function loadContext(){
  ...
  try{
    const r=await fetch(`/api/symbolic-method-context?method=${encodeURIComponent(method)}&useCurrentRealityContext=${use?'1':'0'}`,{cache:'no-store'});
    ...
    if(execute)execute.disabled=contextPayload.production?.runAllowed!==true;
...
async function execute(){
  ...
  finally{button.disabled=contextPayload?.production?.runAllowed!==true;}
}

qa('[data-method]').forEach(b=>b.addEventListener('click',()=>setMethod(b.dataset.method)));

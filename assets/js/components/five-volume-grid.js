...
async function render(root){const d=await loadSevenVolumeBooks();const l=locale();root.innerHTML=d.books.map((b,i)=>`<article class="puxr-card puxr-book ${cls[i]||''}"><div><div class="puxr-card__kicker">Book ${['I','II','III','IV','V','VI','VII'][i]}</div><h3>${esc(b.title?.en||'')}</h3><p>${esc(b.title?.['zh-Hans']||'')}</p></div><p class="puxr-micro">${esc(b.subtitle?.[l]||b.subtitle?.en||'')}</p><div class="puxr-actions"><a class="puxr-btn" href="/books/">${l==='zh-Hans'?'查看本册':'Explore volume'}</a></div></article>`).join('')}
const roots=[...document.querySelectorAll('[data-px2-five-volumes],[data-px2-seven-volumes]')];

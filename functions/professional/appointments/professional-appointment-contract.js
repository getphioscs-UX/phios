...
import {
  ...
  PRE_APPOINTMENT_CHECKS, PROFESSIONAL_APPOINTMENT_CONTRACT_VERSION
...
export function createProfessionalAppointment(input = {}, options = {}) {
  ...
  const checks = Object.freeze(PRE_APPOINTMENT_CHECKS.reduce((result, key) => {
    result[key] = input.pre_appointment_checks?.[key] === true;
    return result;
  }, {}));
  const incompleteChecks = Object.freeze(PRE_APPOINTMENT_CHECKS.filter(key => !checks[key]));
  return Object.freeze({
    ...
    service_type: allowed(input.service_type, APPOINTMENT_SERVICE_TYPES, 'service_type'),

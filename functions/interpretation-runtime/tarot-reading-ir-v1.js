 ...
 */
import {bindTarotCorpusProjections} from './adapters/tarot-interpretation-adapter-v2.js';

export const TAROT_READING_IR_VERSION='1.0.0';
export const TAROT_READING_IR_SCHEMA='PHI-OS-TAROT-READING-IR-v1.0.0';
...
function uncertaintyFor(binding,rcc,input={}){
  ...
  const editorialMissing=binding.cards.filter(card=>card.sourcePerspectives.find(x=>x.perspectiveClass==='AUTHOR_SPECIFIC')?.editorialClaims.length===0);
  if(editorialMissing.length) add('UNRESOLVED',`SOURCE_LOCATOR_PRESENT_BUT_EDITORIAL_PARAPHRASE_NOT_INGESTED_FOR_${editorialMissing.length}_CARD${editorialMissing.length===1?'':'S'}`,'AUTHOR_SPECIFIC');
...
function selectedReflectiveQuestion(card){
  ...
  const unit=reflective?.inquiryUnits?.find(x=>x.focusId===focus)||reflective?.inquiryUnits?.find(x=>x.focusId==='NEXT_OBSERVATION')||reflective?.inquiryUnits?.[0];
  if(!unit) throw new TypeError(`TAROT_REFLECTIVE_QUESTION_UNAVAILABLE:${card.structuralCard.cardId}`);
  return Object.freeze({
    ...
    selectionBasis:'SPREAD_POSITION_ONLY_NOT_CARD_MEANING',
...
function compositionEvidenceRecord({binding,draw,authorities,evidence={}}){
  ...
  return Object.freeze({
    ...
    reflectiveLensRegistryVersion:authorities.reflectiveLensRegistry.registryVersion,
...
export function createTarotReadingIR({question='',contextDisclosure={},projections,authorities,realityEvidence={},compositionEvidence={}}={}){
  ...
  const binding=bindTarotCorpusProjections(projections,authorities);
  ...
  const reflectiveComposition=Object.freeze({
    mode:'DETERMINISTIC_REFLECTIVE_QUESTION_SELECTION',
    questions:Object.freeze(binding.cards.map(selectedReflectiveQuestion)),
    selectionUsesCardMeaning:false,
    selectionUsesSpreadPositionOnly:true,
  ...
  return Object.freeze({
    ...
    authority:Object.freeze({
      ...
      interpretationSuccessor:'TARI_CORPUS_BOUND_V2',

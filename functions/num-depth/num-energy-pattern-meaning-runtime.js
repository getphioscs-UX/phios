import {freezeDeep,NUM_DEPTH_ENERGY_SCHOOL,NUM_DEPTH_PRODUCTION_ACTIVE} from './num-depth-rules.js';
...
export function buildNumEnergyPatternMeaningCandidate({pattern,locale='en'}={}){const p=String(pattern||'');const row=P[p];if(!row)return freezeDeep({schemaVersion:NUM_D5_PATTERN_SCHEMA,workCode:'NUM-D5',school:NUM_DEPTH_ENERGY_SCHOOL,pattern:p,availability:'STRUCTURAL_ONLY_NO_SOURCE_WITNESSED_MEANING',runtimeUseAllowed:false,customerPublishable:false});const canonical={'156':'516','786':'876','819':'189','977':'797','729':'279','639':'369','459':'549','538':'358','898':'988','933':'393','213':'123','742':'472','415':'145','865':'685'}[p]||p;return freezeDeep({schemaVersion:NUM_D5_PATTERN_SCHEMA,workCode:'NUM-D5',school:NUM_DEPTH_ENERGY_SCHOOL,pattern:p,canonicalClaimPattern:canonical,text:locale==='zh-Hans'?row[1]:row[0],sourceClaimId:`NUM-D5-PATTERN-${canonical}`,availability:'SOURCE_WITNESSED_CANDIDATE',reviewState:'HUMAN_ADMITTED',runtimeUseAllowed:NUM_DEPTH_PRODUCTION_ACTIVE,customerPublishable:NUM_DEPTH_PRODUCTION_ACTIVE,boundaries:{wealthGuaranteeRemoved:true,careerPrescriptionRemoved:true,fortunePredictionAllowed:false,financialAdviceAllowed:false}})}

import {freezeDeep,NUMBER_THEMES,NAME_ROLE_META,NUM_DEPTH_WESTERN_SCHOOL,NUM_DEPTH_PRODUCTION_ACTIVE} from './num-depth-rules.js';
...
export function buildNumNameRoleMeaningCandidate({role,value,locale='en',secondaryChart,currentNameProfile}={}){
 ...
 const runtimeUseAllowed=NUM_DEPTH_PRODUCTION_ACTIVE&&eligible;
 return freezeDeep({schemaVersion:NUM_D1_NAME_MEANING_SCHEMA,workCode:'NUM-D1',school:NUM_DEPTH_WESTERN_SCHOOL,role,value:current,text,sourceClaimId:`NUM-D1-${role}-${String(current).padStart(2,'0')}`,availability:runtimeUseAllowed?'AVAILABLE':'BLOCKED_METHOD_ALIGNMENT',methodAlignment:{state:alignment,eligible,required:true,sourceAlignedValue:sourceValue,currentValue:Number(currentValue)},reviewState:'HUMAN_ADMITTED',runtimeUseAllowed,customerPublishable:runtimeUseAllowed,boundaries:{symbolicTraditionOnly:true,identityTotalizationAllowed:false,fortunePredictionAllowed:false,sourceCalculationOverrideAllowed:false}});
}

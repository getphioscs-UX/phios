...
export const TAROT_PRODUCT_VIEW_MODEL_VERSION='1.0.0';
...
export function createTarotProductPublicViewModel(readingIr={}){
  if(readingIr?.schemaVersion!=='PHI-OS-TAROT-READING-IR-v1.0.0'||readingIr?.methodCode!=='TAROT') throw new TypeError('TAROT_READING_IR_REQUIRED');
  ...
  return Object.freeze({...view,hierarchy,schemaVersion:'PHI-OS-TAROT-PRODUCT-PUBLIC-VIEW-MODEL-v1.0.0',viewModelVersion:TAROT_PRODUCT_VIEW_MODEL_VERSION,readingIrVersion:readingIr.readingIrVersion,tarotSurface:Object.freeze({cards,agency:clone(readingIr.agency),uncertainty:clone(readingIr.uncertainty),compositionEvidence:clone(readingIr.compositionEvidence)}),production:Object.freeze({surfaceReady:true,runAllowed:false,productionCapabilityPromoted:false})});
}

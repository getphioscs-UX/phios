...
export const TAROT_RUNTIME_VERSION = '2.0.0';
...
function authorities({deckContract,cardRegistry,spreadRegistry,orientationPolicy}) {
  ...
  const spreadEntries=spreadRegistry.entries||[];
  const spreads=new Map(spreadEntries.map(x=>[x.spreadId,x]));
  const aliases=new Map();
  for (const spread of spreadEntries) {
    if (!spread?.spreadId || ![1,3,5,6,7,10].includes(spread.cardCount) || !Array.isArray(spread.positions) || spread.positions.length !== spread.cardCount) throw new TypeError('TAROT_TSCP_SPREAD_REGISTRY_INVALID');
    for (const alias of spread.legacyAliases||[]) aliases.set(alias,spread.spreadId);
  }
  if (spreads.size < 8 || !spreads.has('ONE_CARD_FOCUS') || !spreads.has('THREE_CARD_SITUATION') || !spreads.has('CELTIC_CROSS_10')) throw new TypeError('TAROT_TSCP_SPREAD_REGISTRY_REQUIRED');
  const cards=new Map(cardRegistry.entries.map(x=>[x.cardId,x]));
  return Object.freeze({deck:Object.freeze(clone(deck)),cards,eligibleCards:Object.freeze(cardRegistry.entries.map(x=>x.cardId)),spreads,aliases,orientationPolicy:Object.freeze(clone(orientationPolicy))});
}
...
export function createTarotSelectionRuntime(config) {
  ...
  return Object.freeze({
    ...
    async select(request={}) {
      ...
      const resolvedSpreadId=a.aliases.get(request.spreadId)||request.spreadId;
      const spread=a.spreads.get(resolvedSpreadId);
      ...
      if (inputMode === 'MANUAL_SELECTION') {
        ...
        if (spread.systemRandomAllowed !== true) throw new TypeError('TAROT_CUSTOMER_MANUAL_SELECTION_REQUIRED_FOR_SPREAD');
...
export async function validateTarotEvidenceBundle(bundle, config) {
  ...
  const resolvedSpreadId=a.aliases.get(d.spreadId)||d.spreadId; const spread=a.spreads.get(resolvedSpreadId); if (!spread || d.spreadVersion !== spread.spreadVersion) throw new TypeError('TAROT_GOVERNED_SPREAD_REQUIRED');

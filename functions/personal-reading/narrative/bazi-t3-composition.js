 ...
 try{
  return await Promise.race([(async()=>{
   ...
   for(let attempt=0;attempt<2;attempt++){
    const readingRules='Only admittedInterpretations license personal interpretation. Numeric method facts do not establish behavioral causes. Glossary entries define terms but do not establish their occurrence or personal effect; cite their term IDs only for definitions. Counter-signal and reflection questions are questions, not observed answers. Boundary-only policy supports boundaries, never personal claims. A hypothetical everyday illustration must be explicitly conditional and express an already admitted relationship, without a new causal mechanism or assertion that it happened. Never name a functional emphasis as an established pattern verdict. Preserve relative emphasis explicitly in the lead or interpretation: when the source names a leading group, say which themes stand out most in ordinary language, then distinguish secondary contextual themes. For example, if and only if admitted, say learning and support stand out most here; do not flatten ranked themes into an equal list. A relative symbolic emphasis is not a fixed trait, causal effect or pattern verdict. Avoid the technical phrase leading functional group. Preserve this ranking in every repair, without exposing weights, percentages or counts. Do not demand raw weights in repair instructions.';

...
import {buildS02HumanReviewFollowup} from './bazi-s02-editorial-depth.js';
...
export async function composeBaziT3Section({pack,registry={},env={},fetcher,providerAdapters,timeoutMs=45000,snapshot=null}={}){
 ...
 if(pack.schemaVersion!=='BAZI_SECTION_EVIDENCE_PACK_V5'||!pack.licensedClaims.some(c=>c.relationType!=='BOUNDARY'))return fallback('INSUFFICIENT_ADMITTED_INTERPRETATION');
 ...
 try{
  return await Promise.race([(async()=>{
   ...
   for(let attempt=0;attempt<2;attempt++){
    ...
    const editorialIntent={maximumMainUnits:pack.claimIrVersion?Math.max(pack.locale==='en'?420:700,planned*(pack.locale==='en'?70:110)):pack.locale==='en'?320:520,maximumItemUnits:pack.locale==='en'?90:140,noMinimumLength:!pack.editorialDepthContract,semanticDepthMinimums:pack.editorialDepthContract?.minimumUnits||null,semanticContinuationPages:Boolean(pack.claimIrVersion)};
    const humanReviewFollowup=buildS02HumanReviewFollowup(pack);
    const humanReviewInstructions=humanReviewFollowup?' English S02 has a recorded human editorial rejection: a previous machine-pass candidate was too close to internal-structure paraphrase and did not provide enough customer explanation. Treat humanReviewFollowup and editorialBenchmark as mandatory editorial guidance, never as factual licenses. Do not repair this by inventing manifestations, behavioral effects, causes or examples. When manifestationLicenses is empty, howThisMayShowUp remains empty. Keep the unresolved strong-or-weak point brief and subordinate. Substantive interpretation must precede reflection prompts.':'';
    const candidate=await call('REPORT_SECTION_COMPOSITION',{repair,editorialIntent,sectionNarrativeBrief:pack.sectionNarrativeBrief||null,...(humanReviewFollowup?{humanReviewFollowup,editorialBenchmark:pack.editorialGoldStandard}: {})},COMPOSITION_SCHEMA,COMPOSITION_PROMPT+' Observe editorialIntent maximum layout limits. When semanticDepthMinimums are present they are editorial-depth acceptance thresholds, not permission to add filler. Follow SectionNarrativeBrief and its locale-specific style contract. Explain only the ordered licensed meaning atoms. Do not repeat preceding visual counts or percentages. Preserve substantive conditions without repeating general disclaimers in every paragraph; one local scope sentence is enough, with complete limits in Method & Appendix.'+scopeInstructions+humanReviewInstructions);

...
export async function buildBaZiNarrativeClaimIR({reading,sectionKey,locale,temporalSnapshot}){
 ...
 if(topic&&narrative){
  ...
  if(narrative.development?.[lang]){const secondaryLabels=secondary.map(g=>group(g.groupCode)).filter(Boolean),domainText=sectionKey==='S02_PERSONALITY'?say(`Capability is not read here as a single trait. ${group(topic.leadGroup?.groupCode)} is the first emphasis, while ${secondaryLabels.join(', ')||'other functional themes'} remain part of the same picture. The method therefore keeps absorption, practice, expression and repeated carrying in view together, while the recorded relations among self-position, environment and expression keep capability context-sensitive rather than reducing it to one personality label.`,`能力在这里不是单一特质。「${group(topic.leadGroup?.groupCode)}」是第一重点，同时${secondaryLabels.join('、')||'其他功能主题'}仍属于同一幅结构图。方法因此把吸收、练习、表达与反复承载放在一起阅读；自我位置、环境与表达之间已经记录的关系，也要求能力保持情境性，而不是被压成一句固定人格标签。`):narrative.development[lang];add('DOMAIN_EXPLANATION','LIFE_DOMAIN_EXPLANATION',topicCode,[topicCode],domainText,[`${narrativeRef}/development`,`${topicRef}/leadGroup`,`${topicRef}/relevantTenGods`,`${topicRef}/relationshipInterfaces`,`${topicRef}/carryingContext`,`${topicRef}/priorityRefs`],{conditions:['METHOD_OWNED_MULTI_FACTOR_EXPLANATION','NOT_OBSERVED_REALITY'],explanationScope:'LIFE_DOMAIN'});}

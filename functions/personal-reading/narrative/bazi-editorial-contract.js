...
export const COMPOSITION_POLICY_REVISION='BAZI_LICENSED_EVIDENCE_ADAPTIVE_V3';
...
export async function buildSectionEvidencePack({interpretation,locale,sectionKey,reading}){
 ...
 const contentPlan={interpretation:ids(['ASSOCIATION','CO_OCCURRING_DIMENSIONS','CONTEXT_MODIFIER','CONTRAST','OPEN_CONDITION','TEMPORAL_RELEVANCE','CROSS_SECTION_RELEVANCE']),supportingConditions:ids(['SUPPORT_CONDITION']),tensionConditions:ids(['TENSION']),howThisMayShowUp:authority.manifestationLicenses.map(c=>c.id),counterSignals:authority.counterPrompts.map(c=>c.id),observationPrompt:authority.reflectionQuestions.map(c=>c.id),realityCheck:[]};
 const facts=interpretation.canonicalFacts||[];
 const pack={schemaVersion:'BAZI_SECTION_EVIDENCE_PACK_V3',explanatoryAuthorityVersion:EXPLANATORY_AUTHORITY_VERSION,compositionPolicyRevision:COMPOSITION_POLICY_REVISION,sectionKey,locale,licensedClaims,licensedRelations:licensedClaims.map(({id,subject,relationType,objects,rank,conditions,openConditions})=>({id,subject,relationType,objects,rank,conditions,openConditions})),primaryThemes:licensedClaims.filter(c=>c.rank===1),secondaryThemes:licensedClaims.filter(c=>c.relationType==='ASSOCIATION'),manifestationLicenses:authority.manifestationLicenses,reflectionQuestions:authority.reflectionQuestions,counterSignals:authority.counterPrompts,temporalClaims:licensedClaims.filter(c=>c.relationType==='TEMPORAL_RELEVANCE'),prohibitedOperators:PROHIBITED_OPERATORS,canonicalFacts:facts,facts,openConditions:licensedClaims.filter(c=>c.relationType==='OPEN_CONDITION'),sourceLineage:authority.sourceLineage,temporalContext:interpretation.temporalContext,temporalAuthority:authority.temporalAuthority,integratedGuidanceIR:authority.integratedGuidanceIR,contentPlan,listMinimums:Object.fromEntries(LIST_FIELDS.map(k=>[k,contentPlan[k].length])),sourceFactIds:[...licensedClaims,...authority.reflectionQuestions,...authority.counterPrompts].map(c=>c.id)};
 return withEditorialMeaningBrief({...pack,canonicalEvidenceHash:await sha256Stable(pack)});
}
export const COMPOSITION_PROMPT=`Write a plain-language BaZi section using ONLY the licensed claims and question licenses in SectionEvidencePack V3. Raw canonicalFacts are context, not permission to invent an explanation. Every nonempty block must cite licensed IDs in factRefs and declare its kind, allowed composer operator and exact licensed relationType. Each block expresses ONE relation type. Use CUSTOMER_CLAIM for interpretation, OBSERVATION_PROMPT for questions, COUNTER_PROMPT for counter questions, BOUNDARY for boundaryNote, TECHNICAL_NOTE only for optional definitions. An observation question is not evidence, an event or an answer. Follow contentPlan: one compact block per listed ID in its assigned list, preserving distinct pairwise endpoints. Empty lists in contentPlan MUST be empty in output. If no manifestation license exists, howThisMayShowUp MUST be []; use observationPrompt instead. Do not fill absent support conditions. Preserve primary emphasis explicitly, but do not turn reading priority into a personality measurement. headline, lead and closingInsight may paraphrase a primary or dimensions claim; boundaryNote must cite the boundary claim. technicalNote may be an empty block. Do not expose IDs, weights, percentages, provider information or source-machine phrasing. Only PARAPHRASE, PLAIN_LANGUAGE_ABSTRACTION, RANK_PRESERVING_SUMMARY, CONDITIONAL_REFRAME, CONTRAST and QUESTION_GENERATION are permitted. Association is not cause; co-occurring dimensions are not a sequence. Never say learning leads to expression, practice produces reliability, a context improves performance, or a hypothetical behavior occurs unless a specific license permits it. All current causal/sequence/reality permissions are false. Questions may be faithfully paraphrased, never expanded into new counterexamples. Main prose should be concise and can be short when evidence is sparse. No minimum word target and no filler. S09 must retain the method-owned IntegratedGuidanceIR order, links and timing relevance; do not choose your own themes. Write naturally in the requested locale, not as a description of your task.`;
...
export function validateEditorial(n,pack){
 ...
 for(const k of LIST_FIELDS)if(!Array.isArray(n?.[k])||n[k].length>16)issues.push(`LIST_SCHEMA:${k}`);
 ...
 return {version:EDITORIAL_VERSION,status:issues.length?'REJECT':'PASS',issues,defects,...(quality?{quality}:{})};
}

...
export const PPR_R3_SPECIALIST_RENDERER_REFERENCE_CONTRACT='PHI-OS-PPR-R3-SPECIALIST-RENDERER-PORT-v1.0.0';
...
function specialistRendererReference(value,methodId){
 if(value==null)return null;if(!value||typeof value!=='object'||!value.rendererId)fail('PPR_R3_SPECIALIST_RENDERER_ID_REQUIRED',{methodId});
 for(const key of Object.keys(value))if(/module|export|url|href|src/i.test(key))fail('PPR_R3_EXECUTABLE_RENDERER_DESCRIPTOR_FORBIDDEN',{methodId,key});
 if(value.surfaceContract!==PPR_R3_SPECIALIST_RENDERER_REFERENCE_CONTRACT)fail('PPR_R3_SPECIALIST_SURFACE_CONTRACT_REQUIRED',{methodId});
 return freeze({rendererId:String(value.rendererId),surfaceContract:value.surfaceContract,capabilities:uniq(value.capabilities),ownerMethod:methodId});
}
export function buildMethodProductEnvelope({methodId,productType,locale='en',state,publication,hero,navigation=[],sections=[],visuals=[],lineage={},boundaries={},sourceProduct=null,specialistRenderer=null}={}){
 ...
 return freeze({schemaVersion:PERSONAL_REALITY_METHOD_PRODUCT_SCHEMA,methodId,productType,locale:localeOf(locale),state,publication:freeze({...publication}),hero:freeze({...hero}),navigation:nav,sections:xs,visuals:list(visuals),specialistRenderer:specialistRendererReference(specialistRenderer,methodId),lineage:freeze({...lineage}),boundaries:freeze({...baseBoundaries,...boundaries}),sourceProduct});
}

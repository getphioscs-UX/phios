...
import {buildNumExpansionProductionIR} from '../num-expansion/num-expansion-production-runtime.js';
import {buildNumDepthProductionIR} from '../num-depth/num-depth-production-runtime.js';
...
export function buildNumIntegratedReadingIR({projection,bundle,localeProjection,expansionInput={}}={}){
 ...
 const locale=localeProjection.locale==='zh-Hans'?'zh-Hans':'en';const snapshot=coreSnapshot(projection,locale);const relationships=relationshipFindings(projection,locale);const groups=cycleGroups(projection);const meanings=meaningClusters(localeProjection);const standoutThemes=themes(projection,relationships,locale);const richReading=buildNumRichReviewPreview({snapshot,relationships,calendar:groups.calendar,locale});const expansion=buildNumExpansionProductionIR({projection,expansionInput});const depth=buildNumDepthProductionIR({projection,expansionInput,expansion,locale});
 const result={schemaVersion:NUM_INTEGRATED_READING_IR_SCHEMA,rulesSchema:NUM_FP_SCHEMA,runtimeVersion:NUM_FP_VERSION,methodCode:'NUMEROLOGY',publicationState:'CUSTOMER_PUBLISHABLE',customerPublishable:true,reviewPreviewEligible:false,sourceProjectionId:projection.projectionId,sourceMeaningBundleCode:bundle.bundleCode,locale,executionCompleteness:projection.projection?.status==='COMPLETE'?'COMPLETE':projection.projection?.status==='PARTIAL'?'PARTIAL':'UNAVAILABLE',sections:{snapshot,standoutThemes,relationships,canonicalMeaningClusters:meanings,timing:{calendar:groups.calendar,pinnacles:groups.pinnacles,challenges:groups.challenges,lifePeriod:expansion.sections?.lifePeriod?.runtimeUseAllowed===true?expansion.sections.lifePeriod:{availability:'BIRTH_DATE_CONTEXT_REQUIRED'}},integratedNarrative:integratedNarrative(projection,relationships,locale),realityReflection:reflectionPrompts(relationships,projection,locale),calculationDetails:{values:snapshot,structures:projection.calculation?.structures||[],evidence:projection.evidence||[],unknown:projection.unknown||[]},sourceReviewCandidates:{karmicDebtCompounds:compoundReviewCandidates(projection)},richReading,expansion,depth},semanticDepth:{numberMeaning:'HUMAN_ADMITTED_RICH_TRADITION',roleNumberRichMeaning:'HUMAN_ADMITTED',relationships:'STRUCTURAL_PLUS_ADMITTED_COMPOSITION',timingMeaning:'HUMAN_ADMITTED_1_TO_9',expansionStructure:'NUM_R18_HUMAN_ADMITTED_CALCULATION_BOUND_SCOPE',depthMeaning:'NUM_D1_D8_HUMAN_ADMITTED_FULL_PRODUCTION'},deduplication:{canonicalMeaningsBefore:(localeProjection.items||[]).length,canonicalMeaningClustersAfter:meanings.length,repeatedRoleDefinitionsSuppressed:true},boundaries:{fortunePredictionCreated:false,professionalJudgmentCreated:false,recalculated:false,meaningInvented:false,karmicDebtMeaningAsserted:false,lifePeriodInvented:false,identityFactCreated:false,expansionFortunePredictionCreated:false,relationshipCompatibilityScoreCreated:false,numD8FortunePredictionCreated:false,numD8CompatibilityScoreCreated:false}};

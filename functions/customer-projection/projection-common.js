...
export const upstreamState=(value,fallback='UNKNOWN')=>clean(value)||fallback;
...
export function boundary(){return deepFreeze({createsAuthority:false,calculates:false,infersNewFinding:false,changesMeaning:false,recommends:false,createsTruth:false,createsAvailability:false,createsEntitlement:false,createsAuthentication:false,createsProfessionalJudgment:false});}
export function sourceLineage(authorities=[]){return deepFreeze({sourceAuthorities:uniq(list(authorities).map(clean).filter(Boolean)),authorityPreserved:true});}

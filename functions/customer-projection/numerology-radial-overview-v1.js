...
function secondaryMetric(chart,locale){const hp=arr(chart?.hiddenPassions).slice(0,2),kl=arr(chart?.karmicLessons);if(hp.length)return `${text(locale,'Passion','热能')} ${joinValues(hp).replace(/ · /g,'·')}`;if(kl.length)return `${text(locale,'Missing','缺失')} ${kl.length}`;const values=[chart?.balance,chart?.rationalThought].filter(v=>v!==null&&v!==undefined);return joinValues(values).replace(/ · /g,'·')}
function cycleMetric(timeline,locale){const p=arr(timeline?.pinnacles).map(x=>x?.value).filter(v=>v!==null&&v!==undefined).slice(0,2),c=arr(timeline?.challenges).map(x=>x?.value).filter(v=>v!==null&&v!==undefined).slice(0,2);if(p.length||c.length){const lines=[];if(p.length)lines.push(`${text(locale,'Pinnacle','高峰')} ${joinValues(p).replace(/ · /g,'·')}`);if(c.length)lines.push(`${text(locale,'Challenge','挑战')} ${joinValues(c).replace(/ · /g,'·')}`);return lines.join('\n')}const life=arr(timeline?.lifePeriods).map(x=>x?.pattern).filter(Boolean).slice(0,2);return joinValues(life).replace(/ · /g,' / ')}
...
function relationshipMetric(rel,locale){if(rel?.relationshipNumber?.reducedValue!==null&&rel?.relationshipNumber?.reducedValue!==undefined)return `${text(locale,'Relationship','关系')} ${rel.relationshipNumber.reducedValue}`;const shared=arr(rel?.sharedPresentDigits);return shared.length?`${text(locale,'Shared','共同')} ${joinValues(shared).replace(/ · /g,'·')}`:''}
...
export function buildNumerologyRadialOverview({chartModel,integratedReading=null,locale='en'}={}){
 ...
 const sectors=[
  ...
  sector({id:'REALITY_REFLECTION',label:text(locale,'Reality reflection','现实对照'),metric:arr(integratedReading?.realityReflection).length?`${arr(integratedReading.realityReflection).length} ${text(locale,'questions','个问题')}`:'',targetSection:'REALITY_REFLECTION',priority:'P6',available:Boolean(arr(integratedReading?.realityReflection).length)})
 ].filter(Boolean);

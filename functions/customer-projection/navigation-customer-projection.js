...
export function projectNavigationForCustomer(navigation={}, {locale='en'}={}){
 ...
 return deepFreeze({schemaVersion:`${CX_PROJECTION_VERSION}:NAVIGATION`,locale:localeOf(locale),state:clean(source.state||source.status)||'NOT_ESTABLISHED',options,selectedId:clean(source.selectedId||source.selectedPathId)||null,governance:{selectionMadeBySystem:false,...boundary()}});
}

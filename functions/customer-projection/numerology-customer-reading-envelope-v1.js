...
const NAME_LABELS=Object.freeze({expression:['Expression','表达数'],soulUrge:['Soul Urge','灵魂驱动数'],personality:['Personality','人格数'],maturity:['Maturity','成熟数']});
...
function snapshotNode(item){return freeze({id:item.role,label:item.label,value:item.value,rawValue:item.rawValue??null,reductionSteps:arr(item.reductionSteps),masterNumberPreserved:item.masterNumberPreserved===true,tier:CORE_ROLES.has(item.role)?'CORE':'SECONDARY'})}
...
function buildNameLayer(expansion,locale){const n=expansion?.sections?.nameNumbers;if(!n?.customerPublishable||!n?.values)return null;const values=Object.entries(n.values).map(([code,item])=>freeze({code,label:label(locale,NAME_LABELS[code]?.[0]||code,NAME_LABELS[code]?.[1]||code),value:item?.reducedValue??null,rawValue:item?.rawValue??null,reductionSteps:arr(item?.reductionSteps)}));return freeze({enabled:true,values,customerConfirmed:n.input?.customerConfirmed===true,sourceAlignedMeaningRequired:true})}
...
function projectPublicIntegratedReading(ir,{expansionInput={},chartModel=null,locale='en'}={}){
 ...
 if(clean(expansionInput?.targetDate)&&!arr(chartModel?.timingBand?.western).length){
   narrative=narrative.filter(item=>!/No target date was supplied|未提供目标日期/.test(clean(item)));
...
export function buildNumerologyChartModel({integratedReading,locale='en'}={}){
 ...
 return freeze({schemaVersion:CHART_SCHEMA,methodId:'NUM',locale,overviewTiles:snapshot.filter(x=>CORE_ROLES.has(x.role)).map(snapshotNode),coreNumberMap:{nodes:snapshot.map(snapshotNode),relations:arr(integratedReading.sections?.relationships).map(relationshipEdge)},wholeChartPattern:buildPatternBoard(expansion),nameLayerMap:buildNameLayer(expansion,locale),secondaryChartMap:buildSecondaryChart(depth),cycleTimeline:buildCycleTimeline(integratedReading),timingBand:buildTimingBand(integratedReading),energyPatternMap:buildEnergyPatternMap(integratedReading),relationshipOverlay:buildRelationshipOverlay(integratedReading),priorityThemes:arr(integratedReading.sections?.standoutThemes).slice(0,5).map(x=>freeze({themeCode:x.themeCode,priority:x.priority,title:x.title,summary:x.summary,evidence:arr(x.evidence)})),boundaries:{chartCreatesMeaning:false,chartChangesCalculation:false,schoolMergeCreated:false,compatibilityScoreCreated:false,fortunePredictionCreated:false}})
}

...
export async function composeKirGroundedAnswer({understanding,evidencePack,modelDecision,allowedContext=null,provider=null,upstreamGroundedAnswer=null}){
  ...
  if(supported&&provider&&modelDecision.requestedModel!=='DETERMINISTIC'){
    const payload={question:understanding.question,evidencePack,allowedContext,answerContract:{directFirst:true,noNewPhiMeaning:true,noMethodHijack:true,noInternalJargonDump:true}};
    text=clean(await provider({model:modelDecision.requestedModel,payload})); providerInvoked=true;
  }
  if(!text&&supported){const lead=evidenceSentence(primary[0]); const second=primary[1]?evidenceSentence(primary[1]):''; text=understanding.locale==='zh-Hans'?`这个问题首先指向「${lead}」${second?`，同时与「${second}」有关。`: '。'} 这些判断只来自当前获准的 Book I–III / canonical evidence；如果你的问题包含个人情境，还需要把实际事实与限制另外带入，不能从知识材料直接推断你的现实。`:`The strongest grounded match is “${lead}”${second?`, with supporting relevance from “${second}”`:''}. This answer uses only admitted Book I–III / canonical evidence; personal facts must be supplied separately rather than inferred from knowledge.`;}
  ...
  return Object.freeze({schemaVersion:'PHI-OS-KIR-R2-GROUNDED-COMPOSER-v1.0.0',text,providerInvoked,model:modelDecision.requestedModel,knowledgeEvidencePackConsumed:true,upstreamGroundedAnswerPresent:Boolean(upstreamGroundedAnswer),upstreamGroundedAnswerConsumed:Boolean(upstreamGroundedAnswer),usedEvidenceIds:primary.map(x=>x.sourceId),allowedContextConsumed:Boolean(allowedContext),authority:{createsPhiMeaning:false,createsMethodMeaning:false,createsRealityTruth:false}});
}

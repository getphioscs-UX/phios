...
async function registry(env){if(!env?.ASSETS?.fetch)return null;const response=await env.ASSETS.fetch(new Request(`https://assets.local/${REGISTRY}`));return response.ok?response.json():null;}
...
export async function retrieveFormationScope({env={},scope,locale='zh-Hans'}={}){
 ...
 if(!selected||selected.bookCode!=='BOOK-1'||!selected.sourceRefs?.canonicalNodeCodes?.length||!selected.sourceRefs?.manuscriptSectionRefs?.length)return {sources:[],nodeCodes:[],chain:[{stage:'SELECTED_STRUCTURED_OBJECT',status:'UNAVAILABLE'}]};
 const relatedIds=data.details?.[selected.objectId]?.relatedMechanisms||[];
 const objects=[selected,...relatedIds.map(id=>data.objects.find(o=>o.objectId===id)).filter(Boolean)].slice(0,6);

...
import {createTarotSelectionRuntime,TAROT_DECK_ID,TAROT_DECK_VERSION,TAROT_ORIENTATION_POLICY_ID,TAROT_ORIENTATION_POLICY_VERSION} from '../core-method-runtime/tarot-selection-runtime-v2.js';
import {createTarotRuntime,TAROT_RUNTIME_CODE} from '../core-method-runtime/tarot-runtime-v2.js';
import {createTarotCardProjector} from '../core-method-runtime/tarot-card-projection-mapper-v2.js';
import {createTarotReadingIR} from '../interpretation-runtime/tarot-reading-ir-v3.js';
import {createTarotProductPublicViewModel} from '../symbolic-method-public-ux/tarot-product-view-model-v2.js';
import {composeTarotSpread} from '../interpretation-runtime/tarot-spread-composition-runtime-v1.js';

export const TAROT_PRODUCTION_RUNTIME_VERSION='1.1.0';
...
function normalizeSpread(value,spreadRegistry){const raw=clean(value||'THREE_CARD_SITUATION').toUpperCase();const entries=spreadRegistry?.entries||[];const direct=entries.find(x=>x.spreadId===raw);if(direct)return direct;const aliased=entries.find(x=>(x.legacyAliases||[]).includes(raw));if(aliased)return aliased;throw new TypeError(`TAROT_GOVERNED_SPREAD_REQUIRED:${raw}`);}
...
export async function executeTarotProductRuntime(request={},allAuthorities={}){
  ...
  const {structural,interpretation}=authorities(allAuthorities);const spread=normalizeSpread(request.spread||request.spreadId,structural.spreadRegistry);const spreadId=spread.spreadId;
  ...
  const common={inputMode,sessionId,timestamp,deckId:TAROT_DECK_ID,deckVersion:TAROT_DECK_VERSION,orientationPolicyId:TAROT_ORIENTATION_POLICY_ID,orientationPolicyVersion:TAROT_ORIENTATION_POLICY_VERSION,spreadId,spreadVersion:spread.spreadVersion,projectionVersion:'1.0.0'};
  ...
  publicView.spreadComposition=composeTarotSpread({readingIr,authorities:interpretation});
  publicView.production={...publicView.production,state:'FULL_PRODUCTION',runAllowed:true,productionCapabilityPromoted:true};
  return Object.freeze({
    ...
    production:Object.freeze({state:'FULL_PRODUCTION',runAllowed:true,limitedProductionActivated:false,fullProduction:true,providerUsed:false,aiCardSelection:false,aiInterpretation:false,spreadCompositionRuntimeVersion:'1.0.0'}),

...
function assertRegistryBoundary(admissionRegistry, mappingRegistry, activationRegistry, mode, methodCode, publicMethodCode) {
  if (!admissionRegistry || !mappingRegistry) fail('CMP_REGISTRY_MISSING');
  if (mode === 'production') {
    if (mappingRegistry.productionActivated !== true || !activationRegistry?.runtimeOperational) fail('CMP_PRODUCTION_NOT_ACTIVATED');
    const method = (activationRegistry.methods || []).find(item => item.methodCode === methodCode && item.publicMethodCode === publicMethodCode);
    if (!method || method.productionActivated !== true || method.acceptancePassed !== true) fail('CMP_METHOD_PRODUCTION_NOT_ACTIVATED', methodCode);
  } else if (mode !== 'foundation_validation') {
    fail('CMP_EXECUTION_MODE_INVALID', mode);
  }
}

export async function resolveCanonicalMeaningItems({ projection, admissionRegistry, mappingRegistry, activationRegistry = null, mode = 'foundation_validation' }) {
  const { methodCode, publicMethodCode } = assertCanonicalMethodProjectionBoundary(projection);
  assertRegistryBoundary(admissionRegistry, mappingRegistry, activationRegistry, mode, methodCode, publicMethodCode);

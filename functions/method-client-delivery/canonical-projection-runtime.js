...
import {executeMcd4Request} from './execution-runtime.js';
...
export async function buildCanonicalMethodProjection(request,result,{executedAt=new Date().toISOString()}={}){
  ...
  const unknowns=deriveUnknowns(result); const calc=request.methodCode==='NUMEROLOGY'?mapNumCalculation(result):emptyCalculation(result); const status=projectionStatus(result);
...
export async function executeAndProjectMcd5Request(request){
  const startedAt=new Date().toISOString();
  const execution=await executeMcd4Request(request);
  if(execution.executionStatus==='BLOCKED_BY_MPA')return Object.freeze({execution,canonicalProjection:null,executedAt:startedAt});
  const canonicalProjection=await buildCanonicalMethodProjection(request,execution,{executedAt:new Date().toISOString()});

...
export async function onRequestPost(context){
  ...
  let targetActivationReference=null;
  if(body?.hdrTargetContext){
    try{targetActivationReference=await buildHdrTargetActivationReference({targetContext:body.hdrTargetContext,confirmedProfile:body.confirmedExternalProfile})}
    catch(error){targetActivationReference=freeze({state:'UNAVAILABLE',reasonCode:error?.code||error?.message||'HDR_TARGET_REFERENCE_UNAVAILABLE',boundary:{confirmedChartChanged:false,interpretationCreated:false,persisted:false}})}
  }
  const notRun=(state,extra={})=>json({ok:true,shadowValidation:freeze({state,...extra,targetActivationReference,boundary:{internalValidationOnly:true,referenceOnly:true,customerProfileOverwritten:false,persisted:false}})});
  const birthCrossCheckRequested=body?.birthCrossCheckRequested===true;
  if(!birthCrossCheckRequested)return notRun(targetActivationReference?.state==='AVAILABLE'?'TARGET_REFERENCE_ONLY':'NOT_RUN_NOT_REQUESTED');
  ...
  try{
    ...
    return json({ok:true,shadowValidation:freeze({...result.comparison,targetActivationReference,governance:result.governance,privacy:{saved:false,runtimeMemoryWritten:false}})});

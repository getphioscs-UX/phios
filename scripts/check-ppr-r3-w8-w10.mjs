...
const freeze=j(`${base}/authority/ppr-r3-w10-successor-freeze-v1.json`);assert.equal(freeze.status,'FROZEN_PPR_R3_SPECIALIST_HOST');const ecrMandalaSuccessor=j('content/embodied-configuration/ecr-customer-mandala-authority-audit-v1.json');const {r4,postR4Proof,w10aProof}=assertPprC1CurrentSuccessor();
...
for(const [p,d] of Object.entries(freeze.protectedConvergenceFiles)){
 ...
 const r4Proof=r4?.sharedFileSuccessorProof?.[p];const successor=ecrMandalaSuccessor?.protectedSuccessors?.[p];
 if(r4Proof){assert(successor,`W10 missing predecessor successor chain before PPR-R4 ${p}`);assert.equal(successor.predecessorSha256,d,`W10 predecessor-chain mismatch ${p}`);assert.equal(successor.successorSha256,r4Proof.predecessorSha256,`W10 PPR-R4 predecessor mismatch ${p}`);assert.equal(r4Proof.changeClass,'PPR_R4_METHOD_INPUT_EXTENSION_ONLY',`W10 PPR-R4 successor class not admitted ${p}`);if(current!==r4Proof.successorSha256){const later=postR4Proof(p);assert(later,`W10 PPR-R4 current successor proof missing ${p}`);assert.equal(later.predecessorSha256,r4Proof.successorSha256,`W10 post-R4 predecessor mismatch ${p}`);assert.equal(later.successorSha256,current,`W10 post-R4 successor digest drift ${p}`);}else assert.equal(r4Proof.successorSha256,current);continue;}
 const astShared=w10aProof(p);if(astShared){assert.equal(astShared.predecessorSha256,d,`W10 W10A predecessor mismatch ${p}`);assert.equal(astShared.successorSha256,current,`W10 W10A successor digest drift ${p}`);assert.match(astShared.changeClass,/^AST_TARGET_CONTEXT_/);continue;}
 assert(successor,`W10 protected drift without governed successor ${p}`);assert.equal(successor.predecessorSha256,d,`W10 protected successor predecessor mismatch ${p}`);assert.equal(successor.successorSha256,current,`W10 protected successor digest drift ${p}`);const admittedClass=p==='perspectives/personal/index.html'?'ECR_BRAND_ASSET_CORRECTION_ONLY':p==='assets/customer-ui/js/surfaces/personal-reality.js'?'BASELINE_RETIRED_RENDERER_DANGLING_IMPORT_REMOVAL_ONLY':null;assert.equal(successor.changeClass,admittedClass,`W10 protected successor class not admitted ${p}`);
}

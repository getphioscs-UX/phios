...
const freeze=j(`${base}/authority/ppr-r3-w10-successor-freeze-v1.json`);assert.equal(freeze.status,'FROZEN_PPR_R3_SPECIALIST_HOST');const ecrMandalaSuccessor=j('content/embodied-configuration/ecr-customer-mandala-authority-audit-v1.json');const {r4}=assertPprC1CurrentSuccessor();
...
for(const [p,d] of Object.entries(freeze.protectedConvergenceFiles)){
 ...
 if(r4Proof){assert(successor,`W10 missing predecessor successor chain before PPR-R4 ${p}`);assert.equal(successor.predecessorSha256,d,`W10 predecessor-chain mismatch ${p}`);assert.equal(successor.successorSha256,r4Proof.predecessorSha256,`W10 PPR-R4 predecessor mismatch ${p}`);assert.equal(r4Proof.successorSha256,current,`W10 PPR-R4 successor digest drift ${p}`);assert.equal(r4Proof.changeClass,'PPR_R4_METHOD_INPUT_EXTENSION_ONLY',`W10 PPR-R4 successor class not admitted ${p}`);continue;}

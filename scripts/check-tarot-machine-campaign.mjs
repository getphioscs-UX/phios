...
assert.ok(execute.includes('SYMBOLIC_LIMITED_PRODUCTION_NOT_ACTIVATED')); assert.ok(execute.includes('runAllowed:false')); assert.ok(context.includes("runAllowed:false"));
...
console.log('✓ TPA-W38 Aggregate Machine Gate passed: Tarot machine acceptance is COMPLETE while human review, live browser, production SHA, PCM promotion and public runAllowed remain closed.');
console.log('  Current baseline does not contain the earlier TPA-H persistence acceptance artifacts; activation must restore/reconcile that persistence gate before promotion.');

...
assert.equal(pkg.scripts['check:mpa-w23'],'node scripts/check-mpa-w23-bzr-activation.mjs');
assert.equal(pkg.scripts['check:mpa-bzr-activation'],'npm run check:mpa-w23');
...
assert.deepEqual(segments.slice(0,7),[
  ...
  'npm run check:mpa-projection-integration','npm run check:mpa-num-activation','npm run check:mpa-ast-activation','npm run check:mpa-bzr-activation'
]);

...
assert.match(h04Html, /data-hpc2-consumer-state="MISSING_PENDING_CKA"/);
assert.match(h04Html, /data-hpc2-planned-route="\/knowledge-search"/);
assert.match(h04Html, /data-hpc2-route-state="HOMEPAGE_ENTRY_BLOCKED_PENDING_CKA"/);
assert.match(h04Html, /data-hpc2-planned-route="\/about\/system"/);
assert.doesNotMatch(h04Html, /\bhref=/i);
...
const en = (await import(`${pathToFileURL(`${process.cwd()}/${paths.localeEn}`).href}?hpc2w5`)).default;
const zh = (await import(`${pathToFileURL(`${process.cwd()}/${paths.localeZh}`).href}?hpc2w5`)).default;
...
assert.equal(/href=["']\/knowledge-search\/?["']/.test(h04Html), false);
...
console.log('HPC2-W5 PHI OS Runtime Composition: ACCEPTED (repository implementation)');
...
console.log('  Ask PHI OS: position reserved; functional elements, route hrefs and API calls = 0 pending CKA');

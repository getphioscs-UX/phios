...
// The historical W20 local dashboard presentation was legitimately retired by CX-P1 after browser acceptance.
// Preserve W20/JR runtime assertions, but validate the current route successor instead of restoring the deleted HTML.
const p1Delete = readJson('content/customer-experience-rebuild/acceptance/p1-physical-legacy-delete-acceptance-v1.json');
const p1Cutover = readJson('content/customer-experience-rebuild/migration/priority-route-cutover-registry-v3.json');
...
assert.ok(p1Delete.deletedPresentationFiles.includes('reality-dashboard.html'));
const currentMyReality = p1Cutover.surfaces.find(surface => surface.surfaceId === 'MY_REALITY');
assert.ok(currentMyReality);
assert.equal(currentMyReality.canonicalPath, '/reality/');
assert.equal(currentMyReality.htmlPath, 'reality/index.html');
assert.ok(currentMyReality.legacyRoutes.includes('/reality-dashboard'));
assert.ok(currentMyReality.legacyRoutes.includes('/reality-dashboard.html'));
assert.equal(exists('reality-dashboard.html'), false);
assert.equal(exists(currentMyReality.htmlPath), true);
const currentMyRealityHtml = readText(currentMyReality.htmlPath);
assert.ok(currentMyRealityHtml.includes('data-cx-surface="MY_REALITY"'));
const redirects = readText('_redirects');
assert.ok(redirects.includes('/reality-dashboard /reality/ 308'));
assert.ok(redirects.includes('/reality-dashboard.html /reality/ 308'));

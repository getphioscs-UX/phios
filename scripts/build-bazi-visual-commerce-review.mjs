...
function atomicWrite(file,content){
 ...
 try{fs.renameSync(tmp,file);}catch(error){
  try{if(fs.existsSync(file))fs.rmSync(file,{force:true});fs.renameSync(tmp,file);}
  catch{try{if(fs.existsSync(tmp))fs.rmSync(tmp,{force:true});}catch{}throw error;}
 }
}
...
for(const locale of ['en','zh-Hans'])for(const mode of ['free','locked','paid']){
 ...
 atomicWrite(`${folder}/review.html`,html(mode,locale));
 if(full)atomicWrite(`${root}/bazi-${locale}.json`,JSON.stringify(report,null,2)+'\n');
}
for(const mode of ['free','locked','paid']){fs.mkdirSync(`${root}/${mode}`,{recursive:true});atomicWrite(`${root}/${mode}/review.html`,html(mode,'en'));}
atomicWrite(`${root}/review.html`,html('paid',null));

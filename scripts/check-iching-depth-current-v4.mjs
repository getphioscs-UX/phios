...
const predecessor=read('content/interpretation/iching/reconciliation/iching-depth-current-successor-v3.json');
const current=read('content/interpretation/iching/reconciliation/iching-depth-current-successor-v4.json');
assert.equal(current.successorOf,'content/interpretation/iching/reconciliation/iching-depth-current-successor-v3.json');assert.equal(current.historicalPredecessorMutated,false);assert.equal(current.baselineCommit,'252cc66eb6902b2752a001e53087dee40ef635cc');
assert.equal(predecessor.admittedCoverage.humanApproved,'448/448');assert.equal(current.admittedCoverage.humanApproved,'448/448');assert.equal(current.productionBoundary.publicRunAllowed,false);
...
console.log('✓ ICHI-DEPTH current v4 successor passed: 448/448 HUMAN_APPROVED depth is unchanged while I Ching persistence consumes shared context by semantic contract.');
console.log('  Tarot Phase M shared-context evolution is preserved; v3 remains historical and public execution authority is not granted by this successor.');

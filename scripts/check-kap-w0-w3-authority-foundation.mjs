...
assert.equal(pkg.scripts['check:kap-foundation'],'node scripts/check-kap-w0-w3-authority-foundation.mjs'); const kapSegments=String(pkg.scripts['check:kap']||'').split(' && '); assert.ok(kapSegments.includes('npm run check:kap-foundation')); const foundationIndex=kapSegments.indexOf('npm run check:kap-foundation'); const groundingIndex=kapSegments.indexOf('npm run check:kap-grounding'); if(groundingIndex>=0) assert.ok(foundationIndex<groundingIndex);

...
if(fs.existsSync(p1DeletePath)&&JSON.parse(read(p1DeletePath)).status==='PHYSICAL_LEGACY_PRESENTATION_DELETE_COMPLETE'){const current=read('knowledge/ask/index.html');assert.ok(current.includes('data-cx-answer-limits'));assert.ok(current.includes('data-cx-basis-groups'));assert.equal(fs.existsSync('knowledge-search.html'),false);console.log('✓ KAP-W17 Unknown boundary current successor passed after P1 physical delete: Contextual Ask exposes limits while legacy Knowledge Search presentation is absent.');process.exit(0);}

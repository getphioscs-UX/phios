...
const digest=async p=>crypto.createHash('sha256').update((await fs.readFile(path.join(root,p),'utf8')).replace(/^\uFEFF/,'').replace(/\r\n?/g,'\n'),'utf8').digest('hex');
...
const r5Active=await exists(r5FreezePath);
if(r5Active){
 ...
 assert.equal(nodeCodes.size,718,'KAU-R5 successor must expose exactly 718 Canonical Nodes.');
 assert.equal(await digest('content/knowledge/registry/nodes.json'),r5.canonicalAuthority.successorSha256);
}else assert.equal(nodeCodes.size,716,'KAU must preserve the 716-node predecessor Canonical Authority before an accepted successor.');
...
console.log('✓ KAU-W9 Node Coverage Analysis passed: all 716 frozen predecessor Nodes remain assessed without authority mutation.');
...
if(r5Active) console.log('  The validation-only KAU v1 freeze is preserved inside the exact 718-node KAU-R5 Canonical successor.');

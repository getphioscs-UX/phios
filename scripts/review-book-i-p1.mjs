...
function takeValue(args, index, flag) {
  ...
  if (!value || value.startsWith('--')) throw coded('P1_REVIEW_ARGUMENT_VALUE_REQUIRED', { flag });
  return { value, consumed: 1 };
}

export function parseP1ReviewArgs(command, args) {
  if (command === 'review') {
    let mode = 'dry-run';
    ...
    for (let index = 0; index < args.length; index += 1) {
      ...
      if (argument === '--dry-run' || argument === '--approve') {
        if (explicitMode && explicitMode !== argument) throw coded('P1_REVIEW_MODE_CONFLICT');
      ...
      if (!flag) throw coded('P1_REVIEW_ARGUMENT_UNKNOWN', { argument });
    ...
    if (mode === 'dry-run' && (expectedSha256 || reviewerRole || confirmations.length)) {
      throw coded('P1_REVIEW_APPROVAL_ARGUMENTS_REQUIRE_APPROVE');
    }
  ...
  if (command === 'upload') {
    ...
    for (const argument of args) {
      if (!['--dry-run', '--apply'].includes(argument)) {
        throw coded('P1_UPLOAD_ARGUMENT_UNKNOWN', { argument });
      }
      if (explicitMode && explicitMode !== argument) throw coded('P1_UPLOAD_MODE_CONFLICT');
      explicitMode = argument;
      mode = argument === '--apply' ? 'apply' : 'dry-run';
    }
    return { command, mode };
  }
  throw coded('P1_REVIEW_COMMAND_REQUIRED', { allowed: ['review', 'upload'] });
}

export async function runP1ReviewCommand(command, args = [], options = {}) {
  const parsed = parseP1ReviewArgs(command, args);
  if (command === 'review') {
    return reviewP1Candidate({
...
async function main() {
  ...
  try {
    console.log(JSON.stringify(await runP1ReviewCommand(command, args), null, 2));
  } catch (error) {
    console.error(JSON.stringify({
      ...
      status: 'blocked',
      code: error.code || 'P1_HUMAN_REVIEW_FAILED',

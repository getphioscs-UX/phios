...
const report={observedAt:http.observedAt,bucket:'phios-public-assets',totalObjects:read(dir+'r2-bucket-inventory-2026-09-19.json').totalObjects,inventoryComplete:!http.latestIncrementalCheck,latestIncrementalCheck:http.latestIncrementalCheck||null,total:rows.length,httpImagePass:http.httpImagePass,counts,missingAtlasObjects:missing,missingSampleFigures,productionDeployment:'NOT_PERFORMED',allRuntimeUsageVerified:false,rows};
...
fs.writeFileSync(dir+'R2-ALL-VISUAL-ASSETS-REVIEW.html',`<!doctype html><html lang="zh-Hans"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>R2 全量视觉素材审核</title><style>*{box-sizing:border-box}body{overflow-wrap:anywhere;margin:0;background:#081b28;color:#eee9db;font:16px/1.65 system-ui}main{max-width:1320px;margin:auto;padding:24px}h1,h2{color:#e7ce99}a{color:#a4dbeb}input,select,button{font:inherit;padding:9px;max-width:100%;border:1px solid #68808c;border-radius:6px}button{cursor:pointer}.controls{display:flex;gap:12px;flex-wrap:wrap;align-items:end}.controls label{display:grid;gap:5px}.grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:18px;margin:24px 0}article{background:#112b3b;border:1px solid #3b5360;padding:16px;border-radius:12px;overflow-wrap:anywhere}img{width:100%;height:220px;object-fit:contain;background:#fff}code{font-size:12px}summary{cursor:pointer}p{margin:.65rem 0}.notice{border-left:3px solid #d6b46d;padding:12px 20px;background:#102a3b}:focus-visible{outline:3px solid #e7ce99;outline-offset:3px}@media(max-width:750px){.grid{grid-template-columns:1fr}main{padding:16px}}</style></head><body><main><h1>R2 全量视觉素材审核</h1><p id="totals"></p><div class="notice"><p>每张图片都有用途记录。已绑定、仅本地审阅、仅登记、旧版留档和待安排会明确区分；不会把图片存在或源码引用计成线上已展示。</p><p>当前结果属于本地工作树，尚未部署。每页最多加载 12 张图。桶列表是原审计快照；图片清单包含后续 4E 增量验证。</p><p id="missing"></p></div><p><a href="../../../tools/review/BOOK-V-CIV-ATLAS-R1-M1-FINAL-HUMAN-REVIEW.html">第五册集中人工审核</a> · <a href="../../../docs/qa/commerce-stripe-r1/UNIFIED-HUMAN-REVIEW.html">统一人工审核</a></p><div class="controls"><label>搜索<input id="query" type="search" placeholder="玛雅 / HERO / 2026 / book-6"></label><label>用途状态<select id="status"><option value="">全部</option></select></label><label>文件分类<select id="folder"><option value="">全部</option></select></label><button id="prev">上一页</button><button id="next">下一页</button></div><p id="count" role="status"></p><div id="grid" class="grid"></div></main><script id="data" type="application/json">${data}</script><script>
const report=JSON.parse(document.getElementById('data').textContent),labels={BOUND:'已绑定',LOCAL_REVIEW:'仅本地审阅',REGISTERED:'已登记（原260项见终态清单）',ARCHIVE:'旧版留档',NEEDS_PLACEMENT:'待安排展示'};let page=0;const $=id=>document.getElementById(id);const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
$('totals').textContent='bucket 列表快照：'+report.totalObjects+' 个对象 · '+report.total+' 张图片 · HTTP 检查 '+report.httpImagePass+'/'+report.total+' · '+Object.entries(report.counts).map(([s,n])=>labels[s]+' '+n).join(' / ');
$('missing').textContent=(report.missingAtlasObjects.length?'Atlas 待补素材：'+report.missingAtlasObjects.join('；'):'Atlas 图片已齐全，229 张已按所有者授权通过，2026 重构图已接入')+(report.missingSampleFigures.length?'。第一册仍缺 '+report.missingSampleFigures.map(f=>f.figure).join('、'):'。第一册 12 张总结图（含 4E）已全部恢复；本册 407 页。')+'。';

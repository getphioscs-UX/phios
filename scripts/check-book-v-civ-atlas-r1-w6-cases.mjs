...
const registry=json('content/civilization-atlas/cases/civilization-case-registry-v1.json'); assert.equal(registry.status,'ACTIVE'); assert.ok(registry.cases.length>=12);
const ids=new Set();
...
for(const id of required) assert.ok(ids.has(id),`missing original W6 vertical-slice case ${id}`);
assert.equal(registry.cases.find(x=>x.caseId==='CA-T07-05')?.title?.en,'Gupta and Post-Gupta South Asia');
assert.equal(registry.cases.find(x=>x.caseId==='CA-T08-01')?.title?.en,'Tang Imperial Runtime');
...
console.log(`  Original 12 representative cases remain present inside the successor registry (${registry.cases.length} cases total).`);

...
for(const [id,url] of Object.entries(assets.bindings)){assert(fs.existsSync('.'+url),id);assert(url.endsWith('.svg')||url.endsWith('.webp'));if(url.endsWith('.svg')){const svg=fs.readFileSync('.'+url,'utf8');assert(!/<(?:text|foreignObject|script)\b|\bon\w+\s*=/.test(svg));for(const match of svg.matchAll(/(?:xlink:)?href="([^"]+)"/g))assert(/^data:image\/(png|webp);base64,|^#/.test(match[1]));}}

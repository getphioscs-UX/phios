...
fs.writeFileSync(`${root}/contract-evidence.json`,JSON.stringify({machinePass:true,totalPages:reports.map(r=>({locale:r.locale,total:r.totalPages})),semanticPaginationParity:true,physicalPaginationMayDifferByLocale:true,shorterFixturePages:shorter.pages.length+6,sectionLevelProviderRequests:requests.length,canonicalProviderInvocation:false,budgets,humanAccepted:false},null,2)+'\n');

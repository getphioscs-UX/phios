...
for(const locale of ['en','zh-Hans']){
 ...
 const run=async(ids,focus,override={},currentRealityRef=null)=>{
  ...
  const caseId=`CROSS-HP-${String(++index).padStart(3,'0')}-${locale}`;write(`${docs}/cases/${caseId}.json`,{caseId,locale,synthetic:true,focus,...result});rows.push({caseId,locale,focus,methodIds:ids,status:'PASS',path:`cases/${caseId}.json`,unmappedCount:result.unmappedClaims.length,crossClaimCount:result.reading.claims.length,supportTypes:[...new Set(result.reading.claims.map(c=>c.supportType))]});return result;
 };
 ...
 const quick=await adaptProfileCrossEnvelope({...profiles[0],locale}),promoted=structuredClone(quick.claimCollection);promoted.claims[0].claimType='CORE_PATTERN';await assert.rejects(()=>buildCrossPerspectiveInputIR({acceptedMethodReadingEnvelopes:[pairs.AST.envelope,quick.envelope],claimCollections:[pairs.AST.claimCollection,promoted]},{successorReview:true}),/EVIDENCE_WEIGHT_MISMATCH/);

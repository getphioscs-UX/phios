...
for(let i=0;i<260;i++){
 const row=rows[i],result=report.rows[i];assert.equal(Number(row[0]),i+1);assert.equal(rowIdentity(row),frozen.rows[i].immutableHash,'Frozen identity '+(i+1));assert.equal(result.objectKey,row[2]);assert.equal(result.status,row[6]);assert.ok(allowed.has(result.status),result.status);assert.ok(result.reason&&result.traceFile);
 if(result.status.includes('DISPLAYED')||result.status==='BROWSER_ICON_ACTIVE'){
  const variants=new Set();for(const e of result.observations){assert.equal(e.objectKey,row[2]);assert.ok(!/\/docs\/|review|audit/i.test(e.route),'Gallery cannot prove customer display');assert.ok(e.resolvedUrl);assert.equal(decodeURIComponent(new URL(e.resolvedUrl).pathname).slice(1),row[2]);

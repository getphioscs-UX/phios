...
function pageNumber(value, flag) {
  if (!/^[1-9][0-9]*$/u.test(String(value || ''))) {
    throw coded('P0_PAGE_OVERRIDE_INVALID', { flag });
  }
  return Number(value);
}
...
export function parseExtractionArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === '--dry-run' || argument === '--apply') {
      if (explicitMode && explicitMode !== argument) throw coded('P0_EXTRACTION_MODE_CONFLICT');
    ...
    if (flag === '--start-page' || flag === '--end-page') {
      ...
      if (overrides[key] !== undefined) throw coded('P0_PAGE_OVERRIDE_DUPLICATE', { flag });
      overrides[key] = pageNumber(value, flag);
      continue;
    }
    throw coded('P0_EXTRACTION_ARGUMENT_UNKNOWN', { argument });
  }
  if (overrides.startPage && overrides.endPage && overrides.endPage < overrides.startPage) {
    throw coded('P0_PAGE_OVERRIDE_ORDER_INVALID');
  }
...
async function main() {
  try {
    ...
    console.error(JSON.stringify({
      ...
      command: 'extract-p0',
      status: 'blocked',
      code: error.code || 'P0_EXTRACTION_FAILED',

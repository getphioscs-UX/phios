...
const p1Delete = json('content/customer-experience-rebuild/acceptance/p1-physical-legacy-delete-acceptance-v1.json');
const p1Cutover = json('content/customer-experience-rebuild/migration/priority-route-cutover-registry-v3.json');
...
for (const key of ['backendAuthorityDeleted','runtimeAuthorityDeleted','methodAuthorityDeleted','knowledgeAuthorityDeleted','financialAuthorityDeleted','realityAuthorityDeleted','reportAuthorityDeleted']) assert.equal(p1Delete[key], false, key);
...
for (const relativePath of px2Pages) {
  if (retiredPresentation.has(relativePath)) {
    assert.equal(exists(relativePath), false, `P1 retired presentation still exists: ${relativePath}`);
    continue;
  }
...
const cutover = p1Cutover;
assert.equal(cx.status, 'ACTIVE_CX_P1_PUBLIC_IA_SUCCESSOR');
assert.equal(cutover.status, 'P1_ROUTE_CUTOVER_BROWSER_ACCEPTED_PHYSICAL_LEGACY_DELETE_COMPLETE');
...
for (const surface of cutover.surfaces) {
  ...
  for (const marker of [
    `data-cx-surface="${surface.dataCxSurface}"`,
    '<link rel="canonical"',
...
for (const href of ['/reality/', '/perspectives/', '/professional/']) {
  assert.ok(cxNavigation.includes(`href: '${href}'`) || cxNavigation.includes(`href:'${href}'`), `CX navigation route ${href}`);
}
assert.ok(cxNavigation.includes("id: 'ASK'"));
assert.ok(cxNavigation.includes("mode: 'dialog'"));
const cxShell = read(cx.currentAuthority.shellRuntime);
assert.ok(cxShell.includes('action="/knowledge/ask/"'));
assert.ok(cxShell.includes('href="/knowledge/ask/"'));
...
assert.equal(exists('knowledge-search.html'), false);
assert.ok(p1Delete.deletedPresentationFiles.includes('knowledge-search.html'));
assert.match(read('knowledge/ask/index.html'), /data-cx-contextual-ask-form/);
...
assert.equal(exists('ask.html'), false);
assert.ok(p1Delete.deletedPresentationFiles.includes('ask.html'));
assert.match(read('knowledge/ask/index.html'), /data-cx-contextual-ask-form/);
...
assert.match(reality, /data-cx-start-reality/);
...
console.log('✓ PX2 → Stage16 → CX-P1 physical-delete current Public Experience passed.');
console.log('  Non-cutover surfaces retain strict PX2 composition; four governed P1 surfaces consume the CX shell while seven retired presentations remain physically absent.');

...
assert.equal(pkg.scripts['check:mpa-w25'], 'node scripts/check-mpa-w25-future-method-holding-registry.mjs');
assert.equal(pkg.scripts['check:mpa-future-holding'], 'npm run check:mpa-w25');
...
assert.deepEqual(segments.slice(0, foundation.length), foundation);
const holdingIndex = segments.indexOf('npm run check:mpa-future-holding');
assert.ok(holdingIndex >= foundation.length, 'MPA-W25 must be registered after the existing MPA chain.');
assert.equal(segments.filter(item => item === 'npm run check:mpa-future-holding').length, 1);

...
export const LIVE_SHA_EVIDENCE='.runtime-evidence/tarot-production-sha-alignment-v2.json';
...
export async function d1Query({accountId,databaseId,headers,sql,params=[]}){
  ...
  const response=await timedFetch(url,{method:'POST',headers:{...headers,'content-type':'application/json',accept:'application/json','user-agent':'PHI-OS-TPA-M/1.0.0'},body:JSON.stringify({sql,params})});
...
export async function getJson(url,options={}){const response=await timedFetch(url,{...options,headers:{accept:'application/json','user-agent':'PHI-OS-TPA-M/1.0.0',...(options.headers||{})}});let payload=null;try{payload=await response.json();}catch{}return {status:response.status,payload,url:response.url};}

...
export function buildProgressionReadiness(root,batchCode='BATCH-003'){
 ...
 const entries=planReview.entries.map(e=>{
  ...
  if(!c2Frozen)blockers.push('TL_C2_FREEZE_REQUIRED');
  const productionReady=blockers.length===0;
  return {nodeCode:e.nodeCode,state:productionReady?'PRODUCTION_READY':'BLOCKED',blockers,mappingReady,c2Frozen,publicationReady:productionReady};
 });
 const ready=entries.filter(x=>x.state==='PRODUCTION_READY').length;
 return {schemaVersion:'PHI-OS-BFA-PROG-BATCH-C3-READINESS-v1.0.0',work:'BFA-PROG-6',batchCode,status:ready===entries.length?'PRODUCTION_READY':'BLOCKED_AWAITING_TL_C2_REVIEW',summary:{nodeCount:entries.length,productionReadyCount:ready,blockedCount:entries.length-ready},entries,authorityBoundary:{c3IsDerivedReadinessOnly:true,c3DoesNotCreateHumanDecision:true,oldWave1PlanWaveChoreographyRequired:false,bfaFinalApprovalCreated:false}};
}
...
export function buildCompatibleBatchPlan(root,batchCode='BATCH-003'){
 ...
 if(readiness.summary.productionReadyCount!==scheduled.plannedNodeCount)return {plan:null,readiness};
 const order=read(root,ORDER); const map=new Map(order.records.map(x=>[x.nodeCode,x]));
 const entries=scheduled.nodeCodes.map((n,i)=>{const o=map.get(n);return {batchIndex:i+1,nodeCode:n,bookCode:o.bookCode,partCode:o.partCode,locale:'zh-Hans',title:o.titleZhHans,readinessState:'ARTICLE_READY',readinessBlockers:[],readinessAuthorityEvidence:{progressionVersion:'BFA_PROG_V2',c2HumanDecision:BATCH3_DECISIONS,mappingAuthority:evidenceForMapping(root,n)}}});
 const base={schemaVersion:'PHI-OS-APS-3-BFA-PROGRESSION-v2.0.0',work:'APS-3',status:'READY_FOR_APS_4_CANDIDATE_ORCHESTRATION',implementationBaselineCommit:'51ff68497f70df2bfe0a7869ed84646872bddb70',contractReference:`${PROG_ROOT}/contracts/bfa-prog-0-corpus-reconciliation-v1.json`,batchCode,createdAt:new Date().toISOString(),request:{bookCode:scheduled.bookCode,locale:'zh-Hans',requestedCount:scheduled.maximumNewArticles,requestedCountMeaning:'fixed_progression_window_maximum'},sourceReadiness:{work:'BFA-PROG-6',readyCount:readiness.summary.productionReadyCount},selection:{availableReadyCount:scheduled.plannedNodeCount,selectedCount:scheduled.plannedNodeCount,shortfallCount:0,unselectedReadyCount:0,downstreamPjaWaveMaximum:24,chunks:[{chunkCode:'CHUNK-001',maximum:24,nodeCodes:scheduled.nodeCodes}]},entries,governance:{nonAuthoritativeOrchestrationPlan:true,canonicalOrderLocked:true,readyNodeLeapfrogForbidden:true,humanDecisionAuthorityCreated:false,candidateCreated:false,publicationCreated:false},nextWork:'APS-4_CANDIDATE_ORCHESTRATION'};

...
function candidatePath(locale,nodeCode,src=null){return src?.compositionUnit?.candidatePaths?.[locale]??`content/knowledge/production/candidates/${locale}/${nodeCode}/candidate.v1.json`;}
function briefPath(nodeCode,locale='zh-Hans',src=null){return src?.compositionUnit?.briefPaths?.[locale]??(locale==='en'?`content/knowledge/production/briefs/en/${nodeCode}-production-brief.en.v1.json`:`content/knowledge/production/briefs/zh-Hans/${nodeCode}-production-brief.v2.json`);}
...
export async function buildBfaBatchFromAps(root,batchCode,{write=true}={}){
 ...
 for(const src of orchestration.entries??[]){
  const nodeCode=src.nodeCode;const zbp=briefPath(nodeCode,'zh-Hans',src),ebp=briefPath(nodeCode,'en',src);
  ...
  const locales={};for(const locale of ['zh-Hans','en']){const cp=candidatePath(locale,nodeCode,src);if(!exists(path.join(root,cp))){blockers.push(`${locale}:CANDIDATE_MISSING`);continue;}const c=read(root,cp);const pr=projections[locale];if(!pr){continue;}locales[locale]={nodeCode:c.nodeCode,locale,candidateCode:c.candidateCode,candidateDigest:c.candidateDigest,article:c.article,sourceBrief:c.sourceBrief,promptDigest:c.promptDigest??c.provenance?.promptDigest??prompts[locale]?.promptDigest,candidatePath:cp};}
  ...
  const localeIdentityBase={nodeCode,identityModel:'BFA_PACKAGE_SCOPED_BILINGUAL_IDENTITY_CANDIDATE',identities:{'zh-Hans':{slug,displayQuestion:zhBrief.localizedIdentity?.displayQuestion??zhBrief.canonicalMeaning?.canonicalTitle,sourceAuthority:src.compositionUnit?'BFA_ARTICLE_COMPOSITION_UNIT_IDENTITY_CANDIDATE_PENDING_TL_FINAL_APPROVAL':'KNR_L10N_CANONICAL'},en:{slug,displayQuestion:enBrief?.localizedIdentity?.displayQuestion??locales.en?.article?.title??null,sourceAuthority:src.compositionUnit?'BFA_ARTICLE_COMPOSITION_UNIT_IDENTITY_CANDIDATE_PENDING_TL_FINAL_APPROVAL':'BFA_PACKAGE_SCOPED_EN_IDENTITY_CANDIDATE_PENDING_TL_FINAL_APPROVAL'}},independentLocaleAuthority:true,globalLocaleRegistryMutation:false};const localeIdentity={...localeIdentityBase,sourceDigest:sha256(localeIdentityBase)};
  const automaticComposition=src.compositionUnit&&zhBrief?.authority?.canonicalMeaning==='COMPOSITION_OF_AUTO_C2_SOURCE_BOUND_MEMBER_PROJECTIONS';const canonicalBase={nodeCode,source:src.compositionUnit?(automaticComposition?'Canonical member-node registry + source-bound Automatic C2 + deterministic C3 + Article Composition brief binding':'Canonical member-node registry + Human-frozen C2/C3 + Article Composition brief binding'):'Canonical Knowledge Registry + Human-frozen C2/C3 + PJA brief binding',nodeDigest:sha256({nodeCode,canonicalMeaning:zhBrief.canonicalMeaning}),c2C3AuthorityPreserved:true,c2C3Model:automaticComposition?'AUTOMATIC_C2_DETERMINISTIC_C3':'HISTORICAL_HUMAN_GOVERNED'};const canonicalAuthority={...canonicalBase,sourceDigest:sha256(canonicalBase)};const figureBase=figureRequirement(root,src,zhProjection);const figure={...figureBase,sourceDigest:sha256(figureBase)};

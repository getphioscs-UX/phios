...
export function buildPortfolio(root) {
  ...
  return { schemaVersion: 'PHI-OS-PJA-W3R1-PRODUCTION-PORTFOLIO-v1.0.0', authority: 'derived_projection_only', canonicalRegistryPath: 'content/knowledge/registry/nodes.json', secondCanonicalRegistry: false, generatedFromCurrentAuthority: true, entries: registry.nodes.map(node => { const state = c3.entries.find(item => item.nodeCode === node.nodeCode); const packageRoot = NODE_ROOT(node.nodeCode); return { book: node.publicationBookCode || node.sourceBookCode || 'BOOK-1', part: node.publicationPartCode || node.partCode || partOf(node.nodeCode), nodeCode: node.nodeCode, articleArchetype: node.nodeCode === 'KN-PREFACE-001' ? 'mechanism_explanation' : null, readiness: state?.status || 'not_assessed', draftStatus: fs.existsSync(path.join(root, packageRoot, 'draft.md')) ? 'prepared' : 'not_prepared', reviewStatus: fs.existsSync(path.join(root, packageRoot, 'review-status.json')) ? JSON.parse(fs.readFileSync(path.join(root, packageRoot, 'review-status.json'))).status : 'not_reviewed', exportStatus: 'not_exported', publicationStatus: 'not_published', language: 'zh-Hans', wave: node.nodeCode === 'KN-PREFACE-001' ? 'PJA-W3R1-PILOT' : null }; }) };
}

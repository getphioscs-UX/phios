...
export async function loadAcceptedBaziSnapshots({
 ...
 const snapshots={en:{},'zh-Hans':{}},records=[],historicalRecords=[];
 if(!fs.existsSync(snapshotDir))return {acceptance,snapshots,records,historicalRecords};
 ...
 for(const name of files){
  ...
  if(snapshot.compositionVersion!==COMPOSITION_VERSION||snapshot.verifierVersion!==VERIFIER_VERSION||snapshot.editorialVersion!==EDITORIAL_VERSION||snapshot.editorialQualityVersion!==QUALITY_VERSION){historicalRecords.push({locale:snapshot.locale,sectionKey:snapshot.sectionKey,snapshotDigest:snapshot.snapshotDigest,briefDigest:snapshot.sectionNarrativeBriefDigest,file,reason:'SUPERSEDED_VERSION'});continue;}
 ...
 return {acceptance,snapshots,records,historicalRecords};
}

...
export function assertPprC1CurrentSuccessor(){
 ...
 const r4v1=readJson('content/professional/personal-reality/r4/authority/ppr-r4-method-input-successor-freeze-v1.json');
 const v2Path='content/professional/personal-reality/r4/authority/ppr-r4-ast-target-context-input-successor-v2.json';
 const r4=fs.existsSync(v2Path)?readJson(v2Path):r4v1;
 ...
 assert.equal(r4v1.baselineCommit,BASELINE);
 assert.equal(r4v1.status,'ACTIVE_SUCCESSOR_OF_PPR_R3_INPUT_BOUNDARY');
 assert.equal(r4v1.boundaries.pprR3SpecialistRendererAuthorityReplaced,false);
 if(r4!==r4v1){
  assert.equal(r4.status,'ACTIVE_SUCCESSOR_RECONCILING_PPR_R4_WITH_AST_W10A');
  assert.equal(r4.semanticReconciliation.genericMethodInputHostRetained,true);
  assert.equal(r4.semanticReconciliation.baziExtensionRetained,true);
  assert.equal(r4.semanticReconciliation.astW10aTargetCapabilityRestored,true);
  assert.equal(r4.semanticReconciliation.oldSpecialCaseAstClientRestored,false);
  assert.equal(r4.boundaries.pprR3SpecialistRendererAuthorityReplaced,false);
 }
 return Object.freeze({recon,r4,r4v1});
}

...
import {
  P2_CANDIDATE_RELATIVE,
  reviewP2Candidate
} from './p2-human-review.mjs';
...
export const T08_SCHEMA_VERSION = 'PHI-OS-KNR-W2R1-P2-MAPPING-REVIEW-v1.0.0';
export const P2_MAPPING_REVIEW_RELATIVE =
  '.tmp/knowledge-manuscripts/book-1/p2-node-mapping-review.json';
export const P2_MAPPING_RELATIVE =
  'content/knowledge/manuscripts/book-1/node-manuscript-mapping.json';
export const P2_INVENTORY_RELATIVE =
  'content/knowledge/manuscripts/book-1/book-1-section-inventory.json';
export const P2_BLUEPRINT_RELATIVE =
  'content/knowledge/blueprints/book-1-knowledge-blueprint.json';
export const P2_MAPPING_MANIFEST_RELATIVE =
...
function resolveWithin(root, relativePath) {
  ...
  if (!resolved.startsWith(`${resolvedRoot}${path.sep}`)) {
    throw coded('P2_MAPPING_REVIEW_PATH_ESCAPE', { path: relativePath });
  }
  return resolved;
}
...
function isoNow(now) {
  ...
  if (Number.isNaN(value.getTime())) throw coded('P2_MAPPING_REVIEW_TIMESTAMP_INVALID');
...
function candidateState(root) {
  const file = resolveWithin(root, P2_CANDIDATE_RELATIVE);
  if (!fs.existsSync(file)) throw coded('P2_MAPPING_REVIEW_CANDIDATE_MISSING');
  const bytes = fs.readFileSync(file);
  if (!bytes.length) throw coded('P2_MAPPING_REVIEW_CANDIDATE_EMPTY');
...
function loadContext(root) {
  const t05 = reviewP2Candidate({ root, mode: 'dry-run' });
  if (t05.status !== 'human_verified' || t05.humanVerified !== true) {
    throw coded('P2_MAPPING_REVIEW_REQUIRES_T05_HUMAN_VERIFIED');
  }
  const manifest = readJson(
    resolveWithin(root, P2_MAPPING_MANIFEST_RELATIVE),
    'P2_MAPPING_REVIEW_MANIFEST_MISSING'
  );
  const inventory = validateBookISectionInventory(
    readJson(resolveWithin(root, P2_INVENTORY_RELATIVE), 'P2_MAPPING_REVIEW_INVENTORY_MISSING'),
    manifest
  );
  const blueprint = readJson(
    resolveWithin(root, P2_BLUEPRINT_RELATIVE),
    'P2_MAPPING_REVIEW_BLUEPRINT_MISSING'
  );
  const mappingFile = resolveWithin(root, P2_MAPPING_RELATIVE);
  ...
  const p2 = inventory.parts.find(part => part.partCode === 'P2');
  if (
    !p2 ||
    p2.normalizationStatus !== 'human_verified' ||
    p2.humanVerified !== true ||
    p2.stalenessStatus !== 'CURRENT' ||
    candidate.sha256 !== p2.sectionHash ||
    candidate.sha256 !== manifest.contentHashes?.normalizedParts?.P2
  ) {
    throw coded('P2_MAPPING_REVIEW_MANUSCRIPT_STALE', {
      candidateSha256: candidate.sha256,
      inventorySectionHash: p2?.sectionHash || null,
      manifestSectionHash: manifest.contentHashes?.normalizedParts?.P2 || null
    });
  }
  return {
    root,
...
function p2BlueprintNodes(context) {
  return deriveBookIBlueprintNodes(context.blueprint, context.manifest)
    .filter(node => node.partCode === 'P2');
}

export function createP2MappingReviewTemplate(context, { now } = {}) {
  const nodes = p2BlueprintNodes(context);
  ...
  if (nodes.some(node => mappingByNode.get(node.nodeCode)?.mappingStatus !== 'candidate')) {
    throw coded('P2_MAPPING_REVIEW_PREPARE_REQUIRES_CANDIDATES');
  }
  return {
    ...
    mapping: {
      path: P2_MAPPING_RELATIVE,
      sha256: context.mappingSha256,
      applyPolicy: 'atomic_after_all_p2_nodes_approved'
    },
...
function topLevelReviewIdentity(review, context) {
  if (!review || typeof review !== 'object' || Array.isArray(review)) {
    throw coded('P2_MAPPING_REVIEW_INVALID');
  }
  if (
    ...
    throw coded('P2_MAPPING_REVIEW_IDENTITY_MISMATCH');
  }
  if (
  ...
  if (
    review.mapping?.path !== P2_MAPPING_RELATIVE ||
    !HASH_PATTERN.test(clean(expectedMappingHash)) ||
    expectedMappingHash !== context.mappingSha256 ||
    review.mapping?.applyPolicy !== 'atomic_after_all_p2_nodes_approved'
  ) {
    throw coded('P2_MAPPING_REVIEW_MAPPING_STALE', {
  ...
  if (
    ...
    throw coded('P2_MAPPING_REVIEW_AUTHORITY_OR_BOUNDARY_MISMATCH');
  }
}
...
function rangeBlockers(range, node, context, allNodeCodes) {
  ...
  if (
    ...
    range.endHeading !== context.inventory.parts.find(part => part.partCode === 'P2')?.endHeading
  ) {
...
export function evaluateP2MappingReview(review, context) {
  topLevelReviewIdentity(review, context);
  const blueprintNodes = p2BlueprintNodes(context);
  ...
  if (JSON.stringify(actualCodes) !== JSON.stringify(expectedCodes)) {
    throw coded('P2_MAPPING_REVIEW_NODE_SEQUENCE_MISMATCH', {
...
export function applyApprovedP2Mapping(review, context) {
  const evaluation = evaluateP2MappingReview(review, context);
  if (review.application?.status === 'applied' && evaluation.status === 'mapped') {
    return clone(context.mapping);
  }
  if (!evaluation.readyForApply) {
    throw coded('P2_MAPPING_REVIEW_INCOMPLETE', {
  ...
  for (const record of nextMapping.mappings.filter(item => item.partCode === 'P2')) {
...
function common(context, overrides = {}) {
  const p2Nodes = p2BlueprintNodes(context);
  return {
    schemaVersion: T08_SCHEMA_VERSION,
    stage: 'KNR-W2R1-T09-P2',
    bookCode: context.manifest.bookCode,
    ...
    mappingPath: P2_MAPPING_RELATIVE,
    mappingSha256: context.mappingSha256,
    reviewPath: P2_MAPPING_REVIEW_RELATIVE,
    blueprintDerivedNodeCount: p2Nodes.length,
    automationMaximumStatus: 'candidate',
...
export function reviewP2NodeMapping({ root, mode = 'dry-run', now }) {
  if (!['dry-run', 'prepare'].includes(mode)) throw coded('P2_MAPPING_REVIEW_MODE_INVALID');
  const context = loadContext(root);
  const reviewFile = resolveWithin(root, P2_MAPPING_REVIEW_RELATIVE);
  let staleReviewArchivedTo = null;
  if (fs.existsSync(reviewFile)) {
    const existing = readJson(reviewFile, 'P2_MAPPING_REVIEW_FILE_INVALID');
    if (!mappingReviewIdentityMatches(existing, context)) {
      if (mode === 'dry-run') {
        return common(context, {
          command: 'review-map-p2',
          mode,
          ...
          approvedNodeCount: 0,
          blockedNodeCount: p2BlueprintNodes(context).length,
          nextAction: 'PREPARE_REGENERATED_PRIVATE_TL_REVIEW_TEMPLATE'
        });
      }
      staleReviewArchivedTo = archiveStaleMappingReview(reviewFile, root, 'P2', now);
    }
  }
  if (!fs.existsSync(reviewFile)) {
    if (mode === 'dry-run') {
    ...
    const template = createP2MappingReviewTemplate(context, { now });
  ...
  const review = readJson(reviewFile, 'P2_MAPPING_REVIEW_FILE_INVALID');
  const evaluation = evaluateP2MappingReview(review, context);
  return common(context, {
    command: 'review-map-p2',
    mode,
    ...
    nextAction: evaluation.readyForApply
      ...
      : evaluation.status === 'mapped'
        ? 'KNR_W2R1_T09_P2_ACCEPTANCE_CHECK'
        : 'CONTINUE_TL_REVIEW'
  });
}

export function applyP2NodeMappingReview({ root, mode = 'dry-run', now }) {
  if (!['dry-run', 'apply'].includes(mode)) throw coded('P2_MAPPING_APPLY_MODE_INVALID');
  const context = loadContext(root);
  const reviewFile = resolveWithin(root, P2_MAPPING_REVIEW_RELATIVE);
  const review = readJson(reviewFile, 'P2_MAPPING_REVIEW_FILE_MISSING');
  const evaluation = evaluateP2MappingReview(review, context);
  if (evaluation.status === 'mapped') {
    return common(context, {
      command: 'apply-map-p2',
      mode,
      status: 'already_mapped',
      approvedNodeCount: evaluation.approvedNodeCount,
      blockedNodeCount: 0,
      nextAction: 'KNR_W2R1_T09_P2_ACCEPTANCE_CHECK'
    });
  }
  if (!evaluation.readyForApply) {
    throw coded('P2_MAPPING_REVIEW_INCOMPLETE', {
  ...
  const nextMapping = applyApprovedP2Mapping(review, context);
  const mappingWritten = writeJsonAtomic(
    resolveWithin(root, P2_MAPPING_RELATIVE),
    nextMapping,
    0o644
  );
  ...
  try {
    ...
    if (mappingWritten) {
      writeJsonAtomic(resolveWithin(root, P2_MAPPING_RELATIVE), context.mapping, 0o644);
    }
    throw error;
  }

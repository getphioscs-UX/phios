...
export const P1_REPORT_RELATIVE =
...
export const EXTRACTION_TOOL_SCHEMA_VERSION =
  'PHI-OS-KNR-W2R1-P1-EXTRACTION-TOOL-v1.0.4';

const BOUNDARY_ALIAS_PROFILES = Object.freeze({
  start: Object.freeze([
    Object.freeze({ id: 'part_one_reality_physics', tokens: Object.freeze(['realityphysics']), partOne: true }),
    Object.freeze({ id: 'part_one_reality_physics_zh', tokens: Object.freeze(['第一部', '现实物理学']), normalizedHeading: '第一部｜现实物理学' })
  ]),
  end: Object.freeze([
    Object.freeze({ id: 'part_two_projection_system', tokens: Object.freeze(['projectionsystem']) }),
    Object.freeze({ id: 'part_two_projection_system_zh', tokens: Object.freeze(['第二部', '投影系统']), normalizedHeading: '第二部｜投影系统' })
  ])
});
...
export function headingDiagnostics(pages) {
  ...
  for (const page of pages) {
    ...
    for (const line of page.lines) {
      ...
      const boundarySignal = normalized.includes('realityphysics') ||
        normalized.includes('projectionsystem') ||
        normalized.includes(anchorForm('第一部｜现实物理学')) ||
        normalized.includes(anchorForm('第二部｜投影系统'));
...
export function locateP1Boundary(pages, manifest, overrides = {}) {
  const p1 = manifest.parts.find(part => part.partCode === 'P1');
  const p2 = manifest.parts.find(part => part.partCode === 'P2');
  if (!p1 || !p2) throw coded('P1_P2_MANIFEST_PARTS_REQUIRED');
  ...
  const starts = pages.flatMap(page => windowCandidates(page, p1.title, 'start'))
    .map(candidate => ({ ...candidate, contentsPage: contentsPages.has(candidate.pageNumber) }));
  const ends = pages.flatMap(page => windowCandidates(page, p2.title, 'end'))
    .map(candidate => ({ ...candidate, contentsPage: contentsPages.has(candidate.pageNumber) }));
  ...
  if (!start || !end) {
    throw coded('P1_BOUNDARY_NOT_FOUND', {
      startAnchor: p1.title,
      endAnchor: p2.title,
  ...
  if (duplicateStrongStartPages.length > 1 && overrides.startPage === undefined) {
    throw coded('P1_BOUNDARY_AMBIGUOUS', {
      boundary: 'start',
...
function sliceBoundaryPages(pages, boundary) {
  ...
  if (!selected.length) throw coded('P1_BOUNDARY_EMPTY');
  return selected;
}
...
function isHeading(line, bodyFont, p1Title) {
  if (anchorForm(line.text) === anchorForm(p1Title)) return true;
...
function isHeadingContinuation(current, next, bodyFont, p1Title) {
  if (!next || current.pageNumber !== next.pageNumber) return false;
  if (!isHeading(next, bodyFont, p1Title) || PART_HEADING_PATTERN.test(next.text)) return false;
  ...
  const shortSemanticContinuation =
    currentLength >= 20 &&
    nextLength <= 10 &&
    anchorForm(current.text) !== anchorForm(p1Title) &&
    !/[|｜:：]/u.test(next.text) &&
...
export function cleanP1Pages(pages, p1Title) {
  ...
  for (const page of pages) {
    for (const line of page.lines) {
      ...
      const workingLine = proposed.actions.length && isHeading(proposedLine, bodyFont, p1Title)
        ? proposedLine
        : line;
      if (workingLine !== line) {
        normalized.push({
      ...
      else if (signature === anchorForm(p1Title)) {
  ...
  if (!retained.length || !titleSeen) {
    throw coded('P1_CLEANING_INVALID', { retainedLineCount: retained.length, titleSeen });
  }
  ...
  for (let index = 0; index < retained.length; index += 1) {
    ...
    if (isHeading(line, bodyFont, p1Title)) {
      flush();
      const headingLines = [line];
      while (isHeadingContinuation(headingLines.at(-1), retained[index + 1], bodyFont, p1Title)) {
      ...
      blocks.push(`${anchorForm(headingText) === anchorForm(p1Title) ? '#' : '##'} ${headingText}`);
      previous = null;
      continue;
    }
  ...
  if (body.length < 100) throw coded('P1_TEXT_LAYER_TOO_SHORT', { characterCount: body.length });
  return {
    body,
...
export function buildCandidateMarkdown({ manifest, source, boundary, cleaned, sourcePageCount }) {
  ...
  const frontmatter = [
    '---',
    'schemaVersion: PHI-OS-KNR-W2R1-P1-EXTRACTION-CANDIDATE-v1.0.0',
...
async function writePrivatePair(candidatePath, candidate, reportPath, report) {
  if (fs.existsSync(candidatePath) || fs.existsSync(reportPath)) {
    throw coded('P1_CANDIDATE_ALREADY_EXISTS', {
      candidatePath,
      reportPath,
...
export async function extractP1Candidate({ root, manifest, mode = 'dry-run', overrides = {} }) {
  if (!['dry-run', 'apply'].includes(mode)) throw coded('P1_EXTRACTION_MODE_INVALID');
  ...
  const candidatePath = assertPrivatePath(root, P1_CANDIDATE_RELATIVE);
  const reportPath = assertPrivatePath(root, P1_REPORT_RELATIVE);
  ...
  const boundary = locateP1Boundary(extracted.pages, manifest, overrides);
  const pages = sliceBoundaryPages(extracted.pages, boundary);
  const cleanedBase = cleanP1Pages(pages, boundary.start.text);
  ...
  const report = {
    schemaVersion: 'PHI-OS-KNR-W2R1-P1-EXTRACTION-REPORT-v1.0.1',
  ...
  return {
    ...
    productionModified: false,
    nextAction: 'KNR-W2R1-T09-P1_HUMAN_NORMALIZATION_REVIEW'
  };
}

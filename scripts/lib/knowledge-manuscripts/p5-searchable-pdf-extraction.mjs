...
export const P5_REPORT_RELATIVE =
...
export const EXTRACTION_TOOL_SCHEMA_VERSION =
  'PHI-OS-KNR-W2R1-P5-EXTRACTION-TOOL-v1.0.4';

const BOUNDARY_ALIAS_PROFILES = Object.freeze({
  start: Object.freeze([
    Object.freeze({ id: 'part_five_conscious_runtime', tokens: Object.freeze(['consciousruntime']), normalizedHeading: '第五部｜意识运行' })
  ]),
  end: Object.freeze([])
});
...
export function headingDiagnostics(pages) {
  ...
  for (const page of pages) {
    ...
    for (const line of page.lines) {
      ...
      const boundarySignal = normalized.includes('consciousruntime') ||
        normalized.includes('第五部意识运行');
...
export function locateP5Boundary(pages, manifest, overrides = {}) {
  const p5 = manifest.parts.find(part => part.partCode === 'P5');
  if (!p5) throw coded('P5_MANIFEST_PART_REQUIRED');
  ...
  const starts = pages.flatMap(page => windowCandidates(page, p5.title, 'start'))
  ...
  if (!start) throw coded('P5_BOUNDARY_NOT_FOUND', { startAnchor: p5.title, headingDiagnostics: headingDiagnostics(pages) });
  const requestedEndPage = overrides.endPage ?? pages.at(-1)?.pageNumber;
  const endPage = pages.find(page => page.pageNumber === requestedEndPage);
  if (!endPage) throw coded('P5_END_PAGE_INVALID', { requestedEndPage });
  const end = { kind: 'end', pageNumber: endPage.pageNumber, startLineIndex: endPage.lines.length,
    endLineIndex: endPage.lines.length, text: 'END_OF_BOOK_I', fontSize: 0,
    matchMode: overrides.endPage ? 'page_override' : 'end_of_document', matchProfile: 'book_i_end',
    strongHeading: true, likelyRunningHeader: false, contentsPage: false };
  return { startAnchor: p5.title, endAnchor: 'END_OF_BOOK_I', contentsPages: [...contentsPages], start, end };
}
...
function sliceBoundaryPages(pages, boundary) {
  ...
  if (!selected.length) throw coded('P5_BOUNDARY_EMPTY');
  return selected;
}
...
function isHeading(line, bodyFont, p5Title) {
  if (anchorForm(line.text) === anchorForm(p5Title)) return true;
...
function isHeadingContinuation(current, next, bodyFont, p5Title) {
  if (!next || current.pageNumber !== next.pageNumber) return false;
  if (!isHeading(next, bodyFont, p5Title) || PART_HEADING_PATTERN.test(next.text)) return false;
  ...
  const shortSemanticContinuation =
    currentLength >= 20 &&
    nextLength <= 10 &&
    anchorForm(current.text) !== anchorForm(p5Title) &&
    !/[|｜:：]/u.test(next.text) &&
...
export function cleanP5Pages(pages, p5Title) {
  ...
  for (const page of pages) {
    for (const line of page.lines) {
      ...
      const workingLine = proposed.actions.length && isHeading(proposedLine, bodyFont, p5Title)
        ? proposedLine
        : line;
      if (workingLine !== line) {
        normalized.push({
      ...
      else if (signature === anchorForm(p5Title)) {
  ...
  if (!retained.length || !titleSeen) {
    throw coded('P5_CLEANING_INVALID', { retainedLineCount: retained.length, titleSeen });
  }
  ...
  for (let index = 0; index < retained.length; index += 1) {
    ...
    if (isHeading(line, bodyFont, p5Title)) {
      flush();
      const headingLines = [line];
      while (isHeadingContinuation(headingLines.at(-1), retained[index + 1], bodyFont, p5Title)) {
      ...
      blocks.push(`${anchorForm(headingText) === anchorForm(p5Title) ? '#' : '##'} ${headingText}`);
      previous = null;
      continue;
    }
  ...
  if (body.length < 100) throw coded('P5_TEXT_LAYER_TOO_SHORT', { characterCount: body.length });
  return {
    body,
...
export function buildCandidateMarkdown({ manifest, source, boundary, cleaned, sourcePageCount }) {
  ...
  const frontmatter = [
    '---',
    'schemaVersion: PHI-OS-KNR-W2R1-P5-EXTRACTION-CANDIDATE-v1.0.0',
...
async function writePrivatePair(candidatePath, candidate, reportPath, report) {
  if (fs.existsSync(candidatePath) || fs.existsSync(reportPath)) {
    throw coded('P5_CANDIDATE_ALREADY_EXISTS', {
      candidatePath,
      reportPath,
...
export async function extractP5Candidate({ root, manifest, mode = 'dry-run', overrides = {} }) {
  if (!['dry-run', 'apply'].includes(mode)) throw coded('P5_EXTRACTION_MODE_INVALID');
  ...
  const candidatePath = assertPrivatePath(root, P5_CANDIDATE_RELATIVE);
  const reportPath = assertPrivatePath(root, P5_REPORT_RELATIVE);
  ...
  const boundary = locateP5Boundary(extracted.pages, manifest, overrides);
  const pages = sliceBoundaryPages(extracted.pages, boundary);
  const cleanedBase = cleanP5Pages(pages, boundary.start.text);
  ...
  const report = {
    schemaVersion: 'PHI-OS-KNR-W2R1-P5-EXTRACTION-REPORT-v1.0.1',
    ...
    boundary: {
      ...
      endOfBookPage: boundary.end.pageNumber,
  ...
  return {
    ...
    productionModified: false,
    nextAction: 'KNR-W2R1-T09-P5_HUMAN_NORMALIZATION_REVIEW'
  };
}

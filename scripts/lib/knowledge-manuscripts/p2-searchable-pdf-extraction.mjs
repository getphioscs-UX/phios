...
export const P2_REPORT_RELATIVE =
...
export const EXTRACTION_TOOL_SCHEMA_VERSION =
  'PHI-OS-KNR-W2R1-P2-EXTRACTION-TOOL-v1.0.4';

const BOUNDARY_ALIAS_PROFILES = Object.freeze({
  start: Object.freeze([
    Object.freeze({ id: 'part_two_projection_system', tokens: Object.freeze(['projectionsystem']), normalizedHeading: '第二部｜投影系统' })
  ]),
  end: Object.freeze([
...
export function headingDiagnostics(pages) {
  ...
  for (const page of pages) {
    ...
    for (const line of page.lines) {
      ...
      const boundarySignal = normalized.includes('projectionsystem') ||
        normalized.includes('第二部投影系统') ||
        normalized.includes('runtimedynamics') ||
...
export function locateP2Boundary(pages, manifest, overrides = {}) {
  const p2 = manifest.parts.find(part => part.partCode === 'P2');
  const p3 = manifest.parts.find(part => part.partCode === 'P3');
  if (!p2 || !p3) throw coded('P2_P3_MANIFEST_PARTS_REQUIRED');
  ...
  const starts = pages.flatMap(page => windowCandidates(page, p2.title, 'start'))
    .map(candidate => ({ ...candidate, contentsPage: contentsPages.has(candidate.pageNumber) }));
  const ends = pages.flatMap(page => windowCandidates(page, p3.title, 'end'))
    .map(candidate => ({ ...candidate, contentsPage: contentsPages.has(candidate.pageNumber) }));
  ...
  if (!start || !end) {
    throw coded('P2_BOUNDARY_NOT_FOUND', {
      startAnchor: p2.title,
      endAnchor: p3.title,
  ...
  if (duplicateStrongStartPages.length > 1 && overrides.startPage === undefined) {
    throw coded('P2_BOUNDARY_AMBIGUOUS', {
      boundary: 'start',
...
function sliceBoundaryPages(pages, boundary) {
  ...
  if (!selected.length) throw coded('P2_BOUNDARY_EMPTY');
  return selected;
}
...
function isHeading(line, bodyFont, p2Title) {
  if (anchorForm(line.text) === anchorForm(p2Title)) return true;
...
function isHeadingContinuation(current, next, bodyFont, p2Title) {
  if (!next || current.pageNumber !== next.pageNumber) return false;
  if (!isHeading(next, bodyFont, p2Title) || PART_HEADING_PATTERN.test(next.text)) return false;
  ...
  const shortSemanticContinuation =
    currentLength >= 20 &&
    nextLength <= 10 &&
    anchorForm(current.text) !== anchorForm(p2Title) &&
    !/[|｜:：]/u.test(next.text) &&
...
export function cleanP2Pages(pages, p2Title) {
  ...
  for (const page of pages) {
    for (const line of page.lines) {
      ...
      const workingLine = proposed.actions.length && isHeading(proposedLine, bodyFont, p2Title)
        ? proposedLine
        : line;
      if (workingLine !== line) {
        normalized.push({
      ...
      else if (signature === anchorForm(p2Title)) {
  ...
  if (!retained.length || !titleSeen) {
    throw coded('P2_CLEANING_INVALID', { retainedLineCount: retained.length, titleSeen });
  }
  ...
  for (let index = 0; index < retained.length; index += 1) {
    ...
    if (isHeading(line, bodyFont, p2Title)) {
      flush();
      const headingLines = [line];
      while (isHeadingContinuation(headingLines.at(-1), retained[index + 1], bodyFont, p2Title)) {
      ...
      blocks.push(`${anchorForm(headingText) === anchorForm(p2Title) ? '#' : '##'} ${headingText}`);
      previous = null;
      continue;
    }
  ...
  if (body.length < 100) throw coded('P2_TEXT_LAYER_TOO_SHORT', { characterCount: body.length });
  return {
    body,
...
export function buildCandidateMarkdown({ manifest, source, boundary, cleaned, sourcePageCount }) {
  ...
  const frontmatter = [
    '---',
    'schemaVersion: PHI-OS-KNR-W2R1-P2-EXTRACTION-CANDIDATE-v1.0.0',
...
async function writePrivatePair(candidatePath, candidate, reportPath, report) {
  if (fs.existsSync(candidatePath) || fs.existsSync(reportPath)) {
    throw coded('P2_CANDIDATE_ALREADY_EXISTS', {
      candidatePath,
      reportPath,
...
export async function extractP2Candidate({ root, manifest, mode = 'dry-run', overrides = {} }) {
  if (!['dry-run', 'apply'].includes(mode)) throw coded('P2_EXTRACTION_MODE_INVALID');
  ...
  const candidatePath = assertPrivatePath(root, P2_CANDIDATE_RELATIVE);
  const reportPath = assertPrivatePath(root, P2_REPORT_RELATIVE);
  ...
  const boundary = locateP2Boundary(extracted.pages, manifest, overrides);
  const pages = sliceBoundaryPages(extracted.pages, boundary);
  const cleanedBase = cleanP2Pages(pages, boundary.start.text);
  ...
  const report = {
    schemaVersion: 'PHI-OS-KNR-W2R1-P2-EXTRACTION-REPORT-v1.0.1',
  ...
  return {
    ...
    productionModified: false,
    nextAction: 'KNR-W2R1-T09-P2_HUMAN_NORMALIZATION_REVIEW'
  };
}

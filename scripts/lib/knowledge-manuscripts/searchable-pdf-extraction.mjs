...
export const P0_REPORT_RELATIVE =
...
const BOUNDARY_ALIAS_PROFILES = Object.freeze({
  start: Object.freeze([
    ...
    Object.freeze({ id: 'reality_physics_sustained_reality', tokens: Object.freeze(['reality如何形成可持续现实']) })
  ])
});
...
export function locateP0Boundary(pages, manifest, overrides = {}) {
  const p0 = manifest.parts.find(part => part.partCode === 'P0');
  const p1 = manifest.parts.find(part => part.partCode === 'P1');
  if (!p0 || !p1) throw coded('P0_P1_MANIFEST_PARTS_REQUIRED');
  ...
  const starts = pages.flatMap(page => windowCandidates(page, p0.title, 'start'))
    .map(candidate => ({ ...candidate, contentsPage: contentsPages.has(candidate.pageNumber) }));
  const ends = pages.flatMap(page => windowCandidates(page, p1.title, 'end'))
    .map(candidate => ({ ...candidate, contentsPage: contentsPages.has(candidate.pageNumber) }));
  ...
  if (!start || !end) {
    throw coded('P0_BOUNDARY_NOT_FOUND', {
      startAnchor: p0.title,
      endAnchor: p1.title,
  ...
  if (duplicateStrongStartPages.length > 1 && overrides.startPage === undefined) {
    throw coded('P0_BOUNDARY_AMBIGUOUS', {
      boundary: 'start',
...
function sliceBoundaryPages(pages, boundary) {
  ...
  if (!selected.length) throw coded('P0_BOUNDARY_EMPTY');
  return selected;
}
...
function isHeading(line, bodyFont, p0Title) {
  if (anchorForm(line.text) === anchorForm(p0Title)) return true;
...
export function cleanP0Pages(pages, p0Title) {
  ...
  if (!retained.length || !titleSeen) {
    throw coded('P0_CLEANING_INVALID', { retainedLineCount: retained.length, titleSeen });
  }
  ...
  if (body.length < 100) throw coded('P0_TEXT_LAYER_TOO_SHORT', { characterCount: body.length });
  return {
    body,
...
export function buildCandidateMarkdown({ manifest, source, boundary, cleaned, sourcePageCount }) {
  ...
  const frontmatter = [
    '---',
    'schemaVersion: PHI-OS-KNR-W2R1-P0-EXTRACTION-CANDIDATE-v1.0.0',
...
async function writePrivatePair(candidatePath, candidate, reportPath, report) {
  if (fs.existsSync(candidatePath) || fs.existsSync(reportPath)) {
    throw coded('P0_CANDIDATE_ALREADY_EXISTS', {
      candidatePath,
      reportPath,
...
export async function extractP0Candidate({ root, manifest, mode = 'dry-run', overrides = {} }) {
  if (!['dry-run', 'apply'].includes(mode)) throw coded('P0_EXTRACTION_MODE_INVALID');
  ...
  const candidatePath = assertPrivatePath(root, P0_CANDIDATE_RELATIVE);
  const reportPath = assertPrivatePath(root, P0_REPORT_RELATIVE);
  ...
  const boundary = locateP0Boundary(extracted.pages, manifest, overrides);
  const pages = sliceBoundaryPages(extracted.pages, boundary);
  const cleanedBase = cleanP0Pages(pages, boundary.start.text);
  ...
  return {
    ...
    productionModified: false,
    nextAction: 'KNR-W2R1-T05_HUMAN_NORMALIZATION_REVIEW'
  };
}

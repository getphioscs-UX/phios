...
export const T05_SCHEMA_VERSION = 'PHI-OS-KNR-W2R1-P1-HUMAN-REVIEW-v1.0.0';
export const P1_CANDIDATE_RELATIVE =
  '.tmp/knowledge-manuscripts/book-1/p1-reality-physics-candidate.md';
export const P1_EXTRACTION_REPORT_RELATIVE =
  '.tmp/knowledge-manuscripts/book-1/p1-reality-physics-extraction-report.json';
export const P1_HUMAN_REVIEW_RELATIVE =
  '.tmp/knowledge-manuscripts/book-1/p1-reality-physics-human-review.json';
export const P1_MANIFEST_RELATIVE =
  'content/knowledge/manuscripts/book-1/manuscript-manifest.json';
export const P1_R2_TARGET = 'books/book-1/extracted/p1-reality-physics.md';
...
function resolvePrivate(root, relativePath) {
  ...
  if (!resolved.startsWith(`${resolvedRoot}${path.sep}`)) {
    throw coded('P1_PRIVATE_PATH_ESCAPE', { path: relativePath });
  }
  return resolved;
}
...
function isoNow(now) {
  ...
  if (Number.isNaN(value.getTime())) throw coded('P1_REVIEW_TIMESTAMP_INVALID');
  return value.toISOString();
}

function loadManifest(root) {
  const file = resolvePrivate(root, P1_MANIFEST_RELATIVE);
  const manifest = readJson(file, 'P1_MANIFEST_MISSING');
  if (manifest.bookCode !== 'BOOK-1') throw coded('P1_MANIFEST_BOOK_MISMATCH');
  if (manifest.publicAccess !== 'disabled') throw coded('P1_PUBLIC_ACCESS_MUST_BE_DISABLED');
  if (manifest.retrievalEligibility !== 'internal_only') {
    throw coded('P1_RETRIEVAL_MUST_BE_INTERNAL_ONLY');
  }
  if (!/(?:^|-)private(?:-|$)/u.test(clean(manifest.r2Bucket))) {
    throw coded('P1_PRIVATE_BUCKET_REQUIRED');
  }
  const part = manifest.parts?.find(item => item.partCode === 'P1');
  if (!part) throw coded('P1_MANIFEST_PART_MISSING');
  return { file, manifest, part };
}

function candidateState(root) {
  const candidateFile = resolvePrivate(root, P1_CANDIDATE_RELATIVE);
  const extractionReportFile = resolvePrivate(root, P1_EXTRACTION_REPORT_RELATIVE);
  if (!fs.existsSync(candidateFile)) throw coded('P1_CANDIDATE_MISSING');
  const bytes = fs.readFileSync(candidateFile);
  if (!bytes.length) throw coded('P1_CANDIDATE_EMPTY');
  const candidateText = bytes.toString('utf8');
  const extractionReport = readJson(extractionReportFile, 'P1_EXTRACTION_REPORT_MISSING');
  if (extractionReport.stage !== 'KNR-W2R1-T09-P1' || extractionReport.partCode !== 'P1') {
    throw coded('P1_EXTRACTION_REPORT_IDENTITY_MISMATCH');
  }
  if (extractionReport.candidate?.path !== P1_CANDIDATE_RELATIVE) {
    throw coded('P1_EXTRACTION_REPORT_PATH_MISMATCH');
  }
  if (extractionReport.r2TargetObjectKey !== P1_R2_TARGET) {
    throw coded('P1_EXTRACTION_REPORT_TARGET_MISMATCH');
  }
  if (extractionReport.ocrUsed !== false || extractionReport.r2UploadPerformed !== false) {
    throw coded('P1_EXTRACTION_BOUNDARY_MISMATCH');
  }
  return {
    candidateFile,
...
function approvalReport(root) {
  const file = resolvePrivate(root, P1_HUMAN_REVIEW_RELATIVE);
  return { file, report: fs.existsSync(file) ? readJson(file, 'P1_HUMAN_REVIEW_REPORT_INVALID') : null };
}
...
function manifestApprovalMatches(manifest, candidateHash) {
  ...
  return part?.normalizationStatus === 'human_verified' &&
    ...
    manifest.contentHashes?.normalizedParts?.P1 === candidateHash;
}

function reportApprovalMatches(report, candidateHash) {
  return report?.stage === 'KNR-W2R1-T09-P1' &&
...
function commonResult(candidate, automatic, overrides = {}) {
  return {
    ...
    bookCode: 'BOOK-1',
    partCode: 'P1',
    candidatePath: P1_CANDIDATE_RELATIVE,
    ...
    r2TargetObjectKey: P1_R2_TARGET,
...
export function reviewP1Candidate({
  root,
  mode = 'dry-run',
  expectedSha256 = null,
  reviewerRole = null,
  ...
  if (!['dry-run', 'approve'].includes(mode)) throw coded('P1_REVIEW_MODE_INVALID');
  ...
  if (mode === 'dry-run') {
    ...
    return commonResult(candidate, automatic, {
      ...
      reviewReportPath: P1_HUMAN_REVIEW_RELATIVE,
  ...
  if (!/^[a-f0-9]{64}$/u.test(clean(expectedSha256))) {
    throw coded('P1_REVIEW_EXPECTED_SHA256_REQUIRED');
  }
  if (expectedSha256 !== candidate.sha256) {
    throw coded('P1_REVIEW_CANDIDATE_HASH_MISMATCH', {
      expected: expectedSha256,
      actual: candidate.sha256
    });
  }
  if (reviewerRole !== 'TL') throw coded('P1_REVIEWER_ROLE_TL_REQUIRED');
  if (checklist.unknown.length || checklist.missing.length) {
    throw coded('P1_HUMAN_REVIEW_CHECKLIST_INCOMPLETE', {
      missing: checklist.missing,
      unknown: checklist.unknown
    });
  }
  if (automatic.status !== 'passed') {
    throw coded('P1_AUTOMATIC_REVIEW_BLOCKED', { blockers: automatic.blockers });
  }
  if (existingReport && !reportApprovalMatches(existingReport, candidate.sha256)) {
    throw coded('P1_EXISTING_APPROVAL_CONFLICT');
  }

  const nextManifest = clone(manifest);
  const nextPart = nextManifest.parts.find(item => item.partCode === 'P1');
  nextPart.normalizationStatus = 'human_verified';
  nextPart.humanVerified = true;
  nextManifest.contentHashes.normalizedParts.P1 = candidate.sha256;
  ...
  const nextReport = existingReport || {
    ...
    reviewedAt,
    sourceExtraction: {
      reportPath: P1_EXTRACTION_REPORT_RELATIVE,
      sourceCandidateSha256: candidate.sourceCandidateSha256
    },
    candidate: {
...
function credentialsFor(manifest, env, required) {
  ...
  if (state.status === 'incomplete') {
    throw coded('P1_R2_CREDENTIALS_INCOMPLETE', { missingVariables: state.missingVariables });
  }
  if (required && state.status !== 'configured') {
    throw coded('P1_R2_CREDENTIALS_REQUIRED', { missingVariables: state.missingVariables });
  }
  ...
  if (!/^[a-f0-9]{32}$/iu.test(accountId)) throw coded('P1_R2_ACCOUNT_ID_INVALID');
  if (bucket !== manifest.r2Bucket) {
    throw coded('P1_R2_BUCKET_MANIFEST_MISMATCH', { expectedBucket: manifest.r2Bucket });
  }
  return {
    state,
    credentials: {
...
function remoteFailure(error, fallback = 'P1_R2_REQUEST_FAILED') {
  const status = Number(error?.$metadata?.httpStatusCode || 0);
  if (status === 401 || status === 403) return coded('P1_R2_ACCESS_DENIED');
  if (status === 412) return coded('P1_R2_CONDITIONAL_WRITE_CONFLICT');
...
async function headObject(client, bucket, key) {
  try {
    ...
    throw remoteFailure(error, 'P1_R2_HEAD_FAILED');
  }
}
...
function assertRemoteObject(head, candidate) {
  ...
  if (failed.length) throw coded('P1_R2_TARGET_CONFLICT', { failedChecks: failed });
  return checks;
}

function assertApproved(manifest, report, candidate) {
  if (!reportApprovalMatches(report, candidate.sha256)) {
    throw coded('P1_HUMAN_APPROVAL_REQUIRED');
  }
  if (!manifestApprovalMatches(manifest, candidate.sha256)) {
    throw coded('P1_MANIFEST_HUMAN_APPROVAL_REQUIRED');
  }
}

export async function uploadApprovedP1({
  root,
  mode = 'dry-run',
  env = process.env,
  ...
  if (!['dry-run', 'apply'].includes(mode)) throw coded('P1_UPLOAD_MODE_INVALID');
  const candidate = candidateState(root);
  const automatic = automaticCandidateChecks(candidate.text);
  if (automatic.status !== 'passed') {
    throw coded('P1_AUTOMATIC_REVIEW_BLOCKED', { blockers: automatic.blockers });
  ...
  const existingManifestObject = manifest.objects.find(object => object.objectKey === P1_R2_TARGET);
  if (existingManifestObject && existingManifestObject.sha256 !== candidate.sha256) {
    throw coded('P1_MANIFEST_TARGET_CONFLICT');
  }
  ...
  let remoteHead;
  try {
    remoteHead = await headObject(client, credentials.bucket, P1_R2_TARGET);
    if (remoteHead) {
      ...
      try {
        ...
        if (Number(error?.$metadata?.httpStatusCode || 0) !== 412) {
          throw remoteFailure(error, 'P1_R2_UPLOAD_FAILED');
        }
      }
      remoteHead = await headObject(client, credentials.bucket, P1_R2_TARGET);
      if (!remoteHead) throw coded('P1_R2_POST_UPLOAD_HEAD_MISSING');
      assertRemoteObject(remoteHead, candidate);
    }
  } finally {
  ...
  const nextManifest = clone(manifest);
  const nextPart = nextManifest.parts.find(item => item.partCode === 'P1');
  nextPart.normalizedObjectKey = P1_R2_TARGET;
  nextPart.normalizationStatus = 'human_verified';
  nextPart.humanVerified = true;
  nextManifest.contentHashes.normalizedParts.P1 = candidate.sha256;
  const objectRecord = {
    objectRole: 'part_extraction',
    objectKey: P1_R2_TARGET,
    contentType: 'text/markdown',
    sizeBytes: candidate.sizeBytes,
    sha256: candidate.sha256,
    etag,
  ...
  const objectIndex = nextManifest.objects.findIndex(object => object.objectKey === P1_R2_TARGET);

...
import {
  ...
  reviewP0Candidate
} from './p0-human-review.mjs';

export const T08_SCHEMA_VERSION = 'PHI-OS-KNR-W2R1-P0-MAPPING-REVIEW-v1.0.0';
export const P0_MAPPING_REVIEW_RELATIVE =
  '.tmp/knowledge-manuscripts/book-1/p0-node-mapping-review.json';
export const P0_MAPPING_RELATIVE =
  'content/knowledge/manuscripts/book-1/node-manuscript-mapping.json';
export const P0_INVENTORY_RELATIVE =
  'content/knowledge/manuscripts/book-1/book-1-section-inventory.json';
export const P0_BLUEPRINT_RELATIVE =
  'content/knowledge/blueprints/book-1-knowledge-blueprint.json';
export const P0_MAPPING_MANIFEST_RELATIVE =
...
function resolveWithin(root, relativePath) {
  ...
  if (!resolved.startsWith(`${resolvedRoot}${path.sep}`)) {
    throw coded('P0_MAPPING_REVIEW_PATH_ESCAPE', { path: relativePath });
  }
  return resolved;
}
...
function isoNow(now) {
  ...
  if (Number.isNaN(value.getTime())) throw coded('P0_MAPPING_REVIEW_TIMESTAMP_INVALID');
...
function candidateState(root) {
  const file = resolveWithin(root, P0_CANDIDATE_RELATIVE);
  if (!fs.existsSync(file)) throw coded('P0_MAPPING_REVIEW_CANDIDATE_MISSING');
  const bytes = fs.readFileSync(file);
  if (!bytes.length) throw coded('P0_MAPPING_REVIEW_CANDIDATE_EMPTY');
...
function loadContext(root) {
  const t05 = reviewP0Candidate({ root, mode: 'dry-run' });
  if (t05.status !== 'human_verified' || t05.humanVerified !== true) {
    throw coded('P0_MAPPING_REVIEW_REQUIRES_T05_HUMAN_VERIFIED');
  }
  const manifest = readJson(
    resolveWithin(root, P0_MAPPING_MANIFEST_RELATIVE),
    'P0_MAPPING_REVIEW_MANIFEST_MISSING'
  );
  const inventory = validateBookISectionInventory(
    readJson(resolveWithin(root, P0_INVENTORY_RELATIVE), 'P0_MAPPING_REVIEW_INVENTORY_MISSING'),
    manifest
  );
  const blueprint = readJson(
    resolveWithin(root, P0_BLUEPRINT_RELATIVE),
    'P0_MAPPING_REVIEW_BLUEPRINT_MISSING'
  );
  const mappingFile = resolveWithin(root, P0_MAPPING_RELATIVE);
  ...
  const p0 = inventory.parts.find(part => part.partCode === 'P0');
  if (
    !p0 ||
    p0.normalizationStatus !== 'human_verified' ||
    p0.humanVerified !== true ||
    p0.stalenessStatus !== 'CURRENT' ||
    candidate.sha256 !== p0.sectionHash ||
    candidate.sha256 !== manifest.contentHashes?.normalizedParts?.P0
  ) {
    throw coded('P0_MAPPING_REVIEW_MANUSCRIPT_STALE', {
      candidateSha256: candidate.sha256,
      inventorySectionHash: p0?.sectionHash || null,
      manifestSectionHash: manifest.contentHashes?.normalizedParts?.P0 || null
    });
  }
  return {
    root,
...
function p0BlueprintNodes(context) {
  return deriveBookIBlueprintNodes(context.blueprint, context.manifest)
    .filter(node => node.partCode === 'P0');
}

export function createP0MappingReviewTemplate(context, { now } = {}) {
  const nodes = p0BlueprintNodes(context);
  ...
  if (nodes.some(node => mappingByNode.get(node.nodeCode)?.mappingStatus !== 'candidate')) {
    throw coded('P0_MAPPING_REVIEW_PREPARE_REQUIRES_CANDIDATES');
  }
  return {
    ...
    mapping: {
      path: P0_MAPPING_RELATIVE,
      sha256: context.mappingSha256,
      applyPolicy: 'atomic_after_all_p0_nodes_approved'
    },
...
function topLevelReviewIdentity(review, context) {
  if (!review || typeof review !== 'object' || Array.isArray(review)) {
    throw coded('P0_MAPPING_REVIEW_INVALID');
  }
  if (
    ...
    throw coded('P0_MAPPING_REVIEW_IDENTITY_MISMATCH');
  }
  if (
  ...
  if (
    review.mapping?.path !== P0_MAPPING_RELATIVE ||
    !HASH_PATTERN.test(clean(expectedMappingHash)) ||
    expectedMappingHash !== context.mappingSha256 ||
    review.mapping?.applyPolicy !== 'atomic_after_all_p0_nodes_approved'
  ) {
    throw coded('P0_MAPPING_REVIEW_MAPPING_STALE', {
  ...
  if (
    ...
    throw coded('P0_MAPPING_REVIEW_AUTHORITY_OR_BOUNDARY_MISMATCH');
  }
}
...
function rangeBlockers(range, node, context, allNodeCodes) {
  ...
  if (
    ...
    range.endHeading !== context.inventory.parts.find(part => part.partCode === 'P0')?.endHeading
  ) {
...
export function evaluateP0MappingReview(review, context) {
  topLevelReviewIdentity(review, context);
  const blueprintNodes = p0BlueprintNodes(context);
  ...
  if (JSON.stringify(actualCodes) !== JSON.stringify(expectedCodes)) {
    throw coded('P0_MAPPING_REVIEW_NODE_SEQUENCE_MISMATCH', {
...
export function applyApprovedP0Mapping(review, context) {
  const evaluation = evaluateP0MappingReview(review, context);
  if (review.application?.status === 'applied' && evaluation.status === 'mapped') {
    return clone(context.mapping);
  }
  if (!evaluation.readyForApply) {
    throw coded('P0_MAPPING_REVIEW_INCOMPLETE', {
  ...
  for (const record of nextMapping.mappings.filter(item => item.partCode === 'P0')) {
...
function common(context, overrides = {}) {
  const p0Nodes = p0BlueprintNodes(context);
  return {
    schemaVersion: T08_SCHEMA_VERSION,
    stage: 'KNR-W2R1-T08',
    bookCode: context.manifest.bookCode,
    ...
    mappingPath: P0_MAPPING_RELATIVE,
    mappingSha256: context.mappingSha256,
    reviewPath: P0_MAPPING_REVIEW_RELATIVE,
    blueprintDerivedNodeCount: p0Nodes.length,
    automationMaximumStatus: 'candidate',
...
export function reviewP0NodeMapping({ root, mode = 'dry-run', now }) {
  if (!['dry-run', 'prepare'].includes(mode)) throw coded('P0_MAPPING_REVIEW_MODE_INVALID');
  const context = loadContext(root);
  const reviewFile = resolveWithin(root, P0_MAPPING_REVIEW_RELATIVE);
  if (!fs.existsSync(reviewFile)) {
    if (mode === 'dry-run') {
      return common(context, {
        command: 'review-map-p0',
        mode,
        status: 'review_template_required',
        reviewFilePresent: false,
        approvedNodeCount: 0,
        blockedNodeCount: p0BlueprintNodes(context).length,
        nextAction: 'PREPARE_PRIVATE_TL_REVIEW_TEMPLATE'
      });
    }
    const template = createP0MappingReviewTemplate(context, { now });
  ...
  const review = readJson(reviewFile, 'P0_MAPPING_REVIEW_FILE_INVALID');
  const evaluation = evaluateP0MappingReview(review, context);
  return common(context, {
    command: 'review-map-p0',
    mode,
    ...
    nextAction: evaluation.readyForApply
      ...
      : evaluation.status === 'mapped'
        ? 'KNR_W2R1_T08_ACCEPTANCE_CHECK'
        : 'CONTINUE_TL_REVIEW'
  });
}

export function applyP0NodeMappingReview({ root, mode = 'dry-run', now }) {
  if (!['dry-run', 'apply'].includes(mode)) throw coded('P0_MAPPING_APPLY_MODE_INVALID');
  const context = loadContext(root);
  const reviewFile = resolveWithin(root, P0_MAPPING_REVIEW_RELATIVE);
  const review = readJson(reviewFile, 'P0_MAPPING_REVIEW_FILE_MISSING');
  const evaluation = evaluateP0MappingReview(review, context);
  if (evaluation.status === 'mapped') {
    return common(context, {
      command: 'apply-map-p0',
      mode,
      status: 'already_mapped',
      approvedNodeCount: evaluation.approvedNodeCount,
      blockedNodeCount: 0,
      nextAction: 'KNR_W2R1_T08_ACCEPTANCE_CHECK'
    });
  }
  if (!evaluation.readyForApply) {
    throw coded('P0_MAPPING_REVIEW_INCOMPLETE', {
  ...
  const nextMapping = applyApprovedP0Mapping(review, context);
  const mappingWritten = writeJsonAtomic(
    resolveWithin(root, P0_MAPPING_RELATIVE),
    nextMapping,
    0o644
  );
  ...
  try {
    ...
    if (mappingWritten) {
      writeJsonAtomic(resolveWithin(root, P0_MAPPING_RELATIVE), context.mapping, 0o644);
    }
    throw error;
  }

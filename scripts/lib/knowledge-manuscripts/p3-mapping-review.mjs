...
function resolveWithin(root, relativePath) {
  ...
  if (!resolved.startsWith(`${resolvedRoot}${path.sep}`)) {
    throw coded('P3_MAPPING_REVIEW_PATH_ESCAPE', { path: relativePath });
  }
  return resolved;
}
...
function isoNow(now) {
  ...
  if (Number.isNaN(value.getTime())) throw coded('P3_MAPPING_REVIEW_TIMESTAMP_INVALID');
...
function candidateState(root) {
  const file = resolveWithin(root, P3_CANDIDATE_RELATIVE);
  if (!fs.existsSync(file)) throw coded('P3_MAPPING_REVIEW_CANDIDATE_MISSING');
  const bytes = fs.readFileSync(file);
  if (!bytes.length) throw coded('P3_MAPPING_REVIEW_CANDIDATE_EMPTY');
...
function loadContext(root) {
  const t05 = reviewP3Candidate({ root, mode: 'dry-run' });
  if (t05.status !== 'human_verified' || t05.humanVerified !== true) {
    throw coded('P3_MAPPING_REVIEW_REQUIRES_T05_HUMAN_VERIFIED');
  }
  const manifest = readJson(
    resolveWithin(root, P3_MAPPING_MANIFEST_RELATIVE),
    'P3_MAPPING_REVIEW_MANIFEST_MISSING'
  );
  const inventory = validateBookISectionInventory(
    readJson(resolveWithin(root, P3_INVENTORY_RELATIVE), 'P3_MAPPING_REVIEW_INVENTORY_MISSING'),
    manifest
  );
  const blueprint = readJson(
    resolveWithin(root, P3_BLUEPRINT_RELATIVE),
    'P3_MAPPING_REVIEW_BLUEPRINT_MISSING'
  );
  const mappingFile = resolveWithin(root, P3_MAPPING_RELATIVE);
  ...
  const p3 = inventory.parts.find(part => part.partCode === 'P3');
  if (
    !p3 ||
    p3.normalizationStatus !== 'human_verified' ||
    p3.humanVerified !== true ||
    p3.stalenessStatus !== 'CURRENT' ||
    candidate.sha256 !== p3.sectionHash ||
    candidate.sha256 !== manifest.contentHashes?.normalizedParts?.P3
  ) {
    throw coded('P3_MAPPING_REVIEW_MANUSCRIPT_STALE', {
      candidateSha256: candidate.sha256,
      inventorySectionHash: p3?.sectionHash || null,
      manifestSectionHash: manifest.contentHashes?.normalizedParts?.P3 || null
    });
  }
  return {
    root,
...
function p3BlueprintNodes(context) {
  return deriveBookIBlueprintNodes(context.blueprint, context.manifest)
    .filter(node => node.partCode === 'P3');
}

export function createP3MappingReviewTemplate(context, { now } = {}) {
  const nodes = p3BlueprintNodes(context);
  ...
  if (nodes.some(node => mappingByNode.get(node.nodeCode)?.mappingStatus !== 'candidate')) {
    throw coded('P3_MAPPING_REVIEW_PREPARE_REQUIRES_CANDIDATES');
  }
  return {
    ...
    mapping: {
      path: P3_MAPPING_RELATIVE,
      sha256: context.mappingSha256,
      applyPolicy: 'atomic_after_all_p3_nodes_approved'
    },
...
function topLevelReviewIdentity(review, context) {
  if (!review || typeof review !== 'object' || Array.isArray(review)) {
    throw coded('P3_MAPPING_REVIEW_INVALID');
  }
  if (
    review.schemaVersion !== T08_SCHEMA_VERSION ||
    review.stage !== 'KNR-W2R1-T09-P3' ||
    ...
    throw coded('P3_MAPPING_REVIEW_IDENTITY_MISMATCH');
  }
  if (
  ...
  if (
    review.mapping?.path !== P3_MAPPING_RELATIVE ||
    !HASH_PATTERN.test(clean(expectedMappingHash)) ||
    expectedMappingHash !== context.mappingSha256 ||
    review.mapping?.applyPolicy !== 'atomic_after_all_p3_nodes_approved'
  ) {
    throw coded('P3_MAPPING_REVIEW_MAPPING_STALE', {
  ...
  if (
    ...
    throw coded('P3_MAPPING_REVIEW_AUTHORITY_OR_BOUNDARY_MISMATCH');
  }
}
...
function rangeBlockers(range, node, context, allNodeCodes) {
  ...
  if (
    ...
    range.endHeading !== context.inventory.parts.find(part => part.partCode === 'P3')?.endHeading
  ) {
...
export function evaluateP3MappingReview(review, context) {
  topLevelReviewIdentity(review, context);
  const blueprintNodes = p3BlueprintNodes(context);
  ...
  if (JSON.stringify(actualCodes) !== JSON.stringify(expectedCodes)) {
    throw coded('P3_MAPPING_REVIEW_NODE_SEQUENCE_MISMATCH', {
...
export function applyApprovedP3Mapping(review, context) {
  const evaluation = evaluateP3MappingReview(review, context);
  if (review.application?.status === 'applied' && evaluation.status === 'mapped') {
    return clone(context.mapping);
  }
  if (!evaluation.readyForApply) {
    throw coded('P3_MAPPING_REVIEW_INCOMPLETE', {
  ...
  for (const record of nextMapping.mappings.filter(item => item.partCode === 'P3')) {
...
function common(context, overrides = {}) {
  const p3Nodes = p3BlueprintNodes(context);
  return {
    schemaVersion: T08_SCHEMA_VERSION,
    stage: 'KNR-W2R1-T09-P3',
    bookCode: context.manifest.bookCode,
    ...
    mappingPath: P3_MAPPING_RELATIVE,
    mappingSha256: context.mappingSha256,
    reviewPath: P3_MAPPING_REVIEW_RELATIVE,
    blueprintDerivedNodeCount: p3Nodes.length,
    automationMaximumStatus: 'candidate',
...
export function reviewP3NodeMapping({ root, mode = 'dry-run', now }) {
  ...
  if (!fs.existsSync(reviewFile)) {
    ...
    const template = createP3MappingReviewTemplate(context, { now });
  ...
  const review = readJson(reviewFile, 'P3_MAPPING_REVIEW_FILE_INVALID');
  const evaluation = evaluateP3MappingReview(review, context);
  return common(context, {
    command: 'review-map-p3',
    mode,
    ...
    nextAction: evaluation.readyForApply
      ...
      : evaluation.status === 'mapped'
        ? 'KNR_W2R1_T09_P3_ACCEPTANCE_CHECK'
        : 'CONTINUE_TL_REVIEW'
  });
}

export function applyP3NodeMappingReview({ root, mode = 'dry-run', now }) {
  if (!['dry-run', 'apply'].includes(mode)) throw coded('P3_MAPPING_APPLY_MODE_INVALID');
  const context = loadContext(root);
  const reviewFile = resolveWithin(root, P3_MAPPING_REVIEW_RELATIVE);
  const review = readJson(reviewFile, 'P3_MAPPING_REVIEW_FILE_MISSING');
  const evaluation = evaluateP3MappingReview(review, context);
  if (evaluation.status === 'mapped') {
    return common(context, {
      command: 'apply-map-p3',
      mode,
      status: 'already_mapped',
      approvedNodeCount: evaluation.approvedNodeCount,
      blockedNodeCount: 0,
      nextAction: 'KNR_W2R1_T09_P3_ACCEPTANCE_CHECK'
    });
  }
  if (!evaluation.readyForApply) {
    throw coded('P3_MAPPING_REVIEW_INCOMPLETE', {
  ...
  const nextMapping = applyApprovedP3Mapping(review, context);
  const mappingWritten = writeJsonAtomic(
    resolveWithin(root, P3_MAPPING_RELATIVE),
    nextMapping,
    0o644
  );
  ...
  try {
    ...
    if (mappingWritten) {
      writeJsonAtomic(resolveWithin(root, P3_MAPPING_RELATIVE), context.mapping, 0o644);
    }
    throw error;
  }

...
export const P4_REPORT_RELATIVE =
...
export const EXTRACTION_TOOL_SCHEMA_VERSION =
  'PHI-OS-KNR-W2R1-P4-EXTRACTION-TOOL-v1.0.4';
...
export function headingDiagnostics(pages) {
  ...
  for (const page of pages) {
    ...
    for (const line of page.lines) {
      ...
      const boundarySignal = normalized.includes('humanruntimecarrier') ||
        normalized.includes('第四部人类运行载体') ||
        normalized.includes('consciousruntime') ||
...
export function locateP4Boundary(pages, manifest, overrides = {}) {
  const p4 = manifest.parts.find(part => part.partCode === 'P4');
  const p5 = manifest.parts.find(part => part.partCode === 'P5');
  if (!p4 || !p5) throw coded('P4_P5_MANIFEST_PARTS_REQUIRED');
  ...
  const starts = pages.flatMap(page => windowCandidates(page, p4.title, 'start'))
    .map(candidate => ({ ...candidate, contentsPage: contentsPages.has(candidate.pageNumber) }));
  const ends = pages.flatMap(page => windowCandidates(page, p5.title, 'end'))
    .map(candidate => ({ ...candidate, contentsPage: contentsPages.has(candidate.pageNumber) }));
  ...
  if (!start || !end) {
    throw coded('P4_BOUNDARY_NOT_FOUND', {
      startAnchor: p4.title,
      endAnchor: p5.title,
  ...
  if (duplicateStrongStartPages.length > 1 && overrides.startPage === undefined) {
    throw coded('P4_BOUNDARY_AMBIGUOUS', {
      boundary: 'start',
...
function sliceBoundaryPages(pages, boundary) {
  ...
  if (!selected.length) throw coded('P4_BOUNDARY_EMPTY');
  return selected;
}
...
function isHeading(line, bodyFont, p4Title) {
  if (anchorForm(line.text) === anchorForm(p4Title)) return true;
...
function isHeadingContinuation(current, next, bodyFont, p4Title) {
  if (!next || current.pageNumber !== next.pageNumber) return false;
  if (!isHeading(next, bodyFont, p4Title) || PART_HEADING_PATTERN.test(next.text)) return false;
  ...
  const shortSemanticContinuation =
    currentLength >= 20 &&
    nextLength <= 10 &&
    anchorForm(current.text) !== anchorForm(p4Title) &&
    !/[|｜:：]/u.test(next.text) &&
...
export function cleanP4Pages(pages, p4Title) {
  ...
  for (const page of pages) {
    for (const line of page.lines) {
      ...
      const workingLine = proposed.actions.length && isHeading(proposedLine, bodyFont, p4Title)
        ? proposedLine
        : line;
      if (workingLine !== line) {
        normalized.push({
      ...
      else if (signature === anchorForm(p4Title)) {
  ...
  if (!retained.length || !titleSeen) {
    throw coded('P4_CLEANING_INVALID', { retainedLineCount: retained.length, titleSeen });
  }
  ...
  for (let index = 0; index < retained.length; index += 1) {
    ...
    if (isHeading(line, bodyFont, p4Title)) {
      flush();
      const headingLines = [line];
      while (isHeadingContinuation(headingLines.at(-1), retained[index + 1], bodyFont, p4Title)) {
      ...
      blocks.push(`${anchorForm(headingText) === anchorForm(p4Title) ? '#' : '##'} ${headingText}`);
      previous = null;
      continue;
    }
  ...
  if (body.length < 100) throw coded('P4_TEXT_LAYER_TOO_SHORT', { characterCount: body.length });
  return {
    body,
...
export function buildCandidateMarkdown({ manifest, source, boundary, cleaned, sourcePageCount }) {
  ...
  const frontmatter = [
    '---',
    'schemaVersion: PHI-OS-KNR-W2R1-P4-EXTRACTION-CANDIDATE-v1.0.0',
...
async function writePrivatePair(candidatePath, candidate, reportPath, report) {
  if (fs.existsSync(candidatePath) || fs.existsSync(reportPath)) {
    throw coded('P4_CANDIDATE_ALREADY_EXISTS', {
      candidatePath,
      reportPath,
...
export async function extractP4Candidate({ root, manifest, mode = 'dry-run', overrides = {} }) {
  if (!['dry-run', 'apply'].includes(mode)) throw coded('P4_EXTRACTION_MODE_INVALID');
  ...
  const candidatePath = assertPrivatePath(root, P4_CANDIDATE_RELATIVE);
  const reportPath = assertPrivatePath(root, P4_REPORT_RELATIVE);
  ...
  const boundary = locateP4Boundary(extracted.pages, manifest, overrides);
  const pages = sliceBoundaryPages(extracted.pages, boundary);
  const cleanedBase = cleanP4Pages(pages, boundary.start.text);
  ...
  const report = {
    schemaVersion: 'PHI-OS-KNR-W2R1-P4-EXTRACTION-REPORT-v1.0.1',
  ...
  return {
    ...
    productionModified: false,
    nextAction: 'KNR-W2R1-T09-P4_HUMAN_NORMALIZATION_REVIEW'
  };
}

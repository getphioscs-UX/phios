...
export function reviewP5NodeMapping({ root, mode = 'dry-run', now }) {
  ...
  if (fs.existsSync(reviewFile)) {
    const existing = readJson(reviewFile, 'P5_MAPPING_REVIEW_FILE_INVALID');
    if (!mappingReviewIdentityMatches(existing, context)) {
      ...
      staleReviewArchivedTo = archiveStaleMappingReview(reviewFile, root, 'P5', now);
    }
  }
  if (!fs.existsSync(reviewFile)) {
    if (mode === 'dry-run') {

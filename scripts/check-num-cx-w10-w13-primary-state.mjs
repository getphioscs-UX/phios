...
if(pprR3Active){
 assert.equal(v.productRoute?.mode,'SINGLE_METHOD');
 assert.equal(v.productRoute?.state,'CUSTOMER_PUBLISHABLE');
 assert.equal(v.productRoute?.primaryProduct?.methodId,'NUM');
 assert.equal(v.productRoute?.primaryProduct?.state,'CUSTOMER_PUBLISHABLE');
 assert.equal(v.productRoute?.primaryProduct?.specialistRenderer?.rendererId,'PPR_R3_NUM_PRODUCT_V1');
 assert.equal(v.productRoute?.primaryProduct?.sourceProduct?.integratedReading?.customerPublishable,true);
}else{

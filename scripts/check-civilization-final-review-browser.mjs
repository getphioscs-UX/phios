...
try {
 ...
 assert.equal(await page.locator('#articles li').count(),42);assert.equal(await page.locator('#parallel p').count(),8);assert.equal(await page.locator('#missing tr').count(),3);
 ...
 fs.writeFileSync('content/civilization-atlas/maintenance/visual-activation-60247ff/final-review-browser-v1.json',JSON.stringify({status:'PASS',checks:['42 bilingual article links','8 representative paragraphs','13 real local Ask answers','3 honest path exceptions','360/1440 no overflow','12 image pagination','flag selection','Enter/Escape/focus return','current world caption'],humanDecision:'PENDING_HUMAN_REVIEW'},null,2)+'\n');console.log('PASS consolidated review and current world caption');

...
const w16CurrentOwnerReconciliationPath=`${BASE}/authority/ziwei-cx-r1-w16-current-shared-owner-reconciliation-v2.json`;const w16CurrentOwnerReconciliation=fs.existsSync(w16CurrentOwnerReconciliationPath)?j(w16CurrentOwnerReconciliationPath):null;
...
if(w16CurrentOwnerReconciliation){assert.equal(w16CurrentOwnerReconciliation.status,'CURRENT_SHARED_OWNER_RECONCILED_NO_ZIWEI_MUTATION');assert.equal(w16CurrentOwnerReconciliation.historicalFreezeMutated,false);assert.equal(w16CurrentOwnerReconciliation.ziweiSharedSurfaceMutationByThisWork,false);assert.equal(w16CurrentOwnerReconciliation.futureUnregisteredDriftAllowed,false);assert.equal(sha(w16CurrentOwnerReconciliation.sharedFile),w16CurrentOwnerReconciliation.currentOwnerSha256);assert.equal(freeze.files.find(x=>x.path===w16CurrentOwnerReconciliation.sharedFile)?.sha256,w16CurrentOwnerReconciliation.historicalW16Sha256);}

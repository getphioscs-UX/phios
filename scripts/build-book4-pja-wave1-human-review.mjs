...
const reviewMeta = {
  ...
  formalAdmissionStatus:'BOOK-IV-A1_COMPLETE_A2_READY'
};
...
</style></head><body><main><section class="hero"><div class="eyebrow">PHI OS · BOOK IV · PJA WAVE 1 · PRE-ADMISSION</div><h1>《世界如何扩展》Article 人工审阅</h1><p>本页审阅 8 篇中文 Article 候选。它们已绑定第四册最终 PDF 的具体章节与页码，但 <strong>BOOK-IV-A1 Canonical Coverage 已完成</strong>，因此本轮 8/8 已由 TL 接受；这仍不会自动成为 A3 完成、不会加入 KIR、不会发布。</p><div id="summary"></div><div class="toolbar"><button id="acceptAll">全部 ACCEPT</button><button class="secondary" id="clearAll">清空决定</button><button id="export">导出 Article Decisions JSON</button></div></section>
<section class="alert"><strong>A1 需要处理的来源发现</strong><p>原始 PDF 抽取共 127 个正文 occurrence。TL 已确认成本递延（S050）与资源压缩（S051）为重复出现，最终 manuscript authority 排除这两个 second occurrence，形成 125 个最终 section（P10 79 / P11 46）；原始 PDF 与 raw extraction evidence 保留。</p><p class="muted">BOOK-IV-A1 已建立 125 个 completed-manuscript final Canonical Nodes 与 125 个 retrieval identities；predecessor 181 records 保留为 historical compatibility evidence。</p></section>
...
function render(){
 ...
 document.querySelector('#summary').innerHTML='<span class="pill">8 篇中文 Article</span><span class="pill">已决定 '+decided+'/8</span><span class="pill">ACCEPT '+accepted+'/8</span><span class="pill">A1 complete</span><span class="pill">Publication closed</span>';
...
fs.writeFileSync(path.join(outDir,'book4-pja-wave1-review-metadata-v1.json'),JSON.stringify({...reviewMeta,status:'TL_8_OF_8_ACCEPTED_A1_COMPLETE'},null,2)+'\n','utf8');

...
assert(reg.trajectories.length>=6,'W9_VERTICAL_SLICE_SUCCESSOR_MUST_RETAIN_AT_LEAST_6');
assert(new Set(reg.trajectories.map(x=>x.trajectoryId)).size===reg.trajectories.length,'W9_DUPLICATE_ID');
for(const id of ['POPULATION','URBAN_DENSITY','ENERGY','KNOWLEDGE_PARTICIPATION','COMPUTATIONAL_CAPACITY','LOAD_FUTURE_CAPACITY']) assert(reg.trajectories.some(x=>x.trajectoryId===id),'W9_ORIGINAL_TRAJECTORY_MISSING:'+id);
...
console.log(`  Original six trajectory contracts remain present inside the successor registry (${reg.trajectories.length} trajectories total).`);

...
const html=fs.readFileSync('financial-reality.html','utf8');for(const s of ['Financial Reality','data-frp-form','data-frp-results','financialReality=ephemeral','/professional/financial/']) assert.ok(html.includes(s),s);assert.ok(!html.includes('guaranteed return'));

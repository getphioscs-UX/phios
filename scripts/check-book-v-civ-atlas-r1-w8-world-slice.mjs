...
const registry=json('content/civilization-atlas/snapshots/world-snapshots-v1.json'); assert.equal(registry.status,'ACTIVE'); assert.equal(registry.snapshots.length,4);
const ids=new Set(registry.snapshots.map(s=>s.snapshotId)); for(const id of ['WS-1000','WS-1500','WS-1850','WS-2026']) assert.ok(ids.has(id),`missing vertical-slice snapshot ${id}`);

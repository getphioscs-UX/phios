...
import {onRequestPost as sessionEndpoint} from '../functions/api/iching-limited-session.js';
...
const certFetch=async()=>new Response(JSON.stringify({keys:[jwk]}),{status:200,headers:{'content-type':'application/json'}});
...
await assert.rejects(()=>verifyCloudflareAccessJwt({jwt:jwt.slice(0,-1)+'x',teamDomain:team,audiences:new Set([aud]),allowedEmails:new Set([email]),fetchImpl:certFetch,clock:()=>now}),/SIGNATURE/);
...
const request=new Request('https://example.test/api/iching-limited-session',{method:'POST',headers:{'cf-access-jwt-assertion':jwt}});Object.defineProperty(request,'cf',{value:{country:'MY'}});
const originalFetch=globalThis.fetch; globalThis.fetch=certFetch; const sessionResponse=await sessionEndpoint({request,env,data:{}}); globalThis.fetch=originalFetch; assert.equal(sessionResponse.status,200); const setCookie=sessionResponse.headers.get('set-cookie'); assert.ok(setCookie.includes(ICHING_LIMITED_COOKIE)); assert.equal(db.users.size,0,'D1 probe must clean up');
...
const executeReq=new Request('https://example.test/api/symbolic-method-execute',{method:'POST',headers:{'content-type':'application/json',cookie},body:JSON.stringify({method:'I_CHING',question:'What can I observe?',inputMode:'MANUAL_LINES',lines,sessionId:'lp-static',timestamp:'2026-08-26T04:30:00.000Z',projectionVersion:'1.0.0',locale:'en'})});Object.defineProperty(executeReq,'cf',{value:{country:'MY'}});
...
console.log('✓ ICH-PROD-W24-W27 Limited Production static acceptance passed.');
console.log('  RS256 Access JWT + issuer/audience/email/country + exact SHA + D1 write/read/cleanup + signed beta session + stable depth execution are closed.');

...
packageJson.scripts['check:kh-w4b.5-book-3-r1'] =
  'node scripts/check-kh-w4b5-book-3-r1-compatibility-repair.mjs';

const gate='node scripts/check-kh-w4b5-book-3-r1-compatibility-repair.mjs';
const book3Gate='node scripts/check-kh-w4b5-book-3-canonical-node-registry.mjs';
if (!packageJson.scripts.precheck.includes(gate)) {
  packageJson.scripts.precheck=packageJson.scripts.precheck.includes(book3Gate)
    ? packageJson.scripts.precheck.replace(book3Gate,`${book3Gate} && ${gate}`)
    : `${packageJson.scripts.precheck} && ${gate}`;
}
await fs.writeFile(path,`${JSON.stringify(packageJson,null,2)}\n`,'utf8');
console.log('✓ package.json updated for KH-W4B.5 Book 3 R1.');

...
const registry=read('data/customer-activation-r1/issue-registry-v1.json');assert.equal(registry.issues.length,14);assert.equal(registry.issues.filter(x=>x.priority==='P0').length,5);assert.ok(registry.issues.every(x=>x.status!=='CLOSED'));
assert.equal(read('docs/customer-activation-r1/m8-review-requirements-v1.json').productionReadiness,false);

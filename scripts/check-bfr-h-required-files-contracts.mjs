...
const pxrSuccessorPath = 'content/web-production/pxr/successors/pxr-bfr-h-central-checker-successor-v1.json';
assert.ok(exists(pxrSuccessorPath), `Missing PXR central checker successor: ${pxrSuccessorPath}`);
const pxrSuccessor = read(pxrSuccessorPath);
...
assert.equal(pxrSuccessor.status, 'ACTIVE_ADDITIVE_PXR_BEFORE_BFR_H16_SUCCESSOR');
assert.equal(pxrSuccessor.predecessor.architecturePath, archPath);
assert.equal(pxrSuccessor.predecessor.postcheckCommand, 'npm run check:hpc2-cka-direct-mapping && npm run check:bfr-h-cka-direct-mapping && npm run check:bfr-h15 && npm run check:bfr-h16');
assert.equal(pxrSuccessor.predecessor.rewritten, false);
assert.equal(pkg.scripts['postcheck:bfr-h'], pxrSuccessor.successor.postcheckCommand);
assert.deepEqual(pxrSuccessor.successor.sequence, ['HPC2_CKA_DIRECT_MAPPING','BFR_H_CKA_DIRECT_MAPPING','BFR_H15','PXR_P0_P3','BFR_H16']);
assert.equal(pxrSuccessor.boundaries.checkBfrHPredecessorCommandChanged, false);
assert.equal(pxrSuccessor.boundaries.bfrH16AuthorityRecreated, false);
...
console.log('✓ PART H central entry contract passed: one TL-facing BFR-H entry, PXR is inserted additively between H15 and H16, and the frozen predecessor check:bfr-h command remains unchanged.');

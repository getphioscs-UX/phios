...
assert.match(renderer,/detail:\[entityTypeLabel\(x\.entityType\),stateLabel\(x\.dataClass,l\),x\.version\]/);
assert.match(renderer,/declaredStatus==='PRESENT'\?resolveAtlasVisualById\(data\.visualBindings,s\.visualAssetId\):null/);
assert.match(renderer,/data-resolver-state="\$\{esc\(visualState\)\}"/);
assert.match(renderer,/visualState==='MISSING'\?c\.missing:c\.unverified/);
off?.();

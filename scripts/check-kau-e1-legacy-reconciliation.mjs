...
assert.equal(nodes.nodes.length,716,'KAU_E1_CANONICAL_NODE_COUNT_DRIFT');
...
assert.equal(`sha256:${actual}`,acceptance.canonicalNodeRegistryDigestBefore,'KAU_E1_CANONICAL_DIGEST_BASELINE_MISMATCH');
...
console.log(`✓ KAU-E1 legacy reconciliation passed: ${sections.entries.length} inventory entries, ${matches.entries.length} candidate matches, ${review.entries.length} pending human reviews; Canonical Nodes remain 716 and unchanged.`);

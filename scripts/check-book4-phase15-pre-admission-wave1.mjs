...
assert.equal(manifest.articleConceptCount,8); assert.equal(manifest.localeCandidateCount,16); assert.equal(manifest.records.length,16); assert.equal(manifest.masterWork.a1Status,'COMPLETE'); assert.equal(manifest.masterWork.a2Status,'COMPLETE'); assert.ok(['READY_NOT_STARTED','COMPLETE'].includes(manifest.masterWork.a3Status)); assert.equal(manifest.publication.bookIVAdmitted,false);
...
console.log(`✓ BOOK IV Phase 15 pre-admission successor passed: A0/A1/A2 complete; A3 ${manifest.masterWork.a3Status}; 125 final sections after TL-confirmed dedup.`);

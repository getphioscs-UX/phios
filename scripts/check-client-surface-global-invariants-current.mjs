...
const ckaSuccessorPath = 'content/web-production/reconciliation/hpc2-w5-cka-w0-w4-current-successor-v1.json';
const ckaW0Path = 'content/client/knowledge-ask/contracts/cka-w0-ask-entry-contract-v1.json';
...
const ckaSuccessor = readJson(ckaSuccessorPath);
const ckaW0 = readJson(ckaW0Path);
...
assert.equal(invariantSuccessor.status, 'ACTIVE_ADDITIVE_CKA_SUCCESSOR_ALL_TEN_INVARIANTS_PRESERVED');
assert.equal(sha256(invariantSuccessor.predecessorFoundation.checker.path), invariantSuccessor.predecessorFoundation.checker.sha256);
assert.equal(sha256(invariantSuccessor.predecessorFoundation.freeze.path), invariantSuccessor.predecessorFoundation.freeze.sha256);
assert.equal(sha256(invariantSuccessor.successorAuthority.path), invariantSuccessor.successorAuthority.sha256);
assert.equal(sha256(invariantSuccessor.checkerRouting.frozenArtifactChecker), invariantSuccessor.checkerRouting.frozenArtifactCheckerSha256);
...
assert.equal(ckaW0.role, 'GOVERNED_CLIENT_QA_ENTRY_NOT_GENERIC_CHAT_NOT_REALITY_JOURNEY_NOT_METHOD_ENTRY');
...
assert.equal(ckaW4.governance.temporaryContextOnly, true);
assert.equal(ckaW4.governance.createsPersistentCase, false);
assert.equal(ckaW4.governance.automaticEscalation, false);
...
assert.equal(kapAnswer.acceptance.persistentCaseCreated, false);
assert.equal(ckaW4.guest.followUpAvailable, true);
assert.equal(ckaW4.guest.maximumTemporaryFollowUpDepth, 1);
assert.equal(ckaW4.guest.historyPersisted, false);
assert.equal(ckaW4.guest.shadowAccountCreated, false);
...
assert.equal(pkg.scripts['check:client-surface-invariants-frozen'], 'node scripts/check-client-surface-global-invariants-frozen-artifacts.mjs');

...
const contract=JSON.parse(fs.readFileSync('content/customer-experience-rebuild/r12r4b/smr/contracts/smr-semantic-dedup-contract-v1.json','utf8'));assert.deepEqual(contract.levels,['EVIDENCE','CLAIM','NARRATIVE']);assert.equal(contract.rules.evidenceLineageMayBeDeleted,false);assert.equal(contract.rules.crossSectionFullExplanationMax,1);

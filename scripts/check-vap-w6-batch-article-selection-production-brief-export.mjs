...
assert.equal(actualBatch.status, 'GOVERNED_ARTICLE_REVIEW_COHORT_AWAITING_PRODUCTION_GATES');
...
assert.equal(actualBatch.summary.c2BlockedCount, 6);
assert.equal(actualBatch.summary.humanProductionDecisionRequiredCount, 6);
assert.equal(actualBatch.summary.w4rExecutionEligibilityNotEstablishedCount, 6);
assert.equal(actualBatch.summary.productionBriefExportReadyCount, 0);
assert.deepEqual(actualBatch.summary.productionBriefExportReadyNodeCodes, []);
assert.equal(actualBatch.summary.currentBriefExportCount, 0);
assert.equal(actualBatch.entries.length <= 24, true);
assert.equal(actualBatch.humanDecisionRequest.required, true);
assert.equal(actualBatch.humanDecisionRequest.requiredNodeCount, 6);
...
for (const entry of actualBatch.entries) {
  assert.equal(entry.portfolioState, 'ARTICLE_DECISION_REQUIRED_HIGH_SIGNAL', `${entry.nodeCode}: only current high-signal Article decision candidates should be selected.`);
  assert.equal(entry.selectionStatus, 'SELECTED_FOR_GOVERNED_ARTICLE_DECISION_REVIEW');
  assert.equal(entry.humanProductionDecisionRequired, true);
  assert.equal(entry.articleEligibility.articleRoleAutoAssigned, false);
  assert.equal(entry.articleEligibility.status, 'ARTICLE_ELIGIBILITY_ASSESSMENT_REQUIRED_BEFORE_NEW_ARTICLE_DECISION');
  assert.equal(entry.readiness.productionReady, false);
  assert.ok(entry.readiness.blocking.includes('C2_THESIS_BOUNDARY_NOT_FROZEN'));
  assert.equal(entry.locale.requestedLocale, 'zh-Hans');
  assert.equal(entry.locale.readyForBrief, true);
  assert.equal(entry.executionEligibility.evaluatedByW4r, false);
  assert.equal(entry.executionEligibility.newArticleExecutionEligible, false);
  assert.equal(entry.productionBriefExport.ready, false);
  assert.equal(entry.productionBriefExport.status, 'BLOCKED_PENDING_GOVERNANCE');
  for (const blocker of [
    'KPP_ARTICLE_ELIGIBILITY_ASSESSMENT_REQUIRED',
    'HUMAN_KPP_PRODUCTION_DECISION_REQUIRED',
    'FROZEN_ARTICLE_PRODUCTION_PLAN_REQUIRED',
    'FROZEN_ARTICLE_PRODUCTION_WAVE_REQUIRED',
    'C2_THESIS_BOUNDARY_NOT_FROZEN',
    'C3_PRODUCTION_READINESS_NOT_PASSED',
    'W4R_NEW_ARTICLE_EXECUTION_ELIGIBILITY_NOT_ESTABLISHED'
  ]) assert.ok(entry.productionBriefExport.blockers.includes(blocker), `${entry.nodeCode}: ${blocker}`);
}
...
assert.equal(actualActivation.currentBatch.productionBriefExportReadyCount, 0);
assert.equal(actualActivation.currentBatch.currentBriefExportCount, 0);
assert.equal(actualActivation.currentBatch.currentStatus, 'NO_ELIGIBLE_NODES_FAIL_CLOSED');
...
assert.deepEqual(zhPlan.exportReadyNodeCodes, []);
assert.deepEqual(zhPlan.blockedNodeCodes, expectedCodes);
for (const entry of zhPlan.entries) assert.ok(entry.blockers.includes('W4R_NEW_ARTICLE_EXECUTION_ELIGIBILITY_NOT_ESTABLISHED'));
...
assert.ok(packageJson.scripts.postcheck.includes('npm run check:vap-w6'));

console.log('✓ VAP-W6 Batch Article Selection & Production Brief Export passed.');
console.log('✓ 6 high-signal Book 1 nodes form the first bounded Article decision-review cohort; mature PJA batch maximum remains 24.');
console.log('✓ Selection is recommendation-only: no Article role, Human approval, frozen plan, or frozen production wave is invented by VAP-W6.');
console.log('✓ All 6 selected nodes are zh-Hans locale-ready, but all remain blocked by C2/C3/Human/plan/wave/W4R execution gates; current production brief export count is correctly 0.');
console.log('✓ Existing PJA single-node production-brief exporter is reused byte-for-byte; VAP-W6 adds orchestration only.');
console.log('✓ Batch exporter is fail-closed: it exports only nodes with per-locale W4R new-Article execution eligibility and skips blocked nodes.');

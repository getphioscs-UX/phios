...
assert.deepEqual(bpRegistry.totals,{books:5,parts:16,canonicalNodes:718});
assert.deepEqual(Object.fromEntries(bpRegistry.books.map(b=>[b.bookCode,b.canonicalNodeCount])),{'BOOK-1':65,'BOOK-2':182,'BOOK-3':86,'BOOK-4':187,'BOOK-5':198});
const verifiedFreeze=await verifyKnowledgeBlueprintFreeze(process.cwd());
assert.equal(verifiedFreeze.freeze.registryManifestSHA,freeze.blueprintAuthority.registryManifestSha256);
assert.equal(verifiedFreeze.freeze.registryManifestSHA,bpRegistry.books.length?read('content/knowledge/blueprints/knowledge-blueprint-freeze-v2.json').registryManifestSHA:null);
...
console.log('  P10/P11 physical rehome remains deferred to BOOK-W1D/target source authority; no Article/PJA/CAR/VAP production authority is granted by KAU-R5.');

...
for(const [code,role,target] of expected){
 ...
 const assessment=read(item.c3AssessmentReference);assert.equal(assessment.nodeCode,code);assert.equal(assessment.status,'production_ready');assert.equal(assessment.humanProductionDecisionEligible,false);
 ...
 assert.equal(assessment.gates.humanProductionApproval.status,'passed');assert.equal(assessment.gates.exportability.status,'passed');assert.deepEqual(assessment.blocking,[]);
}
...
console.log('✓ Former Figure/Source blockers remain closed; current assessments have advanced to production_ready after Human Production Decision.');
console.log('✓ The historical preflight itself remains byte-preserved and did not create downstream authority.');

...
const requestManual=(url,timeoutMs=20000)=>new Promise((resolve,reject)=>{
  ...
  const req=https.request(url,{method:'GET',headers:{'user-agent':'PHI-OS-POC-B12-ACCEPTANCE/1.1','accept':'*/*'},timeout:timeoutMs},res=>{
...
const assert200=async url=>{const res=await requestManual(url); assert.equal(res.status,200,`${url} expected 200, got ${res.status}`); return res;};

const run=async()=>{
  console.log('POC-B7–B12 Final Production Successor Acceptance v1.1');
  ...
  assert.ok(sitemap.includes('https://getphios.com/'));
  assert.ok(!sitemap.includes('phios-github.pages.dev'));
  assert.ok(!sitemap.includes('www.getphios.com'));
  ...
  console.log('\n--- B10 public routes ---');
  const routes=['/','/explore/','/about/','/knowledge/','/knowledge/ask/','/articles','/books','/reality/','/perspectives/','/perspectives/personal/','/perspectives/relationship/','/professional/','/professional/financial/','/academy'];
  for(const route of routes) await assert200(`${ORIGIN}${route}`);

...
assert.match(js,/METHOD_REQUIREMENTS/);assert.match(js,/requirementsFor/);assert.match(js,/renderReadingMap/);assert.match(js,/req\.place&&!selectedCandidate/);assert.match(api,/const needsPlace=selected\.some/);assert.match(api,/location=null;if\(needsPlace\)/);assert.match(api,/const birthPlace=location\?/);assert.match(api,/source:'GOVERNED_RESOLUTION'/);assert.match(api,/source:'UNKNOWN'/);

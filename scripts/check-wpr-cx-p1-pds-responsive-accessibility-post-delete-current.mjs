...
const cutover = readJson('content/customer-experience-rebuild/migration/priority-route-cutover-registry-v3.json');
...
const files = new Set();
const p1RouteSuccessor = new Map();
for (const surface of cutover.surfaces) {
  for (const legacy of surface.legacyRoutes) p1RouteSuccessor.set(legacy, surface.htmlPath);
  p1RouteSuccessor.set(surface.canonicalPath, surface.htmlPath);
}
for (const record of web.productionRecords) {
  ...
  if (route === '/articles/:slug') {
    ...
    const successorFile = p1RouteSuccessor.get(route);
    const file = successorFile ?? routeFile(route);
    if (file) files.add(file);
  }
}
for (const retired of p1Delete.deletedPresentationFiles) assert.equal(exists(retired), false, `P1_RETIRED_PRESENTATION_REAPPEARED:${retired}`);
...
console.log('✓ WPR-W27 post-delete PDS / PX2 / CX-P1 Responsive and Accessibility Integration passed.');

...
const rebuiltQueue = await buildVapW10ApprovalQueue(root); assert.equal(serialize(rebuiltQueue), serialize(queue), 'VAP_W10_APPROVAL_QUEUE_NOT_DETERMINISTIC');
...
for (const nodeCode of VAP_W10_EXPECTED_NODE_CODES) {
  const q = queue.entries.find(item => item.nodeCode === nodeCode); assert(q); assert.equal(q.humanApprovalState, 'pending_human'); assert.equal(q.productionArticlePackageState, 'blocked_pending_human_approval'); assert.equal(q.review.decision, 'accept'); assert.equal(q.promotion.state, 'promoted_to_pja_approval_eligibility');
...
const pendingValidation = validateApprovalDecisionEnvelope(decisions, queue, { requireAllDecided: false }); assert.equal(pendingValidation.valid, true, JSON.stringify(pendingValidation.errors));
...
assert.equal(activation.status, 'AWAITING_EXPLICIT_HUMAN_APPROVAL'); assert.equal(activation.approvalEligibleCount, 6); assert.equal(activation.humanApprovalDecisionCount, 0); assert.equal(activation.humanApprovedCount, 0); assert.equal(activation.productionArticlePackageCount, 0); assert.equal(activation.publicationCount, 0);
...
const temp = await fs.mkdtemp(path.join(os.tmpdir(), 'vap-w10-approve-')); await fs.mkdir(path.join(temp, 'content/knowledge/production/registry'), { recursive: true }); await fs.writeFile(path.join(temp, VAP_W10_PATHS.pjaApprovalRegistry), serialize(approvalRegistry));
...
const tempApprovalRegistry = JSON.parse(await fs.readFile(path.join(temp, VAP_W10_PATHS.pjaApprovalRegistry), 'utf8')); assert.equal(tempApprovalRegistry.records.length, approvalRegistry.records.length + 6);
for (const nodeCode of VAP_W10_EXPECTED_NODE_CODES) {
...
const tempMixed = await fs.mkdtemp(path.join(os.tmpdir(), 'vap-w10-mixed-')); await fs.mkdir(path.join(tempMixed, 'content/knowledge/production/registry'), { recursive: true }); await fs.writeFile(path.join(tempMixed, VAP_W10_PATHS.pjaApprovalRegistry), serialize(approvalRegistry));
...
console.log('✓ Real Batch 001 has 6/6 accepted Human Reviews promoted to Approval Eligibility, with 0 Human Approvals and 0 Production Article Packages until explicit TL approval.');

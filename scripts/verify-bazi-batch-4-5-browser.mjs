...
const batchNumber=Number(process.argv[2]||4);if(![4,5].includes(batchNumber))throw Error('BATCH_REQUIRED');const pageNumbers=batchNumber===4?[20,21,22]:[23,24,25,26],range=batchNumber===4?'p20-p22':'p23-p26';
const root='docs/guided-report-successor-r1/batch-'+batchNumber;
...
const evidence={batch:'BAZI-DYNAMIC-R1-BATCH-0'+batchNumber,checkedAt:new Date().toISOString(),variants:[],print:[],previousBatchesVisualDiff:[],errors:[],humanVisualAcceptance:'PENDING'};
try{
 for(const locale of ['zh-Hans','en','bilingual']){
  for(const width of [1440,390]){
   ...
   if(width===1440){
    for(const n of pageNumbers){await page.selectOption('#page',String(n));if(await page.locator('#comparison').isHidden())await page.click('#compare');await page.locator('#comparison img').evaluate(img=>img.decode());await page.locator('#comparison').screenshot({path:`${root}/screenshots/compare-${locale}-P${n}.png`});}
    ...
    await page.pdf({path:`${root}/bazi-${range}-${locale}.pdf`,preferCSSPageSize:true,printBackground:true});
   }
   await page.close();
  }
 }
 // Compare the prior batch against committed captures, without rewriting them.
 for(const priorBatch of [1,2,3])for(const locale of ['zh-Hans','en','bilingual'])for(const width of [1440,390]){
  const page=await browser.newPage({viewport:{width,height:1000},deviceScaleFactor:1});
  ...
  for(const number of (priorBatch===1?[6,7,8,9,10]:priorBatch===2?[11,12,13,14,15]:[16,17,18,19])){
   ...
   if(!unchanged){
    ...
    const priorCss=fs.readFileSync('assets/customer-ui/surfaces/visual-report.css','utf8').split('/* BAZI-DYNAMIC-R1-BATCH-04-05')[0];

...
if(ids.length===50) run('scripts/mfig/build-mfig-production.mjs',['--raster']);
else for(const id of ids) run('scripts/mfig/build-mfig-production.mjs',['--raster','--id',id]);
for(const script of [
...
else for(const id of ids) run('scripts/mfig/record-mfig-machine-acceptance.mjs',['--id',id]);

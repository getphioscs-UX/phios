...
for(const id of requestedIds()){
 ...
 if(!raster){
  ...
  if(!fs.existsSync(pngPath(id))||!fs.existsSync(webpPath(id))) throw new Error(`${id}: raster outputs missing; rerun with --raster after npm ci installs the repository raster backend`);
 ...
 if(!fs.existsSync(pngPath(id))||!fs.existsSync(webpPath(id))) throw new Error(`${id}: raster outputs missing; run with --raster after npm ci installs the repository raster backend`);

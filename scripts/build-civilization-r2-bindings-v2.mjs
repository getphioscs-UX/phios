...
const assets=audit.rows.filter(r=>r.result==='VERIFIED_WEBP').map(r=>{
 const source=registry.assets.find(a=>a.assetId===r.assetId),prior=old.assets.find(a=>a.assetId===r.assetId),accepted=prior?.sha256===r.sha256;
...
console.log(`Verified delivery bindings ${assets.length}; previously accepted unchanged bytes ${assets.filter(a=>a.reviewState==='ACCEPTED').length}; human review remains pending.`);

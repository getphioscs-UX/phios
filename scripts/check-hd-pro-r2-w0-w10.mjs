...
for(const [path,proof] of Object.entries(sharedSuccessor.sharedFileSuccessorProof)){
  ...
  else if(currentSharedOwnerRegistry.files[path])assertCurrentPublishedOwner(path,{historicalDigest:proof.successorSha256,label:'HD shared current owner'});else if(w11.runtimeSuccessorProof[path])assert.equal(w11.runtimeSuccessorProof[path].predecessorSha256,proof.successorSha256,`HD W11 predecessor mismatch: ${path}`);else assert.equal(proof.successorSha256,sha(path),`HD shared successor digest drift without declared publication successor: ${path}`);
...
for(const [path,proof] of Object.entries(sharedSuccessor.addedRuntimeFiles)){
  ...
  else if(currentSharedOwnerRegistry.files[path])assertCurrentPublishedOwner(path,{historicalDigest:proof.sha256,label:'HD added current owner'});else if(w11.runtimeSuccessorProof[path])assert.equal(w11.runtimeSuccessorProof[path].predecessorSha256,proof.sha256,`HD W11 added-runtime predecessor mismatch: ${path}`);else assert.equal(proof.sha256,sha(path),`HD added runtime digest drift without declared publication successor: ${path}`);
}

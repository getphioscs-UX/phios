...
if(currentPersonalSha===successor.successorSha256){assert.equal(successor.requiredReplacement,'LOGO-001');}
 ...
 const w19=json('content/embodied-configuration/ecr-mandala-w19-scoped-css-successor-v1.json');
 assert.equal(w19.predecessorSha256,successor.successorSha256);
 assert.equal(w19.changeClass,'ECR_SCOPED_STYLESHEET_LINK_ONLY');
 assert.equal(w19.requiredStylesheet,'/assets/customer-ui/surfaces/ecr-specialist.css');
 if(w19.successorSha256!==currentPersonalSha){
  const current=json('content/embodied-configuration/ecr-mandala-w24-current-main-reconciliation-v1.json');
  assertPprCurrentSharedOwner('perspectives/personal/index.html',{historicalDigest:current.currentSha256,label:'ECR W24'});
  for(const historicalMarker of current.requiredCurrentMarkers){
   // Preserve the historical contract; the explicit icon successor owns this binding.
   const marker=historicalMarker==='data-cx-asset="LOGO-001"'?'data-cx-asset="CXICON-METHOD-ECR"':historicalMarker;
   assert(personal.includes(marker),'Current marker missing: '+marker);
  }
 }
 const currentCssSha=sha('assets/customer-ui/surfaces/ecr-specialist.css');
 if(w19.stylesheetSha256!==currentCssSha){
  const w24=json('content/embodied-configuration/ecr-mandala-w24-visual-css-successor-v1.json');
  assert.equal(w24.predecessorSha256,w19.stylesheetSha256);
  assertPprCurrentSharedOwner('assets/customer-ui/surfaces/ecr-specialist.css',{historicalDigest:w24.successorSha256,label:'ECR W24 CSS'});
  assert.equal(w24.changeClass,'W24_VISUAL_LANGUAGE_AND_CUSTOMER_FIRST_EXPLANATION_POLISH_ONLY');
 }else assert.equal(w19.stylesheetSha256,currentCssSha);
}
...
console.log('  The registered ECR icon supersedes the historical LOGO-001 placeholder through the current shared owner.');

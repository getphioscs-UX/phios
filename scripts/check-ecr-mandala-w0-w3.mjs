...
else{const w19=json('content/embodied-configuration/ecr-mandala-w19-scoped-css-successor-v1.json');assert.equal(w19.predecessorSha256,successor.successorSha256);assert.equal(w19.changeClass,'ECR_SCOPED_STYLESHEET_LINK_ONLY');assert.equal(w19.requiredStylesheet,'/assets/customer-ui/surfaces/ecr-specialist.css');if(w19.successorSha256!==currentPersonalSha){const current=json('content/embodied-configuration/ecr-mandala-w24-current-main-reconciliation-v1.json');assertPprCurrentSharedOwner('perspectives/personal/index.html',{historicalDigest:current.currentSha256,label:'ECR W24'});for(const marker of current.requiredCurrentMarkers)assert.match(personal,new RegExp(marker.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));}const currentCssSha=sha('assets/customer-ui/surfaces/ecr-specialist.css');if(w19.stylesheetSha256!==currentCssSha){const w24=json('content/embodied-configuration/ecr-mandala-w24-visual-css-successor-v1.json');assert.equal(w24.predecessorSha256,w19.stylesheetSha256);assertPprCurrentSharedOwner('assets/customer-ui/surfaces/ecr-specialist.css',{historicalDigest:w24.successorSha256,label:'ECR W24 CSS'});assert.equal(w24.changeClass,'W24_VISUAL_LANGUAGE_AND_CUSTOMER_FIRST_EXPLANATION_POLISH_ONLY');}else assert.equal(w19.stylesheetSha256,currentCssSha);}

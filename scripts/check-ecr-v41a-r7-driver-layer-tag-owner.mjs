...
const reconciliation = read(r7 + 'source-reconciliation-v2.json');
assert.equal(reconciliation.status, 'CORRECTS_R7_V1_AUDIT_SCOPE');
assert.equal(reconciliation.correctedConclusion.driverGenericSemanticSource, 'PRESENT_FOR_D1_D12');
assert.equal(reconciliation.correctedConclusion.personalityDesignLayerSemanticSource, 'PRESENT_AS_MULTI_SOURCE_CANDIDATE');
assert.equal(reconciliation.correctedConclusion.remainingWork, 'CANONICALIZATION_TAG_MAPPING_OWNER_ADMISSION');
assert.equal(reconciliation.ownerProvidedSources.length, 4);

const drivers = read(r7 + 'driver-role-candidates-v2.json');
...
assert(drivers.records.every(x => x.semanticStatus === 'SOURCE_SUPPORTED_CANDIDATE_PENDING_HUMAN_ADMISSION'));
assert(drivers.records.every(x => x.canonicalRoleMeaningCandidate?.en && x.canonicalRoleMeaningCandidate?.['zh-Hans']));
assert.equal(drivers.records.find(x => x.driverId === 'D11').mechanicalStatus, 'MECHANICAL_ACTIVATION_MAY_REMAIN_UNKNOWN_NON_BLOCKING');
assert.equal(drivers.records.find(x => x.driverId === 'D12').bodyBinding.length, 2);

const layers = read(r7 + 'layer-role-candidates-v2.json');
assert.deepEqual(layers.records.map(x => x.layer), ['PERSONALITY','DESIGN']);
assert(layers.records.every(x => x.semanticStatus === 'SOURCE_SUPPORTED_CANDIDATE_PENDING_HUMAN_ADMISSION'));
assert(layers.records.every(x => x.canonicalRoleMeaningCandidate?.en && x.canonicalRoleMeaningCandidate?.['zh-Hans']));
assert(layers.records.every(x => x.tagContributionPolicy === 'LAYER_MODIFIER_ONLY_UNTIL_EXPLICIT_TAG_MAPPING_ADMITTED'));
...
const review = read(r7 + 'human-review-package-v2.json');
assert.equal(review.status, 'SOURCE_RECONCILED_SUPERSEDES_V1_REVIEW');
assert.deepEqual(review.reviewGroups.map(x => x.items.length), [12,2,14,10,14,9,5,1]);
assert.equal(review.releaseBoundary.sourceGapReviewInvalidated, true);
assert.equal(review.releaseBoundary.driverLayerSourceDiscoveryClosed, true);
...
for (const row of review.generatedFrom.candidateFileBindings) {
  assert.equal(gitBlobSha(row.path), row.gitBlobSha, `R7 v2 candidate changed after W6 binding: ${row.path}`);
}
assert.equal(gitBlobSha(review.generatedFrom.sourceReconciliation.path), review.generatedFrom.sourceReconciliation.gitBlobSha);
...
const reviewHtml = readText('docs/ecr-human-runtime-v4-1/r7-driver-layer-tag-owner/review-v2.html');
for (const token of ['ACCEPT','REVISE','REJECT','SOURCE_SUPPORTED_CANDIDATE','ECR-V4.1A-R7-human-review-decisions-v2.json']) assert(reviewHtml.includes(token));

console.log('PASS ECR V4.1A R7 W0-W6 v2: prior sources reconciled; Driver and Personality/Design are source-supported candidates; operational/customer admission remains fail-closed.');

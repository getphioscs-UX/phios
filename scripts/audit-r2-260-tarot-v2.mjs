...
try{browser=await chromium.launch({channel:'msedge',headless:true});
 for(const viewport of [390,1440])for(const locale of ['zh-Hans','en']){
  ...
  assert.equal(bg.resolvedUrl,delivery.cardBack.url);observations.push({...bg,objectKey:new URL(bg.resolvedUrl).pathname.slice(1),route:'/perspectives/tarot/',viewport,locale,selector:'.cx-tarot-card-back',trigger:'QUESTION → SPREAD → SHUFFLE → FACEDOWN_SELECTION',evidenceClass:'CUSTOMER_CSS_BACKGROUND'});
  let previous=[];

...
import { ACCEPTANCE_PATH, MAP_PATHS, PUBLICATION_PATH, RECONCILIATION_PATH, buildBookW1DReconciliation } from './build-book-w1d-canonical-registry-reconciliation.mjs';
...
const [expected,actualReconciliation,actualPublication,actualAcceptance,nodesRaw,r5,w1Contract,w1cRegistry,w1cAcceptance,pkg,audit,...maps]=await Promise.all([
...
for(const [index,record] of mapRecords.entries()){
  ...
  assert.equal(record.humanAcceptance.status,'PENDING_W1B_HUMAN_CANONICAL_ACCEPTANCE');
...
assert.equal(actualAcceptance.status,'BLOCKED_PENDING_PRIOR_HUMAN_GATES');
...
for(const phrase of ['716 existing Canonical Node Authority','2 KAU-R5 Human-accepted admissions','0 silent deletion','473 publication ownership records','W1B and W1C Human Acceptance remain pending']) assert(audit.includes(phrase));
...
console.log('  W1B/W1C Human gates remain pending; nodes.json and active Blueprint authority were not mutated.');

...
const root=process.env.PHIOS_REPORT_REVIEW_ROOT||'docs/guided-report-successor-r2';fs.mkdirSync(`${root}/screenshots`,{recursive:true});
...
try{for(const locale of ['zh-Hans','en'])for(const width of [1440,390]){
 ...
 const expected=JSON.parse(fs.readFileSync(`${root}/bazi-${locale}.json`)).totalPages;
 if(metrics.length!==expected)evidence.errors.push(`PAGE_COUNT:${locale}:${width}`);
 for(const m of metrics){if(m.overflow)evidence.errors.push(`OVERFLOW:${locale}:${width}:${m.pageNumber}`);if(m.pageNumber>=6&&(m.pagination!==1||m.header!==1||m.footer!==1))evidence.errors.push(`CHROME:${locale}:${width}:${m.pageNumber}`);if(m.minBody&&m.minBody<16)evidence.errors.push(`FONT_MIN:${locale}:${width}:${m.pageNumber}`);if(/BAZI_FULL_REPORT:|PPR-C1-|CMP-|[a-f0-9]{32,}|OpenAI|DeepSeek|T[0-3]_|PROVIDER_|未选择目标时间|需要目标时间/.test(m.visibleText))evidence.errors.push(`INTERNAL_OR_PLACEHOLDER:${locale}:${m.pageNumber}`);if(!process.env.PHIOS_SCREENSHOT_PAGES||process.env.PHIOS_SCREENSHOT_PAGES.split(',').map(Number).includes(m.pageNumber))await page.locator(`#report [data-page-number="${m.pageNumber}"]`).screenshot({path:`${root}/screenshots/${locale}-${width}-P${String(m.pageNumber).padStart(2,'0')}.png`});delete m.visibleText;}
 const broken=await page.locator('#report img').evaluateAll(images=>images.filter(i=>!i.complete||!i.naturalWidth).map(i=>i.src));if(broken.length)evidence.errors.push('BROKEN_ASSET:'+JSON.stringify(broken));

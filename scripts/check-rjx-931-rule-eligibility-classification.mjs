...
for(const e of d.entries){assert.ok(allowed.has(e.proposedDisposition),e.nodeCode);assert.equal(e.activeRule,false,e.nodeCode);assert.equal(e.productionEffect,'none',e.nodeCode);assert.equal(e.canonicalMutation,false,e.nodeCode);assert.equal(e.humanAcceptance,null,e.nodeCode);if(e.proposedDisposition==='EXECUTABLE_RULE'||e.proposedDisposition==='PROFESSIONAL_GATED') assert.equal(e.reviewPriority,'TL_REVIEW_REQUIRED',e.nodeCode);}
assert.equal(d.accounting.automaticActiveRuleCount,0); assert.equal(d.authority.classificationIsCandidateOnly,true);

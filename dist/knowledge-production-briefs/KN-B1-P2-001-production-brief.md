# KN-B1-P2-001 Canonical Article Production Brief
...
```json
{
  ...
  "generatedAt": "2026-08-11T05:23:48.752Z",
...
```json
{
  ...
  "canonicalTitle": "为什么现实必须经过投影才能成为世界",
  "canonicalQuestion": "为什么现实必须经过投影才能成为世界",
  "nodeType": "mechanism_question",
  "status": "planned",
  "previousNode": "KN-B1-P1-012",
  "nextNode": [
    "KN-B1-P2-002"
  ],
  "relationships": {
...
```json
{
  "localizedTitle": "为什么现实必须经过投影才能成为世界",
  "localizedQuestion": "为什么现实必须经过投影才能成为世界",
  "localizedSummary": null,
  "searchAliases": [],
  "slug": "projection-makes-reality-experiencable-as-world"
}
```

## 4. Canonical Thesis

现实要成为一个系统能够经验、读取与行动的“世界”，必须先经过投影，因为任何有限载体都只能接收、压缩并组织现实的一部分，而不能直接拥有现实的全部状态。
...
- Reality ≠ World Projection
- Projection ≠ Illusion
- Selection ≠ Fabrication
- Interface ≠ Total Reality

### Must Not Claim

- 不得把投影描述为现实不存在或世界纯属虚构。
- 不得声称所有观察者的投影完全相同或完全任意。
- 不得把 PHI OS Projection 直接等同于某一哲学、心理学或物理学理论。
- 投影等于幻觉。
- 现实只能通过单一投影存在。

### Included Scope

- 有限载体、选择、压缩、映射、可访问现实与经验世界。

### Excluded Scope

- 不讨论特定感官、脑区、物理测量理论或个人心理诊断。
- 不进入后续投影偏差、群体协调的完整机制。
...
```json
{
  ...
  "nextNode": {
    ...
    "semanticBridge": "进入 KN-B1-P2-002，说明投影如何在表面形成可经验的界面。"
  },
  "supportingQuestions": [],
...
```json
{
  ...
  "noFabricationRules": {
    "internalCanonicalSources": [
      "content/knowledge/manuscripts/book-1/node-manuscript-mapping.json#KN-B1-P2-001"
    ],
    "externalSourceNeeds": [],
...
```json
{
  ...
  "whyThisNodeExists": "没有投影，有限系统面对的是无法直接承载的开放复杂性；投影把不可完全访问的现实转化为可被感知、比较、记忆与操作的世界版本。",

...
import {adaptAcceptedMethodResultForSmr} from '../functions/single-method-reading/accepted-method-result-adapter.js';
...
async function projection(methodId){
  ...
  if(methodId==='AST'){
    const output=await executeAndProjectAstV2({...common,methodCode:'ASTROLOGY',methodVersion:'0.1.0',canonicalInput:canonicalBase,executionParameters:{houseSystemCode:'PLACIDUS_V1'}},{astronomyModuleLoader});
    return output.canonicalProjection;
  }
  if(methodId==='BZR'){
    const output=await executeAndProjectMcd5CurrentRequest({...common,methodCode:'BAZI',methodVersion:'0.1.0',canonicalInput:canonicalBase,executionParameters:{traditionalCalculationSex:'FEMALE'}},{astronomyModuleLoader});
    return output.canonicalProjection;
  }
  if(methodId==='NUM'){
    const output=await executeAndProjectMcd5CurrentRequest({...common,methodCode:'NUMEROLOGY',methodVersion:'0.1.0-candidate',canonicalInput:{...canonicalBase,birthDate:'1990-01-15'},executionParameters:{targetDate:'2026-08-28'}},{astronomyModuleLoader});
    return output.canonicalProjection;
  }
  ...
  if(!response.ok||payload?.ok!==true)throw new Error(`SMR_ZWR_FIXTURE_FAILED:${JSON.stringify(payload)}`);
...
export async function acceptedSmrInput(methodId,locale='en'){
  ...
  if(!resultCache.has(key)){
    const methodResult=await buildAcceptedMethodCustomerResult({canonicalProjection:projectionCache.get(methodId),locale,requestedDepth:'STANDARD'});
    resultCache.set(key,{methodResult,acceptedInterpretationResult:adaptAcceptedMethodResultForSmr(methodResult)});
  }
  return resultCache.get(key);
}

export const SMR_METHODS=Object.freeze(['AST','BZR','NUM','ZWR']);
export const SMR_INTENTS=Object.freeze([
  {intentId:'OPEN',en:'Understand the most important structure without assuming a life situation.',zh:'在不假定现实处境的前提下，理解最重要的结构。'},
  {intentId:'DIRECTION',en:'Understand decision and direction patterns.',zh:'理解决策与方向的运行模式。'},
  {intentId:'WORK',en:'Explore work and resource themes where evidence exists.',zh:'在有证据的范围内理解工作与资源主题。'},
  {intentId:'RELATIONSHIP',en:'Explore relationship and exchange themes where evidence exists.',zh:'在有证据的范围内理解关系与交换主题。'},
  {intentId:'PRESSURE',en:'Understand pressure, friction and response patterns.',zh:'理解压力、摩擦与回应方式。'},
  {intentId:'EXPRESSION',en:'Understand expression and communication patterns.',zh:'理解表达与沟通模式。'},
  {intentId:'ENVIRONMENT',en:'Understand environment and support conditions.',zh:'理解环境与支持条件。'},
  {intentId:'OBSERVATION',en:'Identify observable signals to compare with reality.',zh:'找出现实中可以观察和核对的讯号。'}
]);

export function campaignVariants(perMethod=16){
  const output=[];
  for(let index=0;index<perMethod;index++){
    const locale=index%2===0?'en':'zh-Hans',intent=SMR_INTENTS[Math.floor(index/2)%SMR_INTENTS.length];
    output.push({variantIndex:index+1,locale,intentId:intent.intentId,customerIntent:{intentId:intent.intentId,prompt:locale==='zh-Hans'?intent.zh:intent.en}});
  }
  return output;
}
